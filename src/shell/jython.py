...
class JythonCommand(object):
    ...
    def jython_jars(self):
        ...
        elif not os.path.exists(os.path.join(self.jython_home, "jython3-kamalyes.jar")):
            bad_option("""{jython_home} contains neither jython-dev.jar nor jython3-kamalyes.jar.
...
setting {envvar_specifier}JYTHON_HOME.""".format(
            ...
            jars = [os.path.join(self.jython_home, "jython3-kamalyes.jar")]
    ...
    def command(self):
        ...
        if self.args.boot:
            args.append("-Xbootclasspath/a:%s" %
                        self.convert_path(self.make_classpath(jython_jars)))
        else:
        ...
        if "python.home" not in self.args.properties:
            args.append("-Dpython.home=%s" %
                        self.convert_path(self.jython_home))
        if "python.executable" not in self.args.properties:
            args.append("-Dpython.executable=%s" %
                        self.convert_path(self.executable))
        ...
        if self.uname == "cygwin" and "python.console" not in self.args.properties:
            args.append(
                "-Dpython.console=org.kamalyes.jython3.core.PlainConsole")
        if self.args.profile:
            args.append("-XX:-UseSplitVerifier")
            args.append("-javaagent:%s" %
                        self.convert_path(self.java_profile_agent))
        for k, v in self.args.properties.iteritems():
            args.append("-D%s=%s" % (self.convert(k), self.convert(v)))
        args.append("org.kamalyes.jython3.util.jython")
        if self.args.help:
            args.append("--help")
