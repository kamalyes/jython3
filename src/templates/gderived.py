...
python_dir = os.path.join(os.path.dirname(os.path.abspath(scriptdir)),
                          'org', 'python')
core_dir = os.path.join(python_dir, 'core')

DERIVED_HEADER = """\
/* Generated file, do not modify.  See /src/templates/gderived.py. */
package org.kamalyes.jython3.%s;

import java.io.Serializable;
import org.kamalyes.jython3.core.*;
import org.kamalyes.jython3.core.finalization.FinalizeTrigger;
import org.kamalyes.jython3.core.finalization.FinalizablePyObjectDerived;"""
...
class Gen:
    ...
    def get_aux(self, name):
        if self.auxiliary is None:
            ...
            directives.execute(directives.load(
                os.path.join(scriptdir, 'gderived-defs')), aux_gen)
            ...
            if isinstance(node, jast.VariableDeclaratorId):
                ...
                for child in node.children:
                    for x in visit(child):
                        yield x
        extra = jast_make(jast.Expressions, [jast_make(
            jast.Primary, Identifier=x, ArgumentsOpt=None) for x in visit(extraargs.fragment)])
        extra = JavaTemplate(extra)
        self.add_decl(
            ctr.tbind({'base': self.base_class, 'extraargs': extraargs, 'extra': extra}))
        self.ctr_done = 1
    ...
    def dire_unary1(self, name, parm, body):
        ...
        if len(parms) == 1:
            ...
            self.add_decl(unary_body.tbind({'unary': JavaTemplate(meth_name),
                                            'rettype_name':
                                            JavaTemplate(
                                                java_parser.make_qualid(rettype_name)),
    ...
    def dire_binary(self, name, parm, body):
        ...
        for meth_name in meth_names:
            self.add_decl(binary_body.tbind(
                {'binary': JavaTemplate(meth_name)}))
...
def hack_derived_header(fn, result):
    ...
    while True:
        ...
        dirs.insert(0, tail)
        if os_path_samefile(parent, python_dir) or not tail:
            break
