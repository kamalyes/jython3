...
public class DictComp extends expr {
  ...
  private final static PyUnicode[] fields = new PyUnicode[] { new PyUnicode("key"), new PyUnicode("value"),
      new PyUnicode("generators") };
  ...
  public void DictComp___init__(PyObject[] args, String[] keywords) {
    ArgParser ap = new ArgParser("DictComp", args, keywords,
        new String[] { "key", "value", "generators", "lineno", "col_offset" }, 3, true);
  ...
  public DictComp(Integer ttype, Token token, expr key, expr value, java.util.List<comprehension> generators) {
    super(ttype, token);
    this.key = key;
    addChild(key);
    this.value = value;
  ...
  public DictComp(PythonTree tree, expr key, expr value, java.util.List<comprehension> generators) {
    super(tree);
    this.key = key;
    addChild(key);
    this.value = value;
