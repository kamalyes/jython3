...
public class With extends stmt {
  ...
  private final static PyUnicode[] fields = new PyUnicode[] { new PyUnicode("items"), new PyUnicode("body") };
  ...
  public void With___init__(PyObject[] args, String[] keywords) {
    ArgParser ap = new ArgParser("With", args, keywords, new String[] { "items", "body", "lineno", "col_offset" }, 2,
        true);
  ...
  public With(Integer ttype, Token token, java.util.List<withitem> items, java.util.List<stmt> body) {
    super(ttype, token);
    this.items = items;
    if (items == null) {
      this.items = new ArrayList<withitem>();
