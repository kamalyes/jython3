...
public class cmath {
   ...
   * <i>e<sup>x</sup></i>, or equivalently 1 is negligible compared with
   * <i>e<sup>2x</sup></i>, in
   * IEEE-754 floating point. Beyond this, sinh <i>x</i> and cosh <i>x</i> are
   * adequately
   * approximated by 0.5<i>e<sup>x</sup></i>. The smallest theoretical value is 27
   * ln(2).
   */
  ...
  private static PyComplex complexFromPyObject(PyObject obj) {
    ...
    // If neither of the above works, interpret op as a float giving the real part
    // of
  ...
  /**
   * Return the arc cosine of w. There are two branch cuts. One extends right from
   * 1 along the
   * real axis to &infin;, continuous from below. The other extends left from -1
   * along the real
   ...
   * <p>
   * and we use {@link Math#atan2(double, double)} and {@link math#asinh(double)}
   * to obtain
   * <i>x</i> and <i>y</i>.
   * <p>
  ...
  private static PyComplex _acos(PyComplex w) {
    ...
    // If that generated a nan, and there wasn't one in the argument, raise a domain
    // error.
    return exceptNaN(new PyComplex(x, y), w);
  }

  /**
   * Return the hyperbolic arc cosine of w. There is one branch cut, extending
   * left from 1 along
  ...
  /**
   * Helper to compute z = cosh<sup>-1</sup><i>w</i>. The method used is as in
   * CPython:
   * <p>
   ...
   * <p>
   * and we use {@link math#asinh(double)} and {@link Math#atan2(double, double)}
   * to obtain
   * <i>x</i> and <i>y</i>.
   * <p>
   * For <i>w</i> sufficiently large that <i>w<sup>2</sup></i>&#x0226B;1,
   * cosh<sup>-1</sup><i>w</i> &asymp; ln(<i>2w</i>). We do not use this method
   * also to compute
  ...
  private static PyComplex _acosh(PyComplex w) {
    ...
    if (Math.abs(u) > 0x1p27 || Math.abs(v) > 0x1p27) {
      /*
       * w is large: approximate 2cosh(z) by exp(x+iy) or exp(-x-iy), whichever
       * dominates.
      ...
      /*
       * We're on the real axis (and maybe the branch cut). u = cosh x cos y. In all
       * cases,
  ...
  /**
   * Return the arc sine of w. There are two branch cuts. One extends right from 1
   * along the real
   * axis to &infin;, continuous from below. The other extends left from -1 along
   * the real axis to
  ...
  /**
   * Return the hyperbolic arc sine of w. There are two branch cuts. One extends
   * from 1j along the
   * imaginary axis to &infin;j, continuous from the right. The other extends from
   * -1j along the
  ...
  /**
   * Helper to compute either sin<sup>-1</sup><i>w</i> or
   * sinh<sup>-1</sup><i>w</i>. The method
   ...
   * <p>
   * Then, with <i>w = u+iv</i>, <i>z = x+iy</i>, <i>a =
   * a<sub>1</sub>+ia<sub>2</sub></i>, and
   ...
   * <p>
   * For <i>w</i> sufficiently large that <i>w<sup>2</sup></i>&#x0226B;1,
   * sinh<sup>-1</sup><i>w</i> &asymp; ln(<i>2w</i>). When computing
   * sin<sup>-1</sup><i>w</i>, we
   * evaluate <i>-i</i> sinh<sup>-1</sup><i>iw</i> instead.
   *
   * @param w
   * @param h <code>true</code> to compute sinh<sup>-1</sup><i>w</i>,
   *          <code>false</code> to
  ...
  private static PyComplex asinOrAsinh(PyComplex w, boolean h) {
    ...
    if (h) {
      ...
      v = w.real;
      u = -w.imag;
      // Then as before, the function body computes asinh(u+iv) = asinh(iw) = i
      // asin(w),
    ...
    if (Double.isNaN(u)) {
      ...
      /*
       * w is large: approximate 2sinh(z) by exp(x+iy) or -exp(-x-iy), whichever
       * dominates.
       * Hence, z = x+iy = ln(2(u+iv)) or -ln(-2(u+iv))
       */
      x = logHypot(u, v) + math.LN2;
    ...
    // Compose the result w according to whether we're computing asin(w) or
    // asinh(w).
    if (h) {
  ...
  /**
   * Return the arc tangent of w. There are two branch cuts. One extends from 1j
   * along the
   * imaginary axis to &infin;j, continuous from the right. The other extends from
   * -1j along the
   * imaginary axis to -&infin;j, continuous from the left.
  ...
  /**
   * Return the hyperbolic arc tangent of w. There are two branch cuts. One
   * extends from 1 along
   * the real axis to &infin;, continuous from below. The other extends from -1
   * along the real
   * axis to -&infin;, continuous from above.
   *
   * @param w
  ...
  /**
   * Helper to compute either tan<sup>-1</sup><i>w</i> or
   * tanh<sup>-1</sup><i>w</i>. The method
   * used is close to that used in CPython. For <i>z</i> =
   * tanh<sup>-1</sup><i>w</i>:
   * <p>
   ...
   * <i>y = </i>&frac12;tan<sup>-1</sup>(<i>2v /
   * ((1+u)(1-u)-v<sup>2</sup>)</i>)<br>
   * <p>
   * We use {@link math#log1p(double)} and {@link Math#atan2(double, double)} to
   * obtain <i>x</i>
   * and <i>y</i>. The second expression for <code>x</code> is used when
   * <i>u&lt;0</i>. For
   * <i>w</i> sufficiently large that <i>w<sup>2</sup></i>&#x0226B;1,
   * tanh<sup>-1</sup><i>w</i>
   * &asymp; 1/w &plusmn; <i>i&pi;/2</i>). For small <i>w</i>,
   * tanh<sup>-1</sup><i>w</i> &asymp;
   * <i>w</i>. When computing tan<sup>-1</sup><i>w</i>, we evaluate <i>-i</i>
   * tanh<sup>-1</sup><i>iw</i> instead.
   *
   * @param w
   * @param h <code>true</code> to compute tanh<sup>-1</sup><i>w</i>,
   *          <code>false</code> to
  ...
  private static PyComplex atanOrAtanh(PyComplex w, boolean h) {
    ...
    if (h) {
      ...
      v = w.real;
      u = -w.imag;
      // Then as before, the function body computes atanh(u+iv) = atanh(iw) = i
      // atan(w),
    ...
    if (absu >= 0x1p511 || absv >= 0x1p511) {
      ...
      // We don't need the imaginary part of 1/z. Just pi/2 with the sign of v. (If
      // not nan.)
      if (Double.isNaN(v)) {
        y = v;
      } else {
       ...
       * 0.5*ln((1+w)/(1-w)), which for positive u we rearrange as 0.5*ln(1+2w/(1-w))
       * and for
       * negative u as -0.5*ln(1-2w/(1+w)). By use of absu, we reduce the difference
       * between
    ...
    // Compose the result w according to whether we're computing atan(w) or
    // atanh(w).
    if (h) {
   ...
   *
   * @param z
   * @param h <code>true</code> to compute cosh <i>z</i>, <code>false</code> to
   *          compute cos
   *          <i>z</i>.
   * @return
   */
  private static PyComplex cosOrCosh(PyComplex z, boolean h) {
    ...
    if (y == 0.) {
      ...
      // Imaginary argument for cosh (or real for cos): imaginary result at this
      // point.
      u = Math.cos(y);
  ...
  public static PyComplex exp(PyObject z) {
    ...
    /*
     * This has a lot of corner-cases, and some of them make little sense sense, but
     * it matches
    ...
    // If that generated a nan, and there wasn't one in the argument, raise domain
    // error.
  ...
  /**
   * Returns the logarithm of <i>w</i> to the given base. If the base is not
   * specified, returns
   * the natural logarithm of <i>w</i>. There is one branch cut, from 0 along the
   * negative real
   * axis to -&infin;, continuous from above.
   *
   * @param w
  ...
  /**
   * Helper function for the log of a complex number, dealing with the log
   * magnitude, and without
   * intermediate overflow or underflow. It returns ln <i>r</i>, where
   * <i>r<sup>2</sup> =
   * u<sup>2</sup>+v<sup>2</sup></i>. To do this it computes
   * &frac12;ln(u<sup>2</sup>+v<sup>2</sup>). Special cases are handled as
   * follows:
   * <ul>
   ...
   * </ul>
   * We have this function instead of <code>Math.log(Math.hypot(u,v))</code>
   * because a valid
   * result is still possible even when <code>hypot(u,v)</code> overflows, and
   * because there's no
   ...
   *
   * @param z
   * @param h <code>true</code> to compute sinh <i>z</i>, <code>false</code> to
   *          compute sin
   *          <i>z</i>.
   * @return
   */
  private static PyComplex sinOrSinh(PyComplex z, boolean h) {
    ...
    if (y == 0.) {
      ...
      // Imaginary argument for sinh (or real for sin): imaginary result at this
      // point.
      v = Math.sin(y);
  ...
  /**
   * Calculate <i>z = x+iy</i>, such that <i>z<sup>2</sup> = w</i>. In taking the
   * square roots to
   * get <i>x</i> and <i>y</i>, we choose to have <i>x&ge;0</i> always, and
   * <i>y</i> the same sign
   * as <i>v</i>.
   *
  ...
  public static PyComplex sqrt(PyObject w) {
    /*
     * All the difficult parts are written for the first quadrant only (+,+), then
     * the true sign
     * of the parts of w are factored in at the end, by flipping the result around
     * the
     * diagonals.
     */
    PyComplex ww = complexFromPyObject(w);
    ...
    if (Double.isInfinite(u)) {
      ...
      if (v == 0.) {
         ...
         * computed as x**2 = s + a, and y = b /x. Most of the logic here is about
         * managing
         * the scaling.
         */
  ...
  /**
   * Helper to compute either tan <i>z</i> or tanh <i>z</i>. The expression used
   * is:
   * <p>
   * tanh(<i>x+iy</i>) = (sinh <i>x</i> cosh <i>x + i</i> sin <i>y</i> cos
   * <i>y</i>) /
   * (sinh<sup>2</sup><i>x +</i> cos<sup>2</sup><i>y</i>)
   * <p>
   * A simplification is made for x sufficiently large that
   * <i>e<sup>2|x|</sup></i>&#x0226B;1 that
   * deals satisfactorily with large or infinite <i>x</i>. When computing tan, we
   * evaluate
   * <i>i</i> tan <i>iz</i> instead, and the approximation applies to
   * <i>e<sup>2|y|</sup></i>&#x0226B;1.
   *
   * @param z
   * @param h <code>true</code> to compute tanh <i>z</i>, <code>false</code> to
   *          compute tan
   *          <i>z</i>.
  ...
  private static PyComplex tanOrTanh(PyComplex z, boolean h) {
    ...
    if (y == 0.) {
      ...
      // Imaginary argument for tanh (or real for tan): imaginary result at this
      // point.
  ...
  /**
   * Turn a <code>NaN</code> result into a thrown <code>ValueError</code>, a math
   * domain error, if
   * the original argument was not itself <code>NaN</code>. A
   * <code>PyComplex</code> is a
   * <code>NaN</code> if either component is a <code>NaN</code>.
   *
   * @param result to return (if we return)
   * @param arg    to include in check
   * @return result if <code>arg</code> was <code>NaN</code> or
   *         <code>result</code> was not
   *         <code>NaN</code>
   * @throws PyException (ValueError) if <code>result</code> was <code>NaN</code>
   *                     and
  ...
  /**
   * Raise <code>ValueError</code> if <code>result</code> is a <code>NaN</code>,
   * but neither
