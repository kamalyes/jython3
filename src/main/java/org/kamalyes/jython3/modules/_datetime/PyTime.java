...
import org.kamalyes.jython3.modules.time.PyTimeTuple;
...
public class PyTime extends PyObject {
  ...
  public String toString() {
    ...
    if (microsec == 0) {
      ...
      return String.format("%s(%d, %d, %d)", getType().fastGetName(), time.getHour(), time.getMinute(),
          time.getSecond());
    } else {
      return String.format("%s(%d, %d, %d, %d)", getType().fastGetName(), time.getHour(), time.getMinute(),
          time.getSecond(), microsec);
    }
  }
