...
class JITCompiler {
  ...
  JITHandle getHandle(PyObject resultType, PyObject[] parameterTypes, CallingConvention convention,
      boolean ignoreErrno) {
    ...
    for (int i = 0; i < hasParameterConverter.length; i++) {
      ...
      if (parameterType instanceof CType.Builtin) {
         ...
         * nativeParameterTypes[i] = ((MappedType)
         * parameterType).getRealType().getNativeType();
         */
      } else {
        return failedHandle;
      }
