...
import org.kamalyes.jython3.core.JyAttribute;
...
import org.kamalyes.jython3.core.TraverseprocDerived;
import org.kamalyes.jython3.core.Untraversable;
import org.kamalyes.jython3.core.Visitproc;
import org.kamalyes.jython3.core.finalization.FinalizeTrigger;
import org.kamalyes.jython3.modules._weakref.GlobalRef;
 ...
 * based on a
 * <a href="http://en.wikipedia.org/wiki/Tracing_garbage_collection" target=
 * "_blank">
 * mark-and-sweep-approach</a>.
 * </p>
 * <p>
 * This difference becomes most notable if finalizers are involved that perform
 * resurrection.
 * While the resurrected object itself behaves rather similar between Jython and
 * CPython,
 * things are more delicate with objects that are reachable (i.e. strongly
 * referenced)
 * via the resurrected object exclusively.
 * While in CPython such objects do not get their finalizers called, Jython/Java
 * would
 * call all their finalizers. That is because Java detects the whole unreachable
 * subgraph
 * as garbage and thus calls all their finalizers without any chance of direct
 * intervention.
 * CPython instead detects the unreachable object and calls its finalizer, which
 * makes the
 * object reachable again. Then all other objects are reachable from it and
 * CPython does not
 * treat them as garbage and does not call their finalizers at all.
 * This further means that in Jython weak references to such indirectly
 * resurrected objects
 * break, while these persist in CPython.
 * </p>
 * <p>
 * As of Jython 2.7, the gc-module offers some options to emulate
 * CPython-behavior.
 * Especially see the flags {@link #PRESERVE_WEAKREFS_ON_RESURRECTION},
 * {@link #DONT_FINALIZE_RESURRECTED_OBJECTS} and
 * {@link #DONT_FINALIZE_CYCLIC_GARBAGE}
 * for this.
 * </p>
 * <p>
 * Another difference is that CPython's gc-module offers some debug-features
 * like counting
 * of collected cyclic trash, which are hard to support by Jython. As of Jython
 * 2.7 the
 * introduction of a traverseproc-mechanism (c.f.
 * {@link org.kamalyes.jython3.core.Traverseproc})
 * made support of these features feasible. As support of these features comes
 * with a significant emulation-cost, one must explicitly tell gc to perform
 * this.
 * To make objects subject to cyclic trash-counting, these objects must be
 * gc-monitored in
 * Jython. See {@link #monitorObject(PyObject)},
 * {@link #unmonitorObject(PyObject)},
 * {@link #MONITOR_GLOBAL} and {@link #stopMonitoring()} for this.<br>
 * If at least one object is gc-monitored, {@link #collect()} works
 * synchronously in the
 * sense that it blocks until all gc-monitored objects that are garbage actually
 * have been
 * collected and had their finalizers called and completed. {@link #collect()}
 * will report
 * the number of collected objects in the same manner as in CPython, i.e. counts
 * only those
 * that participate in reference cycles. This allows a unified
 * test-implementation across
 * Jython and CPython (which applies to most tests in test_gc.py). If not any
 * object is
 * gc-monitored, {@link #collect()} just delegates to
 * {@link java.lang.System.gc()}, runs
 * asynchronously (i.e. non-blocking) and returns {@link #UNKNOWN_COUNT}.
 * See also {@link #DEBUG_SAVEALL} for a useful gc-debugging feature that is
 * supported by
 * Jython from version 2.7 onwards.
 * </p>
 * <p>
 * Implementing all these features in Jython involved a lot of synchronization
 * logic.
 * While care was taken to implement this without using timeouts as far as
 * possible and
 * rely on locks, states and system/hardware independent synchronization
 * techniques,
 * this was not entirely feasible.<br>
 * The aspects that were only feasible using a timeout are waiting for gc to
 * enqueue all
 * collected objects (i.e. weak references to monitored objects that were gc'ed)
 * to the
 * reference queue and waiting for gc to run all PyObject-finalizers.
 * </p>
 * <p>
 * Waiting for trash could in theory be strictly synchronized by using
 * {@code MXBean}s, i.e.
 * <a href=
 ...
 * However, experiments showed that the arising gc-notifications do not reliably
 * indicate
 * when enqueuing was done for a specific gc-run. We kept the experimental
 * implementation
 * in source-code comments to allow easy reproducibility of this issue. (Note
 * that out-commented
 * code contradicts Jython-styleguide, but this one - however - is needed to
 * document this
 * infeasible approach and is explicitly declared accordingly).<br>
 * But how <b>is</b> sync done now?
 * We insert a sentinel before running gc and wait until this sentinel was
 * collected.
 * Timestamps are taken to give us an idea at which time-scales the gc of the
 * current JVM
 * performs. We then wait until twice the measured time (i.e. duration from call
 * to
 * {@link java.lang.System#gc()} until the sentinel reference was enqueued) has
 * passed after
 * the last reference was enqueued by gc. While this approach is not entirely
 * safe in theory,
 * it passes all tests on various systems and machines we had available for
 * testing so far.
 * We consider it more robust than a fixed-length timeout and regard it the best
 * known feasible
 * compromise to emulate synchronous gc-runs in Java.
 * </p>
 * <p>
 * The other timing-based synchronization issue - waiting for finalizers to run
 * - is solved as
 * follows. Since PyObject-finalizers are based on
 * {@link org.kamalyes.jython3.core.finalization.FinalizeTrigger}s, Jython has
 * full control about
 * these finalization process from a central point. Before such a finalizer
 * runs, it calls
 * {@link #notifyPreFinalization()} and when it is done, it calls
 * {@link #notifyPostFinalization()}. While processing of a finalizer can be of
 * arbitrary
 * duration, it widely holds that Java's gc-thread calls the next finalizer
 * almost
 ...
 * {@link #notifyPostFinalization()} (i.e. that was called by the previous
 * finalizer).
 ...
 * This process can be disturbed by third-party finalizers of non-PyObjects
 * brought
 * into the process by external libraries. If these finalizers are of short
 * duration
 * (which applies to typical finalizers), one can deal with this by adjusting
 * {@link #postFinalizationTimeOut}, which was declared {@code public} for
 * exactly this
 * purpose. However if the external framework causing the issue is Jython-aware,
 * a
 * cleaner solution would be to let its finalizers call
 * {@link #notifyPreFinalization()}
 * and {@link #notifyPostFinalization()} appropriately. In that case these
 * finalizers
 * must not terminate by throwing an exception before
 * {@link #notifyPostFinalization()}
 * was called. This is a strict requirement, since a deadlock can be caused
 * otherwise.<br>
 * <br>
 * Note that the management API
 * (c.f.
 * <a
 * href="https://docs.oracle.com/javase/7/docs/jre/api/management/extension/index.html?com/sun/management/GcInfo.html
 * target="_blank">com.sun.management.GarbageCollectionNotificationInfo</a>)
 * does not emit any
 * notifications that allow to detect the end of the finalization-phase. So this
 * API
 * provides no alternative to the described technique.
 * </p>
 * <p>
 * Usually Java's gc provides hardly any guarantee about its collection- and
 * finalization-
 ...
 * target=
 * "_blank">http://howtodoinjava.com/2012/10/31/why-not-to-use-finalize-method-in-java</a>).
 * While at least the most common JVM implementations usually <b>do</b> call
 * finalizers
 * reliably under normal conditions, there still is no specific
 * finalization-order guaranteed
 * (one might reasonably expect that this would be related to
 * reference-connection graph
 ...
 * {@link #registerPostFinalizationProcess(Runnable)} and related methods one
 * can now
 * listen to beginning and end of the finalization process. Note that this
 * functionality
 * relies on the technique described in the former paragraph (i.e. based on
 * calls to
 * {@link #notifyPreFinalization()} and {@link #notifyPostFinalization()}) and
 * thus
 * underlies its unsafety, if third-party finalizers are involved. Such
 * finalizers can
 * cause false-positive runs of registered (pre/post)-finalization-processes, so
 * this
 * feature should be used with some care. It is recommended to use it only in
 * such a way
...
public class gc {
   ...
   * {@link org.kamalyes.jython3.core.Traverseproc} and
   * are not marked as {@link org.kamalyes.jython3.core.Untraversable}.
   ...
   * {@link org.kamalyes.jython3.core.Untraversable}.
   * </p>
   * <p>
   ...
   * that neither implements {@link org.kamalyes.jython3.core.Traverseproc}
   * nor is marked as {@link org.kamalyes.jython3.core.Untraversable} (even if
   ...
   * {@link org.kamalyes.jython3.core.Traverseproc},
   * nor are annotated with the
   * {@link org.kamalyes.jython3.core.Untraversable}-annotation.
   * </p>
   * <p>
   ...
   * traversed that neither implements
   * {@link org.kamalyes.jython3.core.Traverseproc},
   * nor is annotated with the
   * {@link org.kamalyes.jython3.core.Untraversable}-annotation.
   *
   * @see #setJythonGCFlags(short)
   * @see #getJythonGCFlags()
   ...
   * @see #removeJythonGCFlags(short)
   */
  public static final short VERBOSE = VERBOSE_COLLECT | VERBOSE_WEAKREF | VERBOSE_DELAYED | VERBOSE_FINALIZE;
  ...
  private static class WeakReferenceGC extends WeakReference<PyObject> {
    ...
    WeakReferenceGC(PyObject referent) {
      super(referent);
      cycleMark = (CycleMarkAttr) JyAttribute.getAttr(referent, JyAttribute.GC_CYCLE_MARK_ATTR);
    ...
    public boolean equals(Object ob) {
      ...
      if (ob instanceof WeakReferenceGC) {
        ...
        if (ownReferent == null ||
          ...
          return ownReferent == ((WeakrefGCCompareDummy) ob).compare &&
          // We compare the cached hash-codes in order to get an idea
          // whether in the both-null-case the referent was equal once.
  ...
  /**
   * Works like {@link org.kamalyes.jython3.core.Py#writeDebug(String, String)},
   * but prints to {@link org.kamalyes.jython3.core.Py#writeDebug(String, String)}
   ...
   * @see org.kamalyes.jython3.core.Py#writeDebug(String, String)
   */
  ...
  private static class DelayedFinalizationProcess implements Runnable {
    ...
    private void performFinalization(PyObject del) {
      ...
      FinalizeTrigger ft = (FinalizeTrigger) JyAttribute.getAttr(del, JyAttribute.FINALIZE_TRIGGER_ATTR);
      if (ft != null) {
    ...
    private void restoreFinalizer(PyObject obj, boolean cyclic) {
      FinalizeTrigger ft = (FinalizeTrigger) JyAttribute.getAttr(obj, JyAttribute.FINALIZE_TRIGGER_ATTR);
      FinalizeTrigger.ensureFinalizer(obj);
      boolean notify = false;
      if (ft != null) {
        ((FinalizeTrigger) JyAttribute.getAttr(obj, JyAttribute.FINALIZE_TRIGGER_ATTR)).flags = ft.flags;
      ...
      CycleMarkAttr cm = (CycleMarkAttr) JyAttribute.getAttr(obj, JyAttribute.GC_CYCLE_MARK_ATTR);
    ...
    public void run() {
      ...
      if ((gcFlags & PRESERVE_WEAKREFS_ON_RESURRECTION) != 0) {
        ...
        for (PyObject rst : criticReachablePool) {
          toRestore = (GlobalRef) JyAttribute.getAttr(rst, JyAttribute.WEAK_REF_ATTR);
          if (toRestore != null) {
    ...
    public void run() {
      ...
      while (true) {
        if (!lockPostFinalization && openFinalizeCount == 0
            && current - postFinalizationTimestamp > postFinalizationTimeOut) {
          break;
        }
        try {
   ...
   * {@link org.kamalyes.jython3.core.finalization.FinalizeTrigger}s;
   * to care for other finalizers these must call
   * {@code gc.notifyPreFinalization()} before anything else is done and
   * {@code gc.notifyPostFinalization()} afterwards; between these calls the
   * finalizer
   ...
   * enqueued
   * after finalization of their referents, but this happens in another gc-cycle
   * then.
   * </p>
   * <p>
   * Actually there are still situations that can cause pre-finalization process
   * to
   * run again during finalization phase. This can happen if external frameworks
   * use
   * their own finalizers. This can be cured by letting these finalizers call
   * {@code gc.notifyPreFinalization()} before anything else is done and
   * {@code gc.notifyPostFinalization()} right before the finalization method
   * returns.
   * Between these calls the finalizer must not terminate by throwing an
   * exception.
   * </p>
   * <p>
  ...
  /**
   * Useful if a process wants to remove another one or itself during its
   * execution.
  ...
  public static void notifyPreFinalization() {
    ...
    if (postFinalizationProcess == null)
      return;
    synchronized (postFinalizationProcess) {
  ...
  public static void monitorObject(PyObject ob, boolean initString) {
    ...
    // In contrast to isTraversable(ob) we don't look for
  ...
  public static boolean unmonitorObject(PyObject ob) {
    try {
      synchronized (monitoredObjects) {
        ...
        FinalizeTrigger ft = (FinalizeTrigger) JyAttribute.getAttr(ob, JyAttribute.FINALIZE_TRIGGER_ATTR);
        if (ft != null) {
   ...
   * <br>
   * {@code flags} is a {@code short} and can have the following bits turned
   * on:<br>
   * <br>
   * {@link #MONITOR_GLOBAL} - Automatically monitors all PyObjects created from
   * now on.<br>
   * {@link #DONT_FINALIZE_CYCLIC_GARBAGE} - Adds cyclic finalizable PyObjects to
   * {@link #garbage}.<br>
   * {@link #PRESERVE_WEAKREFS_ON_RESURRECTION} - Keeps weak references alive if
   * the referent is resurrected.<br>
   * {@link #DONT_FINALIZE_RESURRECTED_OBJECTS} -
   * Emulates CPython behavior regarding resurrected objects and finalization.<br>
   * {@link #DONT_TRAVERSE_BY_REFLECTION} - Inhibits reflection-based
   * traversion.<br>
   * {@link #SUPPRESS_TRAVERSE_BY_REFLECTION_WARNING} -
   * Suppress warnings for PyObjects that neither implement
   * {@link org.kamalyes.jython3.core.Traverseproc} nor
   * are marked as {@link org.kamalyes.jython3.core.Untraversable}.<br>
   * {@link #USE_PY_WRITE_DEBUG} - uses
   * {@link org.kamalyes.jython3.core.Py#writeDebug(String, String)} for
   * debugging output instead of directly writing to
   * {@link java.lang.System#err}.<br>
   ...
   * {@link #VERBOSE_DELAYED} - Enable delayed finalization-related verbose
   * output.<br>
   ...
   * {@link org.kamalyes.jython3.core.finalization.FinalizeTrigger}.
   */
   ...
   * Throws {@link org.kamalyes.jython3.core.Py#NotImplementedError}.
   *
   * @throws org.kamalyes.jython3.core.Py.NotImplementedError
   */
  ...
  private static void syncCollect(int[] stat, boolean debugStat) {
    ...
    synchronized (monitoredObjects) {
      ...
      for (WeakReferenceGC wrg : monitoredObjects) {
        ...
        if (wrg.hasFinalizer) {
          ft = (FinalizeTrigger) JyAttribute.getAttr(wrg.get(), JyAttribute.FINALIZE_TRIGGER_ATTR);
          ft.flags |= FinalizeTrigger.NOTIFY_GC_FLAG;
      ...
      if ((debugFlags & DEBUG_SAVEALL) != 0
        ...
        for (WeakReferenceGC wrg : cyclic) {
          if (!wrg.hasFinalizer) {
            ...
            FinalizeTrigger.ensureFinalizer(obj);
            wrg.updateHasFinalizer();
            ft = (FinalizeTrigger) JyAttribute.getAttr(obj, JyAttribute.FINALIZE_TRIGGER_ATTR);
     ...
     * // reason for it.
     * // collectSyncViaMXBean uses inofficial API, i.e. classes from
     * com.sun.management.
     * // In case that a JVM does not provide this API, we have a less elegant
     * fallback
    ...
    if (postFinalizationPending) {
      ...
      // block. Otherwise the notification might come just between the check and the
      // wait,
   ...
   * private static List<WeakReferenceGC> collectSyncViaMXBean(int[] stat,
   * Set<WeakReferenceGC> cyclic) {
   ...
   * }
   * if (waitForGCNotification == null || waitForGCNotification.length !=
   * initialGCCounts.length) {
   ...
   * if (cyclic.contains(trash) && !((WeakReferenceGC)
   * trash).cls.contains("Java")) {
   * ++stat[0];
   * if (collectBuffer != null) {
  ...
  private static List<WeakReferenceGC> collectSyncViaSentinel(int[] stat, Set<WeakReferenceGC> cyclic) {
    WeakReference<GCSentinel> sentRef = new WeakReference<>(new GCSentinel(Thread.currentThread()), gcTrash);
    Reference<? extends Object> trash;
    System.gc();
    List<WeakReferenceGC> collectBuffer = null;
    if (needsCollectBuffer()) {
  ...
  /**
   * Returns all objects from {@code pool} that are part of reference cycles as a
   * new set.
   * If a reference-cycle is not entirely contained in {@code pool}, it will be
   * entirely
   * contained in the resulting set, i.e. missing participants will be added.
   * This method completely operates on weak references to ensure that the
   * returned
   * set does not manipulate gc-behavior.
   * 
   * Note that this method is not thread-safe. Within the gc-module it is only
   * used
  ...
  private static IdentityHashMap<PyObject, WeakReferenceGC> removeNonCyclicWeakRefs(Iterable<WeakReferenceGC> pool) {
    ...
    PyObject referent;
    if (monitorNonTraversable) {
      // this means there might be non-traversable objects in the pool we must filter
      // out now
    ...
    // pools[0] should now be a closed set in the sense that it contains all
    // PyObjects
  ...
  private static IdentityHashMap<PyObject, PyObject> findCyclicObjectsIntern(PyObject start) {
    ...
    @SuppressWarnings("unchecked")
    IdentityHashMap<PyObject, PyObject>[] reachSearch = (IdentityHashMap<PyObject, PyObject>[]) new IdentityHashMap[2];
    ...
    // reachSearch[0] is now the reachable set, but still contains non-cyclic
    // objects
   ...
   * {@link org.kamalyes.jython3.core.PyObject}
   * {@code ob}. It exploits both
   * {@link org.kamalyes.jython3.core.Traverseproc#traverse(Visitproc, Object)}
   * and
   * {@link org.kamalyes.jython3.core.TraverseprocDerived#traverseDerived(Visitproc, Object)}.
   * If {@code ob} neither implements
   * {@link org.kamalyes.jython3.core.Traverseproc} nor
   * {@link org.kamalyes.jython3.core.Traverseproc} and is not annotated with
   * {@link org.kamalyes.jython3.core.Untraversable}, reflection-based traversion
   * via
   * {@link #traverseByReflection(Object, Visitproc, Object)} may be attempted
   * according to {@link #DONT_TRAVERSE_BY_REFLECTION}.
   *
   * @see org.kamalyes.jython3.core.Traverseproc#traverse(Visitproc, Object)
   * @see org.kamalyes.jython3.core.TraverseprocDerived#traverseDerived(Visitproc,
   *      Object)
   * @see #DONT_TRAVERSE_BY_REFLECTION
   * @see org.kamalyes.jython3.core.Untraversable
      ...
      if (!(ob instanceof Traverseproc || ob instanceof TraverseprocDerived ||
        ...
        if (((gcFlags & INSTANCE_TRAVERSE_BY_REFLECTION_WARNING) != 0) ||
          ...
          Py.writeWarning("gc", "The PyObject-subclass " + ob.getClass().getName() + "\n" +
              ...
              "javadoc of org.kamalyes.jython3.core.Traverseproc.java.");
        }
      }
    }
  ...
  private static class ReferentsFinder implements Visitproc {
    ...
    public int visit(PyObject object, Object arg) {
      ((org.kamalyes.jython3.core.PySequenceList) arg).pyadd(object);
      return 0;
    }
  }
  ...
  private static class ReachableFinder implements Visitproc {
    ...
    @SuppressWarnings("unchecked")
    public int visit(PyObject object, Object arg) {
      IdentityHashMap<PyObject, PyObject>[] reachSearch = (IdentityHashMap<PyObject, PyObject>[]) arg;
  ...
  private static class ReachableFinderWeakRefs implements Visitproc {
    ...
    public int visit(PyObject object, Object arg) {
      if (isTraversable(object)) {
        IdentityHashMap<PyObject, WeakReferenceGC>[] pools = (IdentityHashMap<PyObject, WeakReferenceGC>[]) arg;
  ...
  private static class ReferrerFinder implements Visitproc {
    ...
    public int visit(PyObject object, Object arg) {
      if (((PyObject[]) arg)[0].__eq__(object).__bool__()) {
        ((org.kamalyes.jython3.core.PySequenceList) ((PyObject[]) arg)[1]).pyadd(object);
      }
      return 0;
    }
  }
  ...
  /**
   * Like {@code RefInListFinder} this visitproc looks whether the traversed
   * object
   ...
   * for each referent, whether it is in {@code arg[0]}. If it is, then it is
   * added
  ...
  private static class ObjectInListFinder implements Visitproc {
    ...
    public int visit(PyObject object, Object arg) {
      @SuppressWarnings("unchecked")
      IdentityHashMap<PyObject, PyObject>[] pools = (IdentityHashMap<PyObject, PyObject>[]) arg;
