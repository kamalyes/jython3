...
package org.kamalyes.jython3.modules.sre;
...
public class SRE_STATE {
  ...
  /*
   * Generated from Python-2.4.5 like 'python headerToJava.py <
  ...
  public static final int SRE_MAGIC = 20140917;
  ...
  public static final int SRE_OP_RANGE_IGNORE = 32;
  ...
  public static final int SRE_FLAG_DEBUG = 128;
  public static final int SRE_FLAG_ASCII = 256;
  ...
  // From here we're including things from _sre.c in the order they're defined
  // there
  ...
  final boolean sre_category(long category, int ch) {
    switch ((int) category) {
  ...
  private int mark_save(int lo, int hi) { // XXX => data_stack_grow in 2.4
    ...
    // TRACE(0, ptr, "copy " + lo + ":" + hi + " to " + mark_stack_base + " (" +
    // size + ")");
  ...
  final boolean SRE_AT(int ptr, int at) {
    ...
    switch (at) {
      ...
      case SRE_AT_BOUNDARY:
        ...
        if (beginning == end)
          return false;
        thatp = (ptr > beginning) ? SRE_IS_WORD(str[ptr - 1]) : false;
        thisp = (ptr < end) ? SRE_IS_WORD(str[ptr]) : false;
        return thisp != thatp;
      ...
      case SRE_AT_UNI_BOUNDARY:
        if (beginning == end)
          return false;
        thatp = (ptr > beginning) ? SRE_LOC_IS_WORD(str[ptr - 1]) : false;
        thisp = (ptr < end) ? SRE_LOC_IS_WORD(str[ptr]) : false;
        return thisp != thatp;
  ...
  final boolean SRE_CHARSET(long[] set, int setidx, int ch) {
    ...
    for (;;) {
      switch ((int) set[setidx++]) {
        ...
        case SRE_OP_BIGCHARSET:
          ...
          int count = (int) set[setidx++];
          int block;
          if (isBytes) {
            block = (int) set[setidx + (ch >> 8)];
            setidx += 128;
            ...
            if (ch < 65536)
              ...
              block = -1;
            setidx += 64;
            if (block >= 0 && (set[setidx + (block * 256 + (ch & 255)) / 32] & (1 << (ch & 31))) != 0)
              return ok;
            setidx += count * 8;
          }
          break;
  ...
  private int SRE_COUNT(long[] pattern, int pidx, int maxcount, int level) {
    int chr;
    int ptr = this.ptr;
    int end = this.end;
    int i;
  ...
  final int SRE_MATCH(long[] pattern, int pidx, int level) {
    return SRE_MATCH(pattern, pidx, level, false);
  }

  final int SRE_MATCH(long[] pattern, int pidx, int level, boolean fullmatch) {
    /*
    ...
    for (;;) {
      ...
      switch ((int) pattern[pidx++]) {
        ...
        case SRE_OP_LITERAL_IGNORE:
          ...
          if (ptr >= end || lower(str[ptr]) != lower((int) pattern[pidx]))
            return 0;
          pidx++;
          ptr++;
          break;
        ...
        case SRE_OP_REPEAT_ONE:
          ...
          int mincount = (int) pattern[pidx + 1];
          ...
          count = SRE_COUNT(pattern, pidx + 3, (int) pattern[pidx + 2],
              level + 1);
          if (count < 0)
            return count;
          ...
          if (pattern[pidx + (int) pattern[pidx]] == SRE_OP_SUCCESS) {
          ...
          if (pattern[pidx + (int) pattern[pidx]] == SRE_OP_LITERAL) {
            /*
             * tail starts with a literal. skip positions where
             * the rest of the pattern cannot possibly match
             */
            chr = (int) pattern[pidx + (int) pattern[pidx] + 1];
            for (;;) {
              ...
              if (count < mincount)
                break;
              this.ptr = ptr;
              i = SRE_MATCH(pattern, pidx + (int) pattern[pidx],
                  level + 1, fullmatch);
              if (i != 0) {
                if (fullmatch && ptr != end)
                  return 0;
        ...
        case SRE_OP_MIN_REPEAT_ONE:
          ...
          if (pattern[pidx + 1] == 0)
            ...
            count = SRE_COUNT(pattern, pidx + 3, (int) pattern[pidx + 1],
                level + 1);
          ...
          if (!fullmatch && pattern[pidx + (int) pattern[pidx]] == SRE_OP_SUCCESS) {
            /* tail is empty. we're finished */
            this.ptr = ptr;
            return 1;
          } else {
            ...
            while (matchmax || count <= pattern[pidx + 2]) {
              this.ptr = ptr;
              i = SRE_MATCH(pattern, pidx + (int) pattern[pidx], level + 1, fullmatch);
              if (i != 0)
                return i;
              this.ptr = ptr;
        ...
        case SRE_OP_ASSERT:
          ...
          this.ptr = ptr - (int) pattern[pidx + 1];
  ...
  int SRE_SEARCH(long[] pattern, int pidx) {
    int ptr = this.start;
    int end = this.end;
    int status = 0;
    int prefix_len = 0;
    ...
    if (pattern[pidx] == SRE_OP_INFO) {
      ...
      if ((flags & SRE_INFO_PREFIX) != 0) {
        ...
        prefix_len = (int) pattern[pidx + 5];
        prefix_skip = (int) pattern[pidx + 6];
        prefix = pidx + 7;
  ...
  // XXX - this should get hot-spotted out, but that can take some time. Devs
  // should just re-enable.
  ...
  // if (do_trace) {
  // System.out.println(" |" + pidx + "|" + Integer.toHexString(ptr) + ": " +
  // string);
  // }
  // }
}
