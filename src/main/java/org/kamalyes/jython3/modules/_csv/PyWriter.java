...
public class PyWriter extends PyObject implements Traverseproc {
  ...
  final boolean writer_writerow(PyObject seq) {
    ...
    for (i = 0; i < len; i++) {
      ...
      if (field instanceof PyBytes) {
        ...
        PyObject str;
        // TODO: in 3.x this check can go away and we can just always use
        // __str__
  ...
  /**
   * This method behaves differently depending on the value of copy_phase: if
   * copy_phase
   * is false, then the method determines the new record length. If copy_phase is
   * true
