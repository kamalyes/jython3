...
import org.kamalyes.jython3.core.PyBuiltinFunctionNarrow;
...
import org.kamalyes.jython3.modules._io.OpenMode;
import org.kamalyes.jython3.modules._io.PyFileIO;
import org.kamalyes.jython3.util.FilenoUtil;
import org.kamalyes.jython3.util.PosixShim;
...
public class PosixModule {
  ...
  private static class UnixSystem {
     ...
     * com.sun.security.auth.module.UnixSystem will prevent Jython compiling on
     * Windows, where
     * it is not supplied. We therefore take a reflective approach where creation of
     * a singleton
     * instance is allowed to fail, and looks like the non-availability of the
     * function sought.
     * This shouldn't arise if we've used the @Hide(OS.NT) annotation to avoid
     * exposure of
  ...
  // Combine Java FileDescriptor objects with Posix int file descriptors in one
  // representation.
  ...
  // getFD(fd).getIntFD();
  // throw Py.NotImplementedError("Integer file descriptors not currently
  // supported for fdopen");
  // }
  ...
  public static PyObject strerror(int code) {
    ...
    if (errno.name() == errno.toString()) {
      ...
      // TODO: have jnr-constants handle this fallback
  ...
  /**
   * Convert seconds (with a possible fraction) from epoch to a 2 item array of
   * seconds,
  ...
  public static int write(PyObject fd, BufferProtocol bytes) {
    ...
    try (PyBuffer buf = bytes.getBuffer(PyBUF.ND)) {
      // Get a ByteBuffer of that data, setting the position and limit to the real
      // data.
  ...
  /**
   * Helper function for the subprocess module, returns the potential shell
   * commands for
   * this OS.
   *
  ...
  /**
   * Return the absolute, normalised form of path, equivalent to Python
   * os.path.abspath(), except
   * that it is an error for pathObj to be an empty string or unacceptable in the
   * file system.
   *
  ...
  private static Path absolutePath(PyObject pathObj) {
    ...
    if (pathStr.equals("")) {
      // Returning current working directory would be wrong in our context (chdir,
      // etc.).
  ...
  private static Path absolutePath(String pathStr) {
    try {
      ...
      /*
       * Thrown on Windows for paths like foo/bar/<test>, where <test> is the literal
       * text,
       * not a metavariable :) NOTE: CPython, Windows throws the Windows-specific
       * internal
       * error WindowsError [Error 123], but it seems excessive to duplicate this
       * error
       * hierarchy.
       */
    ...
    public PyObject __call__(PyObject path) {
      ...
      try {
        ...
        if (isSymbolicLink != null && isSymbolicLink.booleanValue() && path.toString().endsWith("/")) {
          // Chase the symbolic link, but do not follow further - this is a special case
          // for lstat
  ...
  // in particular this is driven by the fact that Windows CRT does not properly
  // handle
