...
import org.kamalyes.jython3.core.PyNone;
...
/**
 * This class corresponds to the Python _codecs module, which in turn lends its
 * functions to the
 * codecs module (in Lib/codecs.py). It exposes the implementing functions of
 * several codec families
 * called out in the Python codecs library Lib/encodings/*.py, where it is
 * usually claimed that they
 * are bound "as C functions". Obviously, C stands for "compiled" in this
 * context, rather than
 * dependence on a particular implementation language. Actual transcoding
 * methods often come from
...
public class _codecs {
  ...
  /**
   * Decode <code>bytes</code> using the codec registered for the
   * <code>encoding</code>. The
   * <code>encoding</code> defaults to the system default encoding (see
   * {@link codecs#getDefaultEncoding()}). The string <code>errors</code> may name
   * a different
  ...
  /**
   * Encode <code>unicode</code> using the codec registered for the
   * <code>encoding</code>. The
   * <code>encoding</code> defaults to the system default encoding (see
   * {@link codecs#getDefaultEncoding()}). The string <code>errors</code> may name
   * a different
  ...
  /**
   * Enumeration representing the possible endianness of UTF-32 (possibly UTF-16)
   * encodings.
   * Python uses integers <code>{-1, 0, 1}</code>, but we can be more expressive.
   * For encoding
   * UNDEFINED means choose the endianness of the platform and insert a byte order
   * mark (BOM). But
   * since the platform is Java, that is always big-endian. For decoding it means
   * read the BOM
  ...
  enum ByteOrder {
    ...
    /**
     * Returns equivalent to the Python code -1 = LE, 0 = as marked/platform, +1 =
     * BE
     */
  ...
  /**
   * Convenience method to construct the return value of decoders, providing the
   * Unicode result as
   ...
   * Convenience method to construct the return value of decoders, providing the
   * Unicode result as
   * a String, and the number of bytes consumed in decoding as either a
   * single-element array or an
   * int to be used if the array argument is null.
   *
   * @param u           the unicode result as a UTF-16 Java String
   * @param consumed    if not null, element [0] is the number of bytes consumed
   * @param defConsumed if consumed==null, use this as the number of bytes
   *                    consumed
   * @return the tuple (unicode(u), bytesConsumed)
   */
  ...
  /**
   * Convenience method to construct the return value of decoders that infer the
   * byte order from
   * the byte-order mark.
   *
   * @param u             the unicode result as a UTF-16 Java String
  ...
  /**
   * Decode a sequence of bytes into Unicode characters via a mapping supplied as
   * a container to
  ...
  public static PyObject translateCharmap(PyUnicode str, String errors, PyObject mapping) {
    ...
    for (Iterator<Integer> iter = str.newSubsequenceIterator(); iter.hasNext();) {
      ...
      if (result == null) {
        ...
        // TODO: We don't support the fancier error handling CPython does here of
   ...
   * <code>EncodingMap</code> of 256 entries, or an arbitrary container indexable
   * with integers
   * using <code>__finditem__</code> and yielding byte strings. If the mapping is
   * null, latin-1
   * (effectively a mapping of character code to the numerically-equal byte) is
   * used
   *
  ...
  /**
   * Helper to implement the several variants of <code>charmap_encode</code>,
   * given an optional
   * mapping. This mapping is either an <code>EncodingMap</code> of 256 entries,
   * or an arbitrary
   * container indexable with integers using <code>__finditem__</code> and
   * yielding byte strings.
   *
  ...
  private static PyTuple charmap_encode_internal(String str, String errors, PyObject mapping,
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (x == null) {
        ...
        // TODO:: (Py3k) Look-up had bytes or str result: output as ... this is a problem
        v.append(x.toString());
  ...
  /**
   * Helper for
   * {@link #charmap_encode_internal(String, String, PyObject, StringBuilder, boolean)}
   * called when we need some kind of substitute in the output for an invalid
   * input.
   *
   * @param str     to be encoded
   * @param errors  error policy name (e.g. "ignore")
  ...
  public static String encode_UTF16(PyObject str, String errors, ByteOrder order) {
    ...
    while (iter.hasNext()) {
      ...
      if (Character.isSurrogate((char) ch)) {
        ...
        PyObject replacementspec = codecs.encoding_error(errors, encoding, str.toString(), i, i + 1,
            "unpaired surrogate");
  ...
  /* --- UTF-32 Codec ------------------------------------------- */
  /**
   * Encode a Unicode Java String as UTF-32 in specified byte order with byte
   * order mark.
   *
   * @param unicode   to be encoded
   * @param errors    error policy name or null meaning "strict"
   * @param byteorder decoding "endianness" specified (in the Python -1, 0, +1
   *                  convention)
  ...
  /**
   * Encode a Unicode Java String as UTF-32 with little-endian byte order. No
   * byte-order mark is
   * generated.
   *
   * @param unicode to be encoded
  ...
  /**
   * Encode a Unicode Java String as UTF-32 with big-endian byte order. No
   * byte-order mark is
   * generated.
   *
   * @param unicode to be encoded
  ...
  /**
   * Encode a Unicode Java String as UTF-32 in specified byte order. A byte-order
   * mark is
   * generated if <code>order = ByteOrder.UNDEFINED</code>, and the byte order in
   * that case will
   * be the platform default, which is BE since the platform is Java.
   * <p>
   * The input String <b>must</b> be valid UTF-16, in particular, if it contains
   * surrogate code
   * units they must be ordered and paired correctly. The last char in
   * <code>unicode</code> is not
   * allowed to be an unpaired surrogate. These criteria will be met if the String
   * <code>unicode</code> is the contents of a valid {@link PyUnicode} or
   * {@link PyBytes}.
   *
   * @param unicode to be encoded
   * @param errors  error policy name or null meaning "strict"
  ...
  private static PyTuple PyUnicode_EncodeUTF32(PyObject unicode, String errors, ByteOrder order) {
    ...
    while (iter.hasNext()) {
      ...
      if (Character.isSurrogate((char) ch)) {
        ...
        PyObject replacementSpec = codecs.encoding_error(errors, encoding, unicode.toString(), i, i + 1,
            "unpaired surrogate");
        v.append(replacementSpec.__getitem__(0).toString());
        i++;
        continue;
  ...
  /**
   * Decode (perhaps partially) a sequence of bytes representing the UTF-32
   * encoded form of a
   * Unicode string and return as a tuple the unicode text, and the amount of
   * input consumed. The
   * endianness used will have been deduced from a byte-order mark, if present, or
   * will be
   * big-endian (Java platform default). The unicode text is presented as a Java
   * String (the
   ...
   * form of a Unicode string and return as a tuple the unicode text, and the
   * amount of input
   * consumed. A (correctly-oriented) byte-order mark will pass as a zero-width
   * non-breaking
   * space. The unicode text is presented as a Java String (the UTF-16
   ...
   * consumed. A (correctly-oriented) byte-order mark will pass as a zero-width
   * non-breaking
   * space. Unicode string and return as a tuple the unicode text, the amount of
   * input consumed.
   * The unicode text is presented as a Java String (the UTF-16 representation
   * used by
   * {@link PyUnicode}).
   *
   * @param bytes   to be decoded (Jython {@link PyBytes} convention)
   ...
   * Decode (perhaps partially) a sequence of bytes representing the UTF-32
   * encoded form of a
   * Unicode string and return as a tuple the unicode text, the amount of input
   * consumed, and the
   * decoding "endianness" used (in the Python -1, 0, +1 convention). The
   * endianness will be that
   * specified, will have been deduced from a byte-order mark, if present, or will
   * be big-endian
   * (Java platform default). Or it may still be undefined if fewer than 4 bytes
   * are presented.
   * (This codec entrypoint is used in the utf-32 codec only untile the byte order
   * is known.) The
   ...
   * Decode (perhaps partially) a sequence of bytes representing the UTF-32
   * encoded form of a
   * Unicode string and return as a tuple the (Jython internal representation of)
   * the unicode
   * text, the amount of input consumed, and if requested, the decoding
   * "endianness" used (in
   * Python -1, 0, +1 conventions). The state we preserve is our read position,
   * i.e. how many
   * bytes we have consumed and the byte order (endianness). If the input ends
   * part way through a
   * UTF-32 sequence (4 bytes) the data reported as consumed is just that up to
   * and not including
   * the first of these bytes. The Java String in the returned tuple is a UTF-16
   * representation of
   * the Unicode result, in line with Java conventions, where Unicode characters
   * above the BMP are
   * represented as surrogate pairs.
   *
   * @param bytes     input represented as String (Jython PyBytes convention)
   * @param errors    error policy name (e.g. "ignore", "replace")
   * @param order     LE, BE or UNDEFINED (meaning bytes may begin with a byte
   *                  order mark)
   * @param isFinal   if a "final" call, meaning the input must all be consumed
   * @param findOrder if the returned tuple should include a report of the byte
   *                  order
   * @return tuple (unicode_result, bytes_consumed [, endianness])
   */
  private static PyTuple PyUnicode_DecodeUTF32Stateful(String bytes, String errors,
    ...
    if (limit > 0) {
       ...
       * accordingly. If we know the byte order (it is LE or BE) then bytes
       * ressembling a byte
       * order mark are actually a ZERO WIDTH NON-BREAKING SPACE and will be passed
       * through to
      ...
      if (order == ByteOrder.UNDEFINED) {
         ...
         * will set the byte order and the BOM will not be copied to the output.
         * Otherwise
      ...
      /*
       * Main codec loop consumes 4 bytes and emits one code point with each pass,
       * until there
    ...
    /*
     * We have processed all we can: if we have some bytes left over that we can't
     * store for
  ...
  /**
   * Helper to
   * {@link #PyUnicode_DecodeUTF32Stateful(String, String, ByteOrder, boolean, boolean)}
   * when big-endian decoding is to be carried out.
   *
   * @param unicode character output
   * @param bytes   input represented as String (Jython PyBytes convention)
   * @param q       number of elements already consumed from <code>bytes</code>
   *                array
  ...
  /*
   * This codec is supposed to deal with an encoded form equal to the internal
   * representation of
   * the unicode object considered as bytes in memory. This was confusing in
   * CPython as it varied
   * with machine architecture (width and endian-ness). In Jython, where both are
   * fixed, the most
   * compatible choice is UTF-32BE. The codec is deprecated in v3.3 as irrelevant,
   * or impossible,
   * in view of the flexible string representation (which Jython emulates in its
   * own way).
   *
   ...
   *
   * Uses a trie structure instead of a dictionary; the speedup primarily comes
   * from not creating
   * integer objects in the process. The trie is created by inverting the encoding
   * map.
   */
  @Untraversable
  ...
  public static class EncodingMap extends PyObject {
    ...
    /**
     * Create and populate an EncodingMap from a 256 length PyUnicode char. Returns
     * a
