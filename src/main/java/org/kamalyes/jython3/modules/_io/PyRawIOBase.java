...
/**
 * An implementation of Python <code>_io._RawIOBase</code> mirroring the
 * arrangement of methods in
 * the CPython version.
 */
...
public class PyRawIOBase extends PyIOBase {
  ...
  /**
   * The read() method is implemented by calling readinto(); derived classes that
   * want to support
   * read() only need to implement readinto() as a primitive operation. In
   * general, readinto() can
   * be more efficient than read().
   *
   * @param n number of bytes to read (if possible)
   * @return a PyBytes holding the bytes read or <code>Py.None</code> (when a
   *         non-blocking source
  ...
  /*
   * CPython comment: (It would be tempting to also provide an implementation of
   * readinto() in
  ...
  /**
   * Implementation of the read() method once the argument has been reduced to an
   * int.
   * 
   * @param n number of bytes to read (if possible)
   * @return a PyBytes holding the bytes read or <code>Py.None</code> (when a
  ...
  private PyObject _read(int n) {
    ...
    if (n < 0) {
      ...
      if (m.isIndex()) {
        ...
        // We can forget view.release() as the bytearray b is garbage outside this
        // method.
  ...
  /**
   * Read until end of file, using multiple <code>read()</code> operations on the
   * underlying
   * stream. If the first <code>read()</code> returns <code>None</code> (only
  ...
  final synchronized PyObject _RawIOBase_readall() {
    ...
    if (!prev.__bool__()) {
      ...
      if (!curr.__bool__()) {
        ...
        // Accumulate the current read result and get another, until we run out of
        // bytes.
        do {
          list.add(curr);
  ...
  /**
   * Read up to <code>len(b)</code> bytes into <code>bytearray b</code> and return
   * the number of
   ...
   * @return number of bytes actually read or <code>Py.None</code> (when a
   *         non-blocking source is
  ...
  /**
   * Write the given bytes or bytearray object to the underlying raw stream and
   * return the number
   * of bytes written.
   *
