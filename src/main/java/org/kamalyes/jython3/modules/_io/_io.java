...
public class _io implements ClassDictInit {
  ...
  /**
   * This method is called when the module is loaded, to populate the namespace
   * (dictionary) of
   * the module. The dictionary has been initialised at this point reflectively
   * from the methods
   * of this class and this method nulls those entries that ought not to be
   * exposed.
   *
  ...
  public static void classDictInit(PyObject dict) {
    ...
    // UnsupportedOperation = makeException(dict, "UnsupportedOperation",
    // ValueError, IOError);
  ...
  /**
   * A function that returns a {@link PyException}, which is a Java exception
   * suitable for
   * throwing, and that will be raised as an <code>UnsupportedOperation</code>
   * Python exception.
   *
  ...
  /**
   * Convenience method for constructing a type object of a Python exception,
   * named as given, and
  ...
  /**
   * Open file and return a stream. Raise IOError upon failure. This is a port to
   * Java of the
   * CPython _io.open (Modules/_io/_iomodule.c) following the same logic, but
   * expressed with the
  ...
  public static PyObject open(PyObject[] args, String[] kwds) {
    ...
    /*
     * Create the Raw file stream. Let the constructor deal with the variants and
     * argument
     * checking.
     */
    ...
    /*
     * From the Python documentation for io.open() buffering = 0 to switch buffering
     * off (only
     * allowed in binary mode), 1 to select line buffering (only usable in text
     * mode), and an
     ...
     * follows:
     * Binary files are buffered in fixed-size chunks; "Interactive" text files
     * (files for which
     * isatty() returns True) use line buffering. Other text files use the policy
     * described
     * above for binary files.
     *
     * In Java, it seems a stream never is *known* to be interactive, but we ask
     * anyway, and
    ...
    if (buffering < 0) {
      /*
       * We are still being asked for the default buffer size. CPython establishes the
       * default
       * buffer size using fstat(fd), but Java appears to give no clue. A useful study
       * of
       * buffer sizes in NIO is http://www.evanjones.ca/software/java-bytebuffers.html
       * . This
    ...
    /*
     * We now know just what particular class of file we are opening, and therefore
     * what stack
    ...
    if (buffering == 0) {
      ...
      if (mode.binary) {
        ...
        PyObject[] textArgs = { buffer, ap.getPyObject(3, Py.None), ap.getPyObject(4, Py.None),
