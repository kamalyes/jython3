...
import org.kamalyes.jython3.core.PyJavaType;
...
public class PyFileIO extends PyRawIOBase {
  ...
  /**
   * The {@link FileIO} or {@link StreamIO} to which we delegate operations not
   * complete locally.
   */
  private RawIOBase ioDelegate;

  /*
   * Implementation note: CPython fileio does not use the base-class, possibly
   * overridden,
   * readable(), writable() and seekable(). Instead it sets local variables for
   * readable and
   * writable using the open mode, and returns these as readable() and writable(),
   * while using
   * them internally. The local variable seekable (and seekable()) is worked out
   * from a one-time
   * trial seek.
   */
   ...
   * <code>writable = writing | updating | appending</code>
   */
  ...
  /**
   * Construct an open <code>_io.FileIO</code> starting with an object that may be
   * a file name or
   * a file descriptor (actually a {@link RawIOBase}). Only the relevant flags
   * within the parsed
   * mode object are consulted (so that flags meaningful to this sub-class need
   * not be processed
   * out).
   *
  ...
  /**
   * Construct an open <code>_io.FileIO</code> for a sub-class constructor
   * starting with an object
   * that may be a file name or a file descriptor (actually a {@link RawIOBase}).
   * Only the
   * relevant flags within the parsed mode object are consulted (so that flags
   * meaningful to this
   ...
   * @param file    path or descriptor on which this should be constructed
   * @param mode    type of access specified
   * @param closefd if <code>false</code>, do not close <code>file</code> on call
   *                to
   *                <code>close()</code>
   */
   ...
   * <code>__new__</code>, into a {@link FileIO}, assigned to {@link #ioDelegate}.
   * It enforces
   * rules on {@link #closefd} and the type of object that may be a file
   * descriptor, and assigns
   * the <code>name</code> attribute to the string name or the file descriptor
   * (see Python docs
   * for io.FileIO.name). This places the logic of those several operations in one
   * place.
   * <p>
   * In many cases (such as construction from a file name, the FileIO is a
   * newly-opened file. When
   * the file object passed in is a file descriptor, the FileIO may be created to
   * wrap that
   * existing stream.
   *
  ...
  private void setDelegate(PyObject file, OpenMode mode) {
    ...
    if (file instanceof PyUnicode || file instanceof PyBytes) {
      ...
      /*
       * Build an _io.FileIO from an existing "file descriptor", which we may or may
       * not want
       * closed at the end. A CPython file descriptor is an int, but this is not the
       * natural
      ...
      if (fd instanceof RawIOBase) {
        /*
         * It is the "Jython file descriptor", of a type suitable to be the ioDelegate.
         * The
  ...
  /**
   * Create a {@link PyFileIO} and its <code>FileIO</code> delegate from the
   * arguments.
   */
  @ExposedNew
  ...
  final PyLong FileIO_readinto(PyObject buf) {
    ...
    if (buf instanceof PyArray) {
      ...
      try {
        // The ioDelegate, if readable, can always provide an InputStream (see
        // setDelegate)
  ...
  final PyLong FileIO_write(PyObject buf) {
    ...
    if (buf instanceof PyArray) {
      ...
      try {
        // The ioDelegate, if writable, can always provide an OutputStream (see
        // setDelegate)
      ...
      try {
        // Access the data as a java.nio.ByteBuffer [pos:limit] within possibly larger
        // array
        ByteBuffer byteBuffer = pybuf.getNIOByteBuffer();
        synchronized (ioDelegate) {
  ...
  /**
   * Close the underlying ioDelegate only if <code>closefd</code> was specified as
   * (or defaulted
   * to) <code>True</code>.
   */
  @Override
  ...
  // And it is a no-op for Jython io.FileIO, but why when there is
  // FileChannel.force()?
  @Override
  public void flush() {
    FileIO_flush();
  ...
  final void FileIO_flush() {
    if (writable()) {
      // Check for *downstream* close. (Locally, closed means "closed to client
      // actions".)
  ...
  /**
   * Convenience method providing the exception when an method requires the file
   * to be open, and
   * it isn't.
   *
   ...
   * Convenience method providing the exception when an method requires the file
   * to be open,
   * readable or writable, and it isn't. If the file is closed, return the message
   * for that,
