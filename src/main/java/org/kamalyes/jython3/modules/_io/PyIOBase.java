...
import org.kamalyes.jython3.core.PyByteArray;
...
import org.kamalyes.jython3.core.finalization.FinalizableBuiltin;
import org.kamalyes.jython3.core.finalization.FinalizeTrigger;
import org.kamalyes.jython3.core.io.FileIO;
import org.kamalyes.jython3.core.Traverseproc;
import org.kamalyes.jython3.core.Visitproc;
import org.kamalyes.jython3.core.io.RawIOBase;
...
import org.kamalyes.jython3.util.FilenoUtil;

/**
 * The Python module <code>_io._IOBase</code>, on which the <code>io</code>
 * module depends directly.
 * <p>
 * <b>Implementation note:</b> The code is based heavily on the Jython 2.6-ish
 * <code>_fileio.PyFileIO</code>, the purely Java-accessible
 * {@link org.kamalyes.jython3.core.io.IOBase} (both
 * Philip Jenvey's work), and the Python implementation in
 * <code>Lib/_pyio</code>. We do not simply
 * delegate to the implementation in {@link org.kamalyes.jython3.core.io}
 * because of the need to override
 * parts of the implementation in Python subclasses. A call to {@link #close()},
 * for example, is
 * required to call {@link #flush()}, but if delegated to the pure Java
 * implementation would not
 * call the version of <code>flush()</code> overridden in a Python sub-class of
 * <code>_io._IOBase</code>. Equally, the use made by
 * <code>PyRawIOBase.read(int)</code> of
 * <code>readinto(bytearray)</code> would not pick up the version of
 * <code>readinto</code> defined
 * in Python.
 */
...
public class PyIOBase extends PyObject implements FinalizableBuiltin, Traverseproc {
  ...
  /**
   * Provide a dictionary in the object, so that methods and attributes may be
   * overridden at
   * instance level.
   */
  @ExposedGet
  ...
  /**
   * Convenience method returning a PyException(_io.UnsupportedOperation), ripe
   * for throwing, and
   * naming the unsupported operation. Like many similar methods, this does not
   * actually throw
   * (raise) the exception: it should be thrown from the place where the problem
   * is.
   *
  ...
  /**
   * Position the read or write pointer at a given byte offset <code>pos</code>
   * relative to a
   * position indicated by <code>whence</code>.
   * <ul>
   * <li>If <code>whence</code>=0, the position will be set to <code>pos</code>
   * bytes.</li>
   * <li>If <code>whence</code>=1 the position will be set to the current position
   * plus
   * <code>pos</code>.</li>
  ...
  /**
   * Truncate file to <code>size</code> bytes to the current position (as reported
   * by
   * <code>tell()</code>).
   *
   * @return the new size
   ...
   * Irrespective of the
   * concrete type of the i/o object, locally-buffered write data is written
   * downstream. Whether
   * the downstream in object is also flushed depends upon the specific type of
   * this object.
   */
  public void flush() {
    _IOBase_flush();
  ...
  final void _IOBase_flush() {
    // Even types for which this remains a no-op must complain if closed (e.g.
    // BytesIO)
    _checkClosed();
  }

  /**
   * True if the object is closed to further <b>client</b> operations. It is the
   * state accessed by
   * <code>closed</code> and checked by {@link #_checkClosed()}. It may be set
   * without making it
   * an error (for this object and its subclasses) to access some downstream
   * object, notably in
   * the implementation of <code>close()</code>.
   */
  /*
   * This is the analogue of the IS_CLOSED(self) macro and iobase_closed_get() in
   * CPython
   * modules/_io/iobase.c. That function checks for the *existence* of an
   * attribute
   * __IOBase_closed. You can find this exposed in CPython. _pyio.IOBase exposes
   * _IOBase__closed
  ...
  final void _IOBase_close() {
    if (!__closed) {
      /*
       * The downstream file (file descriptor) will sometimes have been closed by
       * another
       * client. This should raise an error for us, (since data potentially in our
       * buffers
       * will be discarded) but we still must end up closed. the local close comes
       * after the
  ...
  /**
   * Raise an error if the pointer of underlying IO stream <b>is not</b> capable
   * of being
   * positioned.
   *
   * @param msg optional custom message
   * @throws PyException(ValueError) if the object is closed to client operations
   * @throws PyException(IOError)    if the stream <b>is not</b> capable of being
   *                                 positioned.
   */
  ...
  /**
   * Raise an error if the underlying IO stream <b>is</b> closed. (Note opposite
   * sense from
  ...
  /**
   * Return a file descriptor for the stream. A CPython file descriptor is an int,
   * but this is not
   * the natural choice in Jython, since Java has no such convention of using
   * integers. File
   * descriptors should be passed around opaquely, so their actual type is
   * irrelevant, as long as
   * (say) {@link _io#open(PyObject[], String[])} accepts the type that
   * {@link FileIO#fileno()}
   * returns.
   *
  ...
  /**
   * Is the stream known to be an interactive console? This relies on the ability
   * of the
   * underlying stream to know, which is not always possible.
   *
   * @return <code>true</code> if a console: <code>false</code> if not or we can't
   *         tell
   */
  ...
  private PyObject _readline(int limit) {
    ...
    /*
     * We have two almost independent versions of the implementation, depending on
     * whether we
     * have a peek() method available.
     */
    if (peekMethod != null) {
      /*
       * This type/object has some kind of read-ahead buffer that we can scan for end
       * of line,
       * and refill as required. We are prepared to make a list of the fragments and
       * join them
      ...
      while (remainingLimit > 0) {
        ...
        /*
         * peek() returns a str of bytes from the buffer (if any), doing at most one
         * read to
      ...
      while (--remainingLimit >= 0) {
        ...
        /*
         * read() returns a str of one byte, doing at most one read to refill, or it
         * returns
  ...
  /**
   * Return an iterator on which <code>next</code> may be repeatedly called to
   * produce (usually)
   ...
   * @throws PyException(StopIteration) when iteration has reached a natural
   *                                    conclusion
  ...
  final PyObject _IOBase_next() throws PyException {
    // Implement directly. Calling __next__() fails when PyIOBaseDerived is
    // considered.
    PyObject line = invoke("readline");
    if (!line.__bool__()) {
  ...
  /**
   * Construct a PyBuffer on a given object suitable for writing to the underlying
   * stream. The
   ...
   * @throws PyException (BufferError) if object has buffer API, but is not 1D
   *                     contiguous bytes
  ...
  /**
   * Construct a PyBuffer on a given object suitable for reading into from the
   * underlying stream.
   * The buffer returned will be navigable as a 1D contiguous writable sequence of
   * bytes.
   *
   * @param obj to be wrapped and presented as a buffer
   * @return a 1D contiguous PyBuffer of bytes
  ...
  /**
   * Convenience method providing the exception when an argument is not the
   * expected type. The
