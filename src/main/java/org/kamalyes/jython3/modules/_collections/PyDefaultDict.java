 ...
 * defaultFactory. The remaining functionality is the same as for the dict
 * class.
 *
 * collections.defaultdict([defaultFactory[, ...]]) - returns a new
 * dictionary-like
 * object. The first argument provides the initial value for the defaultFactory
 * attribute;
 * it defaults to None. All remaining arguments are treated the same as if they
 * were
...
public class PyDefaultDict extends PyDictionary implements Traverseproc {
  ...
  /**
   * This method does NOT call __setitem__ instead it relies on the fact that it
   * is
