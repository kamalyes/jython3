 ...
 * Deques are a
 * generalization of stacks and queues (the name is pronounced 'deck' and is
 * short for 'double-ended
 * queue'). Deques support thread-safe, memory efficient appends and pops from
 * either side of the
 ...
 * fixed-length
 * operations and incur O(n) memory movement costs for pop(0) and insert(0, v)
 * operations which
 * change both the size and position of the underlying data representation.
 * 
 * collections.deque([iterable[, maxlen]]) - returns a new deque object
 * initialized left-to-right
 * (using append()) with data from iterable. If iterable is not specified, the
 * new deque is empty.
 * If maxlen is not specified or is None, deques may grow to an arbitrary
 * length. Otherwise, the
 * deque is bounded to the specified maximum length. Once a bounded length deque
 * is full, when new
...
public class PyDeque extends PyObject implements Traverseproc {
  ...
  public synchronized final void deque___init__(PyObject[] args, String[] kwds) {
    ...
    if (iterable != null) {
      if (size != 0) {
        // initializing a deque with an iterator when this deque is not empty means that
        // we discard to empty first
        deque_clear();
      }
