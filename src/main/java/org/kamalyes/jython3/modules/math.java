...
public class math implements ClassDictInit {
  ...
  /**
   * Returns (x<sup>2</sup> +y<sup>2</sup>)<sup>&#189;</sup> without intermediate
   * overflow or
   * underflow. If either argument is infinite, the result is infinite, but
   * overflow during the
  ...
  public static double hypot(double x, double y) {
    ...
    if (Double.isInfinite(mag) && !(Double.isInfinite(x) || Double.isInfinite(y))) {
      // In these circumstances Math.hypot quietly returns inf, but CPython should
      // raise.
  ...
  /**
   * Returns a ValueError("math domain error"), ready to throw from the client
   * code.
   *
  ...
  /**
   * Returns a OverflowError("math range error"), ready to throw from the client
   * code.
   *
   ...
   * </pre>
   *
   * Note that the original function argument is also supplied to this method.
   * Most Java math
   * library methods do exactly what we need for Python, but some return
   * {@value Double#NaN} when
   * Python should raise <code>ValueError</code>. This is a brief way to change
   * that.
   *
  ...
  /**
   * Turn an infinite result into a thrown <code>OverflowError</code>, a math
   * range error, if the
   ...
   * Note that the original function argument is also supplied to this method.
   * Most Java math
   * library methods do exactly what we need for Python, but some return an
   * infinity when Python
   * should raise <code>OverflowError</code>. This is a brief way to change that.
   *
   * @param result to return (if we return)
   * @param arg    to include in check
   * @return result if <code>arg</code> was infinite or <code>result</code> was
   *         not infinite
   * @throws PyException (ValueError) if <code>result</code> was infinite and
   *                     <code>arg</code> was
   *                     not infinite
   */
