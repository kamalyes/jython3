...
public class ContextGuard implements ContextManager {
  ...
  // XXX - tentative support for generators in conjunction w/
  // contextlib.contextmanager

  /*
   * Sample usage:
   * 
   * from org.kamalyes.jython3.core.ContextGuard import makeManager as
   * contextmanager
   * 
   * @contextmanager
   * def my_manager():
      ...
      if (code instanceof PyBaseCode) {
        ...
        if (pyCode.co_flags.isFlagSet(CodeFlag.CO_GENERATOR)) {
          return new PyFunction(function.__globals__,
              ...
              function.__doc__,
              (function.__closure__ == null) ? null : ((PyTuple) function.__closure__).getArray());
        }
      }
    }
