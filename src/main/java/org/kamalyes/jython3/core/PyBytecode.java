...
public class PyBytecode extends PyBaseCode implements Traverseproc {
  ...
  // the following code exploits the fact that f_exits is only used by code
  // compiled to Java bytecode;
  // in their place we implement the block stack for PBC-VM, as mapped below in
  // the comments of pushBlock
  ...
  private static void pushBlock(PyFrame f, PyTryBlock block) {
    if (f.f_exits == null) { // allocate in the frame where they can fit! TODO consider supporting directly
                             // in the frame
  ...
  protected PyObject interpret(PyFrame f, ThreadState ts) {
    ...
    // 1. consider detaching the setting/getting of frame fields to improve
    // performance, instead do this
    // in a shadow version of the frame that we copy back to on entry/exit and
    // downcalls
    ...
    while (!debug || (maxCount == -1 || count < maxCount)) { // XXX - replace with while(true)
      ...
      if (f.tracefunc != null || debug) {
        ...
        line = lineCache.getline(next_instr); // XXX - should also return the range this is valid to avoid an
                                              // unnecessary bisect
        if (line != last_line) {
          f.setline(line);
        }
  ...
  // Utility functions to enable storage of unsigned bytes in co_code, co_lnotab
  // byte[] arrays
