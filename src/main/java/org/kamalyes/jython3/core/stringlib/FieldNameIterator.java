 ...
 * <code>_formatter_field_name_split()</code> returns a pair (tuple) consisting
 * of a head element
 * and an instance of this iterator. The constructor of this class effectively
 * implements that
 * function, since as well as "being" the iterator, the object has an extra
 * method {@link #head()}
...
public class FieldNameIterator extends PyObject implements Traverseproc {
  ...
  /**
   * Create an iterator for the parts of this field name (and extract the head
   * name field, which
   * may be an empty string). According to the Python Standard Library
   * documentation, a
   ...
   * </pre>
   *
   * The object is used from PyUnicode and from PyBytes, and we have to signal
   * which it is, so
   ...
   * <code>PyBytes</code> or <code>PyUnicode</code> according to the type of the
   * original field
   * name string. These objects are being used as field specifiers in navigating
   * arguments to a
   * format statement.
   *
  ...
  /**
   * Return the head object from the field name, as <code>PyInteger</code>,
   * <code>PyBytes</code>
  ...
  /**
   * If originally given a PyBytes, the iterator must return PyBytes not
   * PyUnicode.
   *
  ...
  /**
   * Return the next "chunk" of the field name (or return null if ended). A chunk
   * is a 2-tuple
   * describing:
   * <ol start=0>
