...
/**
 * A class that provides the implementation of <code>str</code> and
 * <code>unicode</code> formatting.
 * In a limited way, it acts like a StringBuilder to which text, formatted
 * according to the format
 * specifier supplied at construction. These are ephemeral objects that are not,
 * on their own,
 * thread safe.
 */
public class TextFormatter extends InternalFormat.Formatter {
  ...
  /**
   * Format the given <code>String</code> into the <code>result</code> buffer.
   * Largely, this is a
   * matter of copying the value of the argument, but a subtlety arises when the
   * string contains
   * supplementary (non-BMP) Unicode characters, which are represented as
   * surrogate pairs. The
   * precision specified in the format relates to a count of Unicode characters
   * (code points), not
   * Java <code>char</code>s. The method deals with this correctly, essentially by
   * not counting
   * the high-surrogates in the allowance. The final value of {@link #lenWhole}
   * counts the UTF-16
   * units added.
   *
  ...
  public TextFormatter format(CharSequence value) {
    ...
    if (Spec.specified(p) && p < n) {
      ...
      /*
       * A precision p was specified less than the length: we may have to truncate.
       * Note we
       * compared p with the UTF-16 length, even though it is the code point length
       * that
  ...
  // Variant to deal with supplementary characters: other formatters don't produce
  // them.
  @Override
