...
/**
 * A class that provides the implementation of integer formatting. In a limited
 * way, it acts like a
 * StringBuilder to which text and one or more numbers may be appended,
 * formatted according to the
 * format specifier supplied at construction. These are ephemeral objects that
 * are not, on their
 * own, thread safe.
 */
public class IntegerFormatter extends InternalFormat.Formatter {
  ...
  /**
   * Format a {@link BigInteger}, which is the implementation type of Jython
   * <code>long</code>,
   * according to the specification represented by this
   * <code>IntegerFormatter</code>. The
   * conversion type, and flags for grouping or base prefix are dealt with here.
   * At the point this
  ...
  /**
   * Format the value as decimal (into {@link #result}). The option for mandatory
   * sign is dealt
  ...
  /**
   * Format the value as hexadecimal (into {@link #result}), with the option of
   * using upper-case
   * or lower-case letters. The options for mandatory sign and for the presence of
   * a base-prefix
  ...
  /**
   * Format the value as octal (into {@link #result}). The options for mandatory
   * sign and for the
   * presence of a base-prefix "0o" are dealt with by reference to the format
   * specification.
   *
   * @param value to convert
   */
  ...
  /**
   * Format the value as binary (into {@link #result}). The options for mandatory
   * sign and for the
   * presence of a base-prefix "0b" are dealt with by reference to the format
   * specification.
   *
   * @param value to convert
   */
   ...
   * <code>IntegerFormatter</code>. The conversion type, and flags for grouping or
   * base prefix are
   * dealt with here. At the point this is used, we know the {@link #spec} is one
   * of the integer
   * types.
   *
   * @param value to convert
  ...
  /**
   * Append to {@link #result} buffer a sign (if one is specified for positive
   * numbers) and, in
   * alternate mode, the base marker provided. The sign and base marker are
   * together considered to
   * be the "sign" of the converted number, spanned by {@link #lenSign}. This is
   * relevant when we
   * come to insert padding.
   *
   * @param base marker "0x" or "0X" for hex, "0o" for octal, "0b" for binary, ""
   *             or
  ...
  /**
   * Append to {@link #result} buffer a minus sign and, in alternate mode, the
   * base marker
   * provided. The sign and base marker are together considered to be the "sign"
   * of the converted
   * number, spanned by {@link #lenSign}. This is relevant when we come to insert
   * padding.
   *
   * @param base marker ("0x" or "0X" for hex, "0" for octal, <code>null</code> or
   *             "" for decimal.
   */
  ...
  /**
   * Append a string (number) to {@link #result} and set {@link #lenWhole} to its
   * length .
   *
  ...
  /**
   * A more efficient algorithm for generating a hexadecimal representation of a
   * byte array.
   * {@link BigInteger#toString(int)} is too slow because it generalizes to any
   * radix and,
   ...
   * @return the hexadecimal representation of value, with "-" sign prepended if
   *         necessary
   */
  private static final String toHexString(BigInteger value) {
    ...
    // before returning the char array as string, remove leading zeroes, but not the
    // last one
  ...
  /**
   * A more efficient algorithm for generating an octal representation of a byte
   * array.
   * {@link BigInteger#toString(int)} is too slow because it generalizes to any
   * radix and,
   * consequently, is implemented using expensive mathematical operations.
   ...
   * @return the octal representation of value, with "-" sign prepended if
   *         necessary
   */
  ...
  /**
   * A more efficient algorithm for generating a binary representation of a byte
   * array.
   * {@link BigInteger#toString(int)} is too slow because it generalizes to any
   * radix and,
   * consequently, is implemented using expensive mathematical operations.
   ...
   * @return the binary representation of value, with "-" sign prepended if
   *         necessary
   */
   ...
   * <code>bin()</code>. The object's __index__ method is called, and is
   ...
   * <code>hex()</code>, or
   * <code>oct()</code>. The object's <code>__index__</code> method is called, and
   * is responsible
   * for raising the appropriate error (which the base
   * {@link PyObject#__index__()} does).
   *
   * @param number to convert
   * @return PyBytes converted result
   */
  ...
  /**
   * A minor variation on {@link IntegerFormatter} to handle "traditional"
   * %-formatting. The
   * difference is in support for <code>spec.precision</code>, the formatting
   * octal in "alternate"
  ...
  public static class Traditional extends IntegerFormatter {
    ...
    /**
     * Append a string (number) to {@link #result}, but insert leading zeros first
     * in order
     * that, on return, the whole-part length #lenWhole should be no less than the
     * precision.
     *
     * @param number to append
     */
     ...
     * that, on return, the whole-part length #lenWhole should be no less than the
     * precision.
     * Octal numbers must begin with a zero if <code>spec.alternate==true</code>, so
     * if the
