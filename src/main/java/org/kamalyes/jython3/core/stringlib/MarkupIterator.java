 ...
 * <code>str._formatter_parser()</code> returns, which
 * is an iterator returning successive 4-tuples, the sequence being equivalent
 * to the original
 * string.
 */
@Untraversable
...
public class MarkupIterator extends PyObject {
  ...
  private int index;
  /**
   * A counter used to auto-number fields when not explicitly numbered in the
   * format.
   */
  ...
  /**
   * Return the next "chunk" of the format (or return null if ended). A chunk is a
   * 4-tuple
   * describing
   * <ol start=0>
   ...
   * <li>any conversion that should be applied (the <code>'s'</code> or
   * <code>'r'</code> codes for
   * <code>str()</code> and <code>repr()</code>)</li>
   * </ol>
   * Elements 1-3 are None if this chunk contains no format specifier. Elements
   * 0-2 are
   * zero-length strings if missing from the format, while element 3 will be None
   * if missing.
   *
   ...
   * <code>PyBytes</code> or
   * <code>PyUnicode</code> according to the type of the original markup string,
   * or
   * <code>Py.None</code> if both arguments are <code>null</code>.
   *
   * @param value        to wrap as a PyObject or null if
   *                     <code>defaultValue</code> should be wrapped.
   * @param defaultValue to return or <code>null</code> if default return is
   *                     <code>None</code>.
  ...
  /**
   * Return the next {@link Chunk} from the iterator, which is a structure
   * containing parsed
   * elements of the replacement field (if any), and its preceding text. This is
   * the Java
  ...
  public Chunk nextChunk() {
    ...
    // Advance pos to the first '{' that is not a "{{" (escaped brace), or pos<0 if
    // none such.
    while (true) {
    ...
    if (pos < 0) {
      // ... except pos<0, and there is no further format specifier, only literal
      // text.
  ...
  /**
   * Parse a "replacement field" consisting of a name, conversion and format
   * specification.
   * According to the Python Standard Library documentation, a replacement field
   * has the
   * structure:
   *
   * <pre>
  ...
  private void parseField(Chunk result, CharSequence fieldMarkup) {
    ...
    // Finally, remember the argument number was specified (perhaps complain of
    // mixed use)
  ...
  /**
   * Class used locally to assign indexes to the automatically-numbered arguments
   * (see String
  ...
  static final class FieldNumbering {
    ...
    /**
     * Generate a numeric argument index automatically, or raise an error if already
     * started
     * numbering manually.
     *
    ...
    /**
     * Remember we are numbering manually, and raise an error if already started
     * numbering
     * automatically.
     */
