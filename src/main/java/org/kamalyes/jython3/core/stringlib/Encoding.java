package org.kamalyes.jython3.core.stringlib;
...
import org.kamalyes.jython3.core.codecs;
import org.kamalyes.jython3.modules.sys.SysModule;
...
public class Encoding {
  ...
  /**
   * Helper common to the Python and Java API for <code>str.replace</code>,
   * returning a new string
   * equal to this string with ocurrences of <code>oldPiece</code> replaced by
   * <code>newPiece</code>, up to a maximum of <code>count</code> occurrences, or
   * all of them.
   * This method also supports
   * {@link PyUnicode#str_replace(PyObject, PyObject, int)}, in
   ...
   *                 them.
   * @return PyBytes (or PyUnicode if this string is one), this string after
   *         replacements.
   */
  public static final String _replace(String s, String oldPiece, String newPiece, int count) {
    ...
    if (len == 0) {
      ...
      /*
       * old="" and new != "", interleave new piece with each char in original, taking
       * into
       * account count
       */
      int i = 0;
  ...
  public static PyObject format(CharSequence s, PyObject formatSpec, boolean bytes) {
    ...
    // Return a result that has the same type (str or unicode) as the formatSpec
    // argument.
    return f.pad().getPyResult();
  }

  /**
   * Common code for {@link PyBytes} and {@link PyUnicode} to prepare a
   * {@link TextFormatter}
   * from a parsed specification. The object returned has format method
   * {@link TextFormatter#format(String)} that treats its argument as UTF-16
   * encoded unicode (not
   * just <code>char</code>s). That method will format its argument (
   * <code>str</code> or
   * <code>unicode</code>) according to the PEP 3101 formatting specification
   * supplied here. This
   * would be used during <code>text.__format__(".5s")</code> or
   * <code>"{:.5s}".format(text)</code> where <code>text</code> is this Python
   * string.
   *
   * @param spec a parsed PEP-3101 format specification.
   * @return a formatter ready to use, or null if the type is not a string format
   *         type.
  ...
  public static final TextFormatter prepareFormatter(InternalFormat.Spec spec) throws PyException {
    ...
    switch (spec.type) {
      ...
      case 's':
        ...
        // spec may be incomplete. The defaults are those commonly used for string
        // formats.
  ...
  /**
   * Return a String equivalent to the argument according to the calling
   * conventions of the
   * certain methods of <code>str</code>. Those methods accept as a byte string
   * anything bearing
   * the buffer interface, or accept a <code>unicode</code> argument which they
   * interpret from its
   * UTF-16 encoded form (the internal representation returned by
  ...
  public static String asUTF16StringOrError(PyObject obj) {
    // PyUnicode accepted here. Care required in the client if obj is not basic
    // plane.
  ...
  /**
   * Return a String equivalent to the argument. This is a helper function to
   * those methods that
   * accept any byte array type (any object that supports a one-dimensional byte
   * buffer), or
   * accept a <code>unicode</code> argument which they interpret from its UTF-16
   * encoded form (the
   ...
   *
   * @param obj to coerce to a String
   * @return coerced value or <code>null</code> if it can't be (including
   *         <code>unicode</code>)
   */
   ...
   * <code>PyNone</code>, but <b>not</b> a <code>unicode</code>. (Or the argument
   * may be omitted,
   * showing up here as null.) These include the <code>strip</code> and
   * <code>split</code> methods
   * of <code>str</code>, where a null indicates that the criterion is whitespace,
   * and
  ...
  /**
   * Implementation of Python <code>str.rsplit()</code> common to exposed and Java
   * API returning a
   * {@link PyList} of <code>PyBytes</code>s. The <code>str</code> will be split
   * at each
   * occurrence of <code>sep</code>, working from the right. If
   * <code>sep == null</code>,
   * whitespace will be used as the criterion. If <code>sep</code> has zero
   * length, a Python
   * <code>ValueError</code> is raised. If <code>maxsplit</code> &gt;=0 and there
   * are more
   * feasible splits than <code>maxsplit</code> the first element of the list
   * contains the what is
  ...
  /**
   * Helper function for <code>.rsplit</code>, in <code>str</code> and
   * <code>unicode</code>,
   * splitting on white space and returning a list of the separated parts. If
   * there are more than
   * <code>maxsplit</code> feasible the first element of the list is the remainder
   * of the original
   * (this) string. The split sections will be {@link PyUnicode} if this object is
   * a
   * <code>PyUnicode</code>.
   *
  ...
  public static List<CharSequence> rsplitfields(CharSequence s, int maxsplit) {
    /*
     * Result built here (in reverse) is a list of split parts, exactly as required
     * for
     * s.rsplit(None, maxsplit). If there are to be n splits, there will be n+1
     * elements.
     */
    ...
    while (end >= 0) {
      ...
      if (end < 0) {
        ...
        // The next segment is the last and contains all characters back to the
        // beginning
        index = -1;
   ...
   * Helper function for <code>.rsplit</code>, in <code>str</code> and
   * <code>unicode</code>,
   * returning a list of the separated parts, <em>in the reverse order</em> of
   * their occurrence in
   * this string. If there are more than <code>maxsplit</code> occurrences of
   * <code>sep</code> the
   * first element of the list is the left end of the original (this) string. The
   * split sections
  ...
  public static final List<CharSequence> rsplitfields(CharSequence s, String sep, int maxsplit) {
     ...
     * Result built here (in reverse) is a list of split parts, exactly as required
     * for
     * s.rsplit(sep, maxsplit). If there are to be n splits, there will be n+1
     * elements.
     */
    List<CharSequence> list = new ArrayList<>();
  ...
  /**
   * Helper common to the Python and Java API returning the last index of the
   * substring or -1 for
   * not found. It accepts slice-like arguments, which may be <code>None</code> or
   * end-relative
  ...
  /**
   * Implementation of Python <code>str.rstrip()</code> common to exposed and Java
   * API, when
   * stripping whitespace. Any whitespace byte/character will be discarded from
   * the right end of
   * this <code>str</code>.
   * <p>
   * Implementation note: although a <code>str</code> contains only bytes, this
   * method is also
   * called by {@link PyUnicode#str_rstrip(PyObject)} when this is a basic-plane
   * string.
   *
   ...
   * Implementation of Python <code>str.rstrip()</code> common to exposed and Java
   * API. Any
   * byte/character matching one of those in <code>stripChars</code> will be
   * discarded from the
   * right end of this <code>str</code>. If <code>stripChars == null</code>,
   * whitespace will be
   * stripped.
   * <p>
   * Implementation note: although a <code>str</code> contains only bytes, this
   * method is also
   * called by {@link PyUnicode#str_strip(PyObject)} when both arguments are
   * basic-plane
   * strings.
   *
  ...
  /**
   * Helper for <code>strip</code>, <code>rstrip</code> implementation, when
   ...
   * @return index of rightmost character not in <code>stripChars</code> or -1 if
   *         they all are.
   */
  ...
  /**
   * Implementation of Python <code>str.strip()</code> common to exposed and Java
   * API, when
   * stripping whitespace. Any whitespace byte/character will be discarded from
   * either end of this
   * <code>str</code>.
   ...
   * Implementation note: although a <code>str</code> contains only bytes, this
   * method is also
   * called by {@link PyUnicode#str_strip(PyObject)} when this is a basic-plane
   * string.
   *
   * @return a new String, stripped of the whitespace characters/bytes
   */
   ...
   * end of this <code>str</code>. If <code>stripChars == null</code>, whitespace
   * will be
   * stripped.
   * <p>
   * Implementation note: although a <code>str</code> contains only bytes, this
  ...
  /**
   * Helper for <code>strip</code>, <code>lstrip</code> implementation, when
   * stripping whitespace.
   *
   * @param s     string to search (only <code>s[0:right]</code> is searched).
   * @param right rightmost extent of string search
   * @return index of lefttmost non-whitespace character or <code>right</code> if
   *         they all are.
   */
   ...
   * characters.
   *
   * @param s          string to search (only <code>s[0:right]</code> is
   *                   searched).
   * @param stripChars specifies set of characters to strip
   * @param right      rightmost extent of string search
   * @return index of leftmost character not in <code>stripChars</code> or
   *         <code>right</code> if
   *         they all are.
   */
  ...
  /**
   * Implementation of Python <code>str.lstrip()</code> common to exposed and Java
   * API, when
   * stripping whitespace. Any whitespace byte/character will be discarded from
   * the left end of
   * this <code>str</code>.
   ...
   * byte/character matching one of those in <code>stripChars</code> will be
   * discarded from the
   * left end of this <code>str</code>. If <code>stripChars == null</code>,
   * whitespace will be
   * stripped.
   * <p>
   * Implementation note: although a <code>str</code> contains only bytes, this
   * method is also
   * called by {@link PyUnicode#str_lstrip(PyObject)} when both arguments are
   * basic-plane
   * strings.
   *
   * @param stripChars characters to strip or null
  ...
  /**
   * Implementation of Python str.split() common to exposed and Java API returning
   * a
   * {@link PyList} of <code>PyBytes</code>s. The <code>str</code> will be split
   * at each
   * occurrence of <code>sep</code>. If <code>sep == null</code>, whitespace will
   * be used as the
   * criterion. If <code>sep</code> has zero length, a Python
   * <code>ValueError</code> is raised.
   * If <code>maxsplit</code> &gt;=0 and there are more feasible splits than
   * <code>maxsplit</code>
   * the last element of the list contains the what is left over after the last
   * split.
   * <p>
   * Implementation note: although a str contains only bytes, this method is also
   * called by
  ...
  /**
   * Helper function for <code>.split</code>, in <code>str</code> and
   * <code>unicode</code>,
   * splitting on white space and returning a list of the separated parts. If
   * there are more than
   * <code>maxsplit</code> feasible the last element of the list is the remainder
   * of the original
   * (this) string. The split sections will be {@link PyUnicode} if this object is
   * a
   * <code>PyUnicode</code>.
  ...
  public static final List<CharSequence> splitfields(CharSequence s, int maxsplit) {
    /*
     * Result built here is a list of split parts, exactly as required for
     * s.split(None,
  ...
  /**
   * Helper function for <code>.split</code> and <code>.replace</code>, in
   * <code>str</code> and
   * <code>unicode</code>, returning a list of the separated parts. If there are
   * more than
   * <code>maxsplit</code> occurrences of <code>sep</code> the last element of the
   * list is the
   * remainder of the original (this) string. If <code>sep</code> is the
   * zero-length string, the
   * split is between each character (as needed by <code>.replace</code>). The
   * split sections will
  ...
  public static final List<CharSequence> splitfields(CharSequence s, String sep, int maxsplit) {
     ...
     * Result built here is a list of split parts, exactly as required for
     * s.split(sep), or to
     * produce the result of s.replace(sep, r) by a subsequent call r.join(L). If
     * there are to
    ...
    if (maxsplit == 0) {
      ...
      /*
       * The separator is "". This cannot happen with s.split(""), as that's an error,
       * but it
       * is used by s.replace("", A) and means that the result should be A interleaved
       * between
       * the characters of s, before the first, and after the last, the number always
       * limited
       * by maxsplit.
       */
  ...
  /**
   * Helper common to the Python and Java API returning the index of the substring
   * or -1 for not
   * found. It accepts slice-like arguments, which may be <code>None</code> or
   * end-relative
   * (negative). This method also supports
  ...
  /**
   * Helper common to the Python and Java API returning the number of occurrences
   * of a substring.
   * It accepts slice-like arguments, which may be <code>None</code> or
   * end-relative (negative).
   * This method also supports
   * {@link PyUnicode#str_count(PyObject, PyObject, PyObject)}.
   *
   * @param sub      substring to find.
   * @param startObj start of slice.
   * @param endObj   end of slice.
  ...
  public static final boolean startswith(CharSequence s, PyObject prefix, PyObject startObj, PyObject endObj, int len) {
    ...
    if (!(prefix instanceof PyTuple)) {
      ...
      // If s is non-BMP, and this is a PyBytes (bytes), result will correctly be
      // false.
  ...
  /**
   * Return the (lazily) compiled regular expression that matches all valid a
   * Python float()
   * arguments, in which Group 1 captures the number, stripped of white space.
   * Various invalid
  ...
  /**
   * Regular expression for an unsigned Python float, accepting also any sequence
   * of the letters
   * that belong to "NaN" or "Infinity" in whatever case. This is used within the
   * regular
   * expression patterns that define a priori acceptable strings in the float and
   * complex
   ...
   * This is used
   * within the regular expression patterns that define a priori acceptable
   * strings in the complex
   * constructors. The expression contributes five named capture groups a, b, x, y
   * and j. x and y
   * are the two floats encountered, and if j is present, one of them is the
   * imaginary part.
   * a and b are the optional parentheses. They must either both be present or
   * both omitted.
   */
  ...
  /**
   * Conversion for non-numeric floats, accepting signed or unsigned "inf" and
   * "nan", in any case.
   *
   * @param s to convert
  ...
  /**
   * Helper for interpreting each part (real and imaginary) of a complex number
   * expressed as a
   * string in {@link #atocx(String)}. It deals with numbers, inf, nan and their
   * variants, and
