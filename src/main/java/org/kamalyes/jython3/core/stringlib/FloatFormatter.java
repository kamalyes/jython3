...
import org.kamalyes.jython3.core.stringlib.InternalFormat.Spec;

/**
 * A class that provides the implementation of floating-point formatting. In a
 * limited way, it acts
 * like a StringBuilder to which text and one or more numbers may be appended,
 * formatted according
 * to the format specifier supplied at construction. These are ephemeral objects
 * that are not, on
...
public class FloatFormatter extends InternalFormat.Formatter {
  ...
  // No-one needs more than log(Double.MAX_VALUE) - log2(Double.MIN_VALUE) = 1383
  // digits.
  ...
  /**
   * The length of the exponent marker ("e"), "inf" or "nan", or zero if there
   * isn't one.
   */
  ...
  /**
   * Construct the formatter from a client-supplied buffer, to which the result
   * will be appended,
  ...
  public FloatFormatter(StringBuilder result, Spec spec) {
    ...
    if (spec.alternate) {
      ...
      /*
       * Every other format (if it does not ignore the setting) will by default trim
       * off all
  ...
  /**
   * Construct the formatter from a specification, allocating a buffer internally
   * for the result.
   *
   * @param spec parsed conversion specification
   */
  ...
  /**
   * Recommend a buffer size for a given specification, assuming one float is
   * converted. This will
   * be a "right" answer for e and g-format, and for f-format with values up to
   * 9,999,999.
   *
   * @param spec parsed conversion specification
   */
  ...
  /**
   * Override the default truncation behaviour for the specification originally
   * supplied. Some
   * formats remove trailing zero digits, trimming to zero or one. Set member
   * <code>minFracDigits</code>, to modify this behaviour.
   *
   * @param minFracDigits if &lt;0 prevent truncation; if >=0 the minimum number
   *                      of fractional
   *                      digits; when this is zero, and all fractional digits are
   *                      zero, the decimal point
  ...
  /**
   * Format a floating-point number according to the specification represented by
   * this
   ...
   * Format a floating-point number according to the specification represented by
   * this
   * <code>FloatFormatter</code>. The conversion type, precision, and flags for
   * grouping or
   * percentage are dealt with here. At the point this is used, we know the
   * {@link #spec} is one
   * of the floating-point types. This entry point allows explicit control of the
   * prefix of
   * positive numbers, overriding defaults for the format type.
   *
   * @param value          to convert
   * @param positivePrefix to use before positive values (e.g. "+") or null to
   *                       default to ""
   * @return this object
   */
  ...
  public FloatFormatter format(double value, String positivePrefix) {
    ...
    // Puts all instance variables back to their starting defaults, and start =
    // result.length().
    setStart();
    ...
    /*
     * By default, the prefix of a positive number is "", but the format specifier
     * may override
    ...
    switch (Character.toLowerCase(spec.type)) {
      ...
      case 'n':
        // Locale-sensitive version of g-format should be here. (Désolé de vous
        // decevoir.)
  ...
  /**
   * Common code to deal with the sign, and the special cases "0", "-0", "nan,
   * "inf", or "-inf".
   * If the method returns <code>false</code>, we have started a non-zero number
   * and the sign is
   * already in {@link #result}. The client need then only encode
   * <i>abs(value)</i>. If the method
   * returns <code>true</code>, and {@link #lenMarker}==0, the value was "0" or
   * "-0": the caller
   * may have to zero-extend this, and/or add an exponent, to match the requested
   * format. If the
   * method returns <code>true</code>, and {@link #lenMarker}>0, the method has
   * placed "nan, "inf"
  ...
  /**
   * The e-format helper function of {@link #format(double, String)} that uses
   * Java's
   * {@link BigDecimal} to provide conversion and rounding. The converted number
   * is appended to
   * the {@link #result} buffer, and {@link #start} will be set to the index of
   * its first
   * character.
   *
   * @param value          to convert
  ...
  private void format_e(double value, String positivePrefix, int precision) {
    ...
    // If the result is not already complete, add point and zeros as necessary, and
    // exponent.
    if (lenMarker == 0) {
  ...
  /**
   * The f-format inner helper function of {@link #format(double, String)} that
   * uses Java's
   * {@link BigDecimal} to provide conversion and rounding. The converted number
   * is appended to
   * the {@link #result} buffer, and {@link #start} will be set to the index of
  ...
  private void format_f(double value, String positivePrefix, int precision) {
    ...
    if (!signAndSpecialNumber(value, positivePrefix)) {
      ...
      // When converted to text, the number of fractional digits is exactly the scale
      // we set.
  ...
  /**
   * Append a decimal point and trailing fractional zeros if necessary for 'e' and
   * 'f' format.
   * This should not be called if the result is not numeric ("inf" for example).
   * This method deals
   * with the following complexities: on return there will be at least the number
   * of fractional
   * digits specified in the argument <code>n</code>, and at least
   * {@link #minFracDigits};
   * further, if <code>minFracDigits&lt;0</code>, signifying the "alternate mode"
   * of certain
   * formats, the method will ensure there is a decimal point, even if there are
   * no fractional
   * digits to follow.
   *
  ...
  /**
   * Implementation of the variants of g-format, that uses Java's
   * {@link BigDecimal} to provide
   * conversion and rounding. These variants are g-format proper, alternate
   * g-format (available
   * for "%#g" formatting), n-format (as g but subsequently "internationalised"),
   * and none-format
   * (type code Spec.NONE).
   * <p>
   * None-format is the basis of <code>float.__str__</code>.
   * <p>
   * According to the Python documentation for g-format, the precise rules are as
   * follows: suppose
   * that the result formatted with presentation type <code>'e'</code> and
   * precision <i>p-1</i>
   * would have exponent exp. Then if <i>-4 &lt;= exp < p</i>, the number is
   * formatted with
   * presentation type <code>'f'</code> and precision <i>p-1-exp</i>. Otherwise,
   * the number is
   * formatted with presentation type <code>'e'</code> and precision <i>p-1</i>.
   * In both cases
   * insignificant trailing zeros are removed from the significand, and the
   * decimal point is also
   * removed if there are no remaining digits following it.
   * <p>
   * The Python documentation says none-format is the same as g-format, but the
   * observed behaviour
   * differs from this, in that f-format is only used if <i>-4 &lt;= exp < p-1</i>
   * (i.e. one
   * less), and at least one digit to the right of the decimal point is preserved
   * in the f-format
   * (but not the e-format). That behaviour is controlled through the following
   * arguments, with
   ...
   * @param positivePrefix  to use before positive values (e.g. "+") or null to
   *                        default to ""
   * @param precision       total number of significant digits (precision 0
   *                        behaves as 1)
   * @param expThresholdAdj <code>+precision =</code> the exponent at which to
   *                        resume using
   ...
   * {@link Double#toString(double)} to provide conversion and rounding. That
   * method gives us
   * almost what we need, but not quite (sometimes it yields 18 digits): here we
   * always round to
   * 17 significant digits. Much of the formatting after conversion is shared with
   * {@link #format_g(double, String, int, int, int)}. <code>minFracDigits</code>
   * is consulted
  ...
  /**
   * Common code for g-format, none-format and r-format called when the conversion
   * yields "inf",
   * "nan" or zero. The method completes formatting of the zero, with the
  ...
  /**
   * Common code for g-format, none-format and r-format used when the exponent is
   * such that a
   * fixed-point presentation is chosen. Normally the method removes trailing
   * digits so as to
   * shorten the presentation without loss of significance. This method respects
   * the minimum
   * number of fractional digits (digits after the decimal point), in member
   * <code>minFracDigits</code>, which is 0 for g-format and 1 for none-format and
   * r-format. When
   * <code>minFracDigits&lt;0</code> this signifies "no truncation" mode, in which
   * trailing zeros
   ...
   * @param exp       would be the exponent (in e-format), used to position the
   *                  decimal point.
   * @param precision of conversion (number of significant digits).
   */
   ...
   * Common code for g-format, none-format and r-format used when the exponent is
   * such that an
   * exponential presentation is chosen. Normally the method removes trailing
   * digits so as to
   * shorten the presentation without loss of significance. Although no minimum
   * number of
  ...
  /**
   * Convert a double to digits and an exponent for use in
   * <code>float.__repr__</code> (or
   * r-format). This method takes advantage of (or assumes) a close correspondence
   * between
   * {@link Double#toString(double)} and Python <code>float.__repr__</code>. The
   * correspondence
   * appears to be exact, insofar as the Java method produces the minimal non-zero
   * digit string.
   * It mostly chooses the same number of digits (and the same digits) as the
   * CPython repr, but in
   * a few cases <code>Double.toString</code> produces more digits. This method
   * truncates to the
  ...
  private static int reprDigits(double value, int maxDigits, StringBuilder buf) {
    ...
    /*
     * XXX This still does not round in all the cases it could. I think Java stops
     * generating
     * digits when the residual is <= ulp/2. This is to neglect the possibility that
     * the extra
     * ulp/2 (before it becomes a different double) could take us to a rounder
     * numeral. To fix
     * this, we could express ulp/2 as digits in the same scale as those in the
     * buffer, and
     * consider adding them. But Java's behaviour here is probably a manifestation
     * of bug
     * JDK-4511638.
     */
  ...
  /**
   * Append the trailing fractional zeros, as required by certain formats, so that
   * the total
   * number of fractional digits is no less than specified. If
   * <code>n&lt;=0</code>, the method
   ...
   * Append the trailing fractional zeros, as required by certain formats, so that
   * the total
   * number of fractional digits is no less than specified. If there is no decimal
   * point
   * originally (and therefore no fractional part), the method will add a decimal
   * point, even if
   * it adds no zeros.
   *
   * @param n smallest number of fractional digits on return
  ...
  /**
   * Remove trailing zeros from the fractional part, as required by certain
   * formats, leaving at
   * least the number of fractional digits specified. If the resultant number of
   * fractional digits
   * is zero, this method will also remove the trailing decimal point (if there is
   * one).
   *
   * @param n smallest number of fractional digits on return
   */
  ...
  /**
   * Append the current value of {@code exp} in the format <code>"e{:+02d}"</code>
   * (for example
