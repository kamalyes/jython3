...
public class InternalFormat {
  ...
  /**
   * Create a {@link Spec} object by parsing a format specification, supplied as
   * an object.
   *
   * @param text to parse
   * @return parsed equivalent to text
  ...
  /**
   * A class that provides the base for implementations of type-specific
   * formatting. In a limited
   * way, it acts like a StringBuilder to which text and one or more numbers may
   * be appended,
   * formatted according to the format specifier supplied at construction. These
   * are ephemeral
  ...
  public static class Formatter implements Appendable {
    ...
    /**
     * The specification according to which we format any number supplied to the
     * method.
     */
    ...
    /**
     * Signals the client's intention to make a PyBytes (or other byte-like)
     * interpretation of
    ...
    protected int mark;
    /**
     * The latest number we are working on floats at the end of the result, and
     * starts here.
     */
    protected int start;
     ...
     * Sets
     * {@link #mark} and {@link #start} to the end of the buffer. The new formatted
     * object will
     * therefore be appended there and, when the time comes, padding will be applied
     * to (just)
     * the new text.
     *
     ...
     * Signals the client's intention to make a PyBytes (or other byte-like)
     * interpretation of
     * {@link #result}, rather than a PyUnicode one. Only formatters that could
     * produce
     ...
     * <code>PyObject</code>, either {@link PyBytes} or {@link PyUnicode} according
     * to
     * {@link #bytes}.
     *
     * @return formatted result
    ...
    /**
     * Clear the instance variables describing the latest object in {@link #result},
     * ready to
     * receive a new one: sets {@link #start} and calls {@link #reset()}. This is
     * necessary when
     * a <code>Formatter</code> is to be re-used. Note that this leaves
     * {@link #mark} where it
     * is. In the core, we need this to support <code>complex</code>: two floats in
     * the same
     ...
     * Clear the instance variables describing the latest object in {@link #result},
     * ready to
     * receive a new one. This is called from {@link #setStart()}. Subclasses
     * override this
    ...
    /**
     * Supports {@link #toString()} by returning the lengths of the successive
     * sections in the
     * result buffer, used for navigation relative to {@link #start}. The
     * <code>toString</code>
     * method shows a '|' character between each section when it prints out the
     * buffer. Override
     ...
     * {@inheritDoc}
     * <p>
     * Overridden to provide a debugging view in which the actual text is shown
     * divided up by
     * the <code>len*</code> member variables. If the dividers don't look right,
     * those variables
    ...
    /**
     * Insert grouping characters (conventionally commas) into the whole part of the
     * number.
    ...
    protected void groupDigits(int groupSize, char comma) {
      ...
      // Work out how many commas (or whatever) it takes to group the whole-number
      // part.
      ...
      if (commasNeeded > 0) {
        ...
        /*
         * Now working from high to low, copy all the digits that have to move. Each
         * pass
         * copies one group and inserts a comma, which makes the to-pointer move one
         * place
    ...
    /**
     * Make a space in {@link #result} of a certain size and position. On return,
     * the segment
     * lengths are likely to be invalid until the caller adjusts them corresponding
     * to the
    ...
    /**
     * Pad the result so far (defined as the contents of {@link #result} from
     * {@link #mark} to
     * the end) using the alignment, target width and fill character defined in
     * {@link #spec}.
     * The action of padding will increase the length of this segment to the target
     * width, if
     * that is greater than the current length.
     * <p>
     * When the padding method has decided that that it needs to add n padding
     * characters, it
     ...
     * </tr>
     * </table>
     * Note that in the "pad after sign" mode, only the last number into the buffer
     * receives the
     * padding. This padding gets incorporated into the whole part of the number.
     * (In other
     * modes, the padding is around <code>result[mark:]</code>.) When this would not
     * be
     ...
     * <code>leftIndex</code> to the end) using the alignment, by <code>n</code>
     * repetitions of
     * the fill character defined in {@link #spec}, and distributed according to
     * <code>spec.align</code>. The value of <code>leftIndex</code> is only used if
     * the
     * alignment is '&gt;' (left) or '^' (both). The value of the critical lengths
     * (lenWhole,
     * lenSign, etc.) are not affected, because we assume that
     * <code>leftIndex &lt;= </code>
     * {@link #start}.
     *
    ...
    /**
     * Fix-up the zero-padding of the last formatted number in {@link #result()} in
     * the special
     * case where a sign-aware padding (<code>{@link #spec}.align='='</code>) was
     * requested, the
     * fill character is <code>'0'</code>, and the digits are to be grouped. In
     * these exact
     * circumstances, the grouping, which must already have been applied to the
     * (whole part)
     ...
     * </pre>
     *
     * The padding has increased the overall length of the result to the target
     * width. About one
     * in three calls to this method adds one to the width, because the whole part
     * cannot start
     * with a comma.
     *
     * <pre>
    ...
    protected void zeroPadAfterSignWithGroupingFixup(int groupSize, char comma) {
       ...
       *
       * All we need do is over-write some of the zeros with the separator comma, in
       * the
      ...
      // Step back down the result array visiting the commas. (Easiest to do all of
      // them.)
      ...
      if (p + step == firstZero) {
        /*
         * Suppose the format call was format(-12e8, "0=30,.4f"). At the beginning, we
         * had
         * something like this in result: . [-|000000000001,200,000,000|.|0000||]
         *
         * And now, result looks like this: [-|,000,000,001,200,000,000|.|0000||] in
         * which
         * the first comma is wrong, but so would be a zero. We have to insert another
         * zero,
    ...
    /**
     * Convenience method returning a {@link Py#ValueError} reporting that alternate
     * form is not
    ...
    /**
     * Convenience method returning a {@link Py#ValueError} reporting that the given
     * alignment
    ...
    /**
     * Convenience method returning a {@link Py#ValueError} reporting that
     * specifying a sign is
    ...
    /**
     * Convenience method returning a {@link Py#ValueError} reporting that zero
     * padding is not
     * allowed in a format specifier for the named type.
     *
     * @param forType the type it was found applied to
    ...
    /**
     * Convenience method returning a {@link Py#ValueError} reporting that some
     * format specifier
     ...
     * Convenience method returning a {@link Py#ValueError} reporting that some
     * format specifier
     * feature is not allowed for the named format code and data type. Produces a
     * message like:
     * <p>
     ...
     * @param outrage committed in the present case
     * @param forType the data type (e.g. "integer") it where it is an outrage
     * @param code    the formatting code for which it is an outrage (or '\0' not to
     *                mention one)
     * @return exception to throw
     */
    public static PyException notAllowed(String outrage, String forType, char code) {
      ...
      String msg = outrage + " not allowed " + withOrIn + forType + " format specifier"
          + codeAsString;
      return Py.ValueError(msg);
    }
  ...
  /**
   * Parsed PEP-3101 format specification of a single field, encapsulating the
   * format for use by
   * formatting methods. This class holds the several attributes that might be
   * decoded from a
   * format specifier. Each attribute has a reserved value used to indicate
   * "unspecified".
   * <code>Spec</code> objects may be merged such that one <code>Spec</code>
   * provides values,
   * during the construction of a new <code>Spec</code>, for attributes that are
   * unspecified in a
   * primary source.
   * <p>
   * This structure is returned by factory method {@link #fromText(String)}, and
   * having public
   * final members is freely accessed by formatters such as
   * {@link FloatFormatter}, and the
   * __format__ methods of client object types.
   * <p>
   * The fields correspond to the elements of a format specification. The grammar
   * of a format
   * specification is:
   *
   * <pre>
  ...
  public static class Spec {
    ...
    /**
     * Alignment indicator is one of {<code>'&lt;', '^', '>', '='</code>, or U+FFFF
     * if
     * unspecified.
     */
    public final char align;
    /**
     * Sign-handling flag, one of <code>'+'</code>, <code>'-'</code>, or
     * <code>' '</code>, or
    ...
    /**
     * Insert the grouping separator (which in Python always indicates a group-size
     * of 3).
     */
    ...
    /**
     * Non-character code point used to represent "no value" in <code>char</code>
     * attributes.
     */
     ...
     * @return true only if the attribute is >=0 (meaning that it has been
     *         specified).
     */
     ...
     *                  {<code>'&lt;', '^', '>', '='</code>
     * @param sign      policy, one of <code>'+'</code>, <code>'-'</code>, or
     *                  <code>' '</code>.
     * @param alternate true to request alternate formatting mode (<code>'#'</code>
     *                  flag).
    ...
    /**
     * Return a merged <code>Spec</code> object, in which any attribute of this
     * object that is
     * specified (or <code>true</code>), has the same value in the result, and any
     * attribute of
     * this object that is unspecified (or <code>false</code>), has the value that
     * attribute
     * takes in the other object. Thus the second object supplies default values.
     * (These
     * defaults may also be unspecified.) The use of this method is to allow a
     * <code>Spec</code>
     * constructed from text to record exactly, and only, what was in the textual
     * specification,
     * while the __format__ method of a client object supplies its type-specific
     * defaults. Thus
     * "20" means "<20s" to a <code>str</code>, ">20.12" to a <code>float</code> and
     * ">20.12g"
     * to a <code>complex</code>.
     *
     * @param other defaults to merge where this object does not specify the
     *              attribute.
   ...
   * {@link #parse()}
   * method that translates the format specification into an <code>Spec</code>
   * object.
   */
  private static class Parser {
     ...
     * {@link Spec},
     * which is an object encapsulating the format for use by formatting methods.
     * This parser
     * deals only with the format specifiers themselves, as accepted by the
     * <code>__format__</code> method of a type, or the <code>format()</code>
     * built-in, not
    ...
    /**
     * Test that the next character is exactly the one specified, and advance past
     * it if it is.
     */
