...
public class PySuper extends PyObject implements Traverseproc {
   ...
   * 
   * - If it is a class, it must be a subclass of 'type'. This case is used for
   * class
   * methods; the return value is obj.
   * 
   * - If it is an instance, it must be an instance of 'type'. This is the normal
   * case;
   * the return value is obj.__class__.
   *
   * But... when obj is an instance, we want to allow for the case where objType
   * is not
   * a subclass of type, but obj.__class__ is! This will allow using super() with
   * a
   * proxy for obj.
   *
