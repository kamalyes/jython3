...
import org.kamalyes.jython3.modules.math;
...
public class PyFloat extends PyObject {
    ...
    if (x == null) {
      ...
      try {
        ...
        if (e.match(Py.AttributeError)) {
          ...
          // TODO: We are using the same message as CPython, even if
  ...
  public static PyObject float_fromhex(PyType type, PyObject o) {
    // TODO::: I'm sure this could be shortened/simplified, but Double.parseDouble()
    // takes
  ...
  /**
   * Format this float according to the specification passed in. Supports
   * <code>__str__</code> and
   * <code>__repr__</code>.
   *
  ...
  /**
   * Coercion logic for float. Implemented as a final method to avoid invocation
   * of virtual
  ...
  /**
   * Python % operator: y = n*x + z. The modulo operator always yields a result
   * with the same sign
   * as its second operand (or zero). (Compare
   * <code>java.Math.IEEEremainder</code>)
   *
   * @param x dividend
   * @param y divisor
  ...
  private static PyFloat _pow(double v, double w) {
    /*
     * This code was translated from the CPython implementation at v2.7.8 by
     * progressively
     * removing cases that could be delegated to Java. Jython differs from CPython
     * in that where
     * C pow() overflows, Java pow() returns inf (observed on Windows). This is not
     * subject to
     * regression tests, so we take it as an allowable platform dependency. All
     * other
     * differences in Java Math.pow() are trapped below and Python behaviour is
     * enforced.
     */
    if (w == 0) {
      // v**0 is 1, even 0**0
      ...
      /*
       * In Java Math pow(1,inf) = pow(-1,inf) = pow(1,-inf) = pow(-1,-inf) = nan, but
       * in
   ...
   * {@link FloatFormatter} from a parsed specification. The object returned has
   * format method
  ...
  public PyTuple as_integer_ratio() {
    ...
    /*
     * self == float_part * 2**exponent exactly and float_part is integral. If
     * FLT_RADIX != 2,
     * the 300 steps may leave a tiny fractional part to be truncated by
     * PyLong_FromDouble().
     */
