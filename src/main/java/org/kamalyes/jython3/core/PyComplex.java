...
import org.kamalyes.jython3.core.stringlib.InternalFormat.Formatter;
...
public class PyComplex extends PyObject {
  ...
  /**
   * Format specification used by str() and none-format. (As CPython, but is that
   * right?)
   */
  ...
  /**
   * Format this complex according to the specification passed in. Supports
   * <code>__str__</code>
   * and <code>__repr__</code>, and none-format.
   * <p>
   * In general, the output is surrounded in parentheses, like
   * <code>"(12.34+24.68j)"</code>.
   * However, if the real part is zero (but not negative zero), only the imaginary
   * part is
   * printed, and without parentheses like <code>"24.68j"</code>. The number
   * format specification
   * passed in is used for the real and imaginary parts individually, with padding
   * to width
  ...
  /**
   * Helper for {@link #complex___eq__(PyObject)} and
   * {@link #complex___ne__(PyObject)}.
   *
   * @param other to compare for equality with this
   * @return 0 = false, 1 = true, 2 = don't know (ask the <code>other</code>
   *         object)
   */
  private int eq_helper(PyObject other) {
    // We only deal with primitive types here. All others delegate upwards (return
    // 2).
    boolean equal;
  ...
  /**
   * Coercion logic for complex. Implemented as a final method to avoid invocation
   * of virtual
  ...
  // Special case __tojava__ for bug 1605, since we broke it with our support for
  // faux floats.
  ...
  final PyObject complex___abs__() {
    ...
    if (Double.isInfinite(mag) && !(Double.isInfinite(real) || Double.isInfinite(imag))) {
      // In these circumstances Math.hypot quietly returns inf, but Python should
      // raise.
  ...
  final PyObject complex___format__(PyObject formatSpec) {
    ...
    // fromText will have thrown if formatSpecStr is not a PyBytes (including
    // PyUnicode)
    String result;
  ...
  /**
   * Validate a parsed specification, for <code>PyComplex</code>, returning 0 if
   * it is a valid
   * none-format specification, 1 if it is a valid float specification, and some
   * other value if it
   * not a valid type. If it has any other faults (e.g. alternate form was
   * specified) the method
   * raises a descriptive exception.
   *
   * @param spec a parsed PEP-3101 format specification.
   * @return 0, 1, or other value for none-format, a float format, or incorrect
   *         type.
