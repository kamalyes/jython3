...
import static org.kamalyes.jython3.core.stringlib.Encoding.encode_UnicodeEscape;
...
public class PyUnicode extends PySequence implements Iterable {
  ...
  /**
   * Nearly every significant method comes in two versions: one applicable when
   * the string
   * contains only basic plane characters, and one that is correct when
   * supplementary characters
   * are also present. Set this constant <code>true</code> to treat all strings as
   * containing
  ...
  /**
   * A singleton provides the translation service (which is a pass-through) for
   * all BMP strings.
   */
  ...
  // Note: place this after BASIC, since the initialization of str type __doc__
  // depends on BASIC
  ...
  /**
   * Construct a PyUnicode interpreting the Java String argument as UTF-16. If it
   * is known that
   * the string contains no supplementary characters, argument isBasic may be set
   * true by the
  ...
  /**
   * Fundamental all-features constructor on which the others depend. If it is
   * known that the
   * string contains no supplementary characters, argument isBasic may be set true
   * by the caller.
  ...
  /**
   * Index translation between code point index (as seen by Python) and UTF-16
   * index (as used in
   * the Java String.
   */
  ...
  /**
   * The instance of index translation in use in this string. It will be set to
   * either
  ...
  /**
   * A class of index translation that uses the cumulative count so far of
   * supplementary
   * characters, tabulated in blocks of a standard size. The count is then used as
   * an offset
   * between the code point index and the corresponding point in the UTF-16
   * representation.
   */
  private final class Supplementary implements IndexTranslator {

    /**
     * Tabulates cumulative count so far of supplementary characters, by blocks of
     * size M.
     */
    final int[] count;
    ...
    public int codePointIndex(int u) {
      /*
       * Let the desired result be j such that utf16Index(j) = u. As we have only a
       * forward
       * index of the string, we have to conduct a search. In principle, we bound j by
       * a pair
       * of values (j1,j2) such that j1<=j<j2, and in successive iterations, we
       * shorten the
       * range until a unique j is found. In the first part of the search, we work in
       * terms of
       * block numbers, that is, indexes into the count array. We have j<u, and so
       * j<k2*M
       * where:
       */
      int k2 = (u >> LOG2M) + 1;
      ...
      int c2 = count[k2 - 1];
      /*
       * Since the count array is non-decreasing, and j < k2*M, we have u-j <=
       * count[k2-1].
      ...
      /*
       * Now, j (to be found) is in an unknown block k, where k1<=k<k2. We make a
       * binary
       * search, maintaining the inequalities, but moving the end points. When
       * k2=k1+1, we
      ...
      /*
       * At this point, j is known to be in block k1 (and k2=k1+1). c1 is the number
       * of
       * supplementary characters to the left of code point index k1*M and c2 is the
       * number of
       * supplementary characters to the left of code point index (k1+1)*M. We have to
       * search
      ...
      // c1 is the number of supplementary characters to the left of u, so the result
      // j is:
      return u - c1;
    }
    ...
    public int utf16Index(int i) {
      ...
      if (d == e) {
        ...
        /*
         * The offset for the code point index (k+1)*M is different (higher). We must
         * scan
    ...
    @Override
    public int suppCount() {
      // The last element of the count array is the total number of supplementary
      // characters.
  ...
  /**
   * Generate the table that is used by the class {@link Supplementary} to
   * accelerate access to
   * the the implementation string. The method returns <code>null</code> if the
   * string passed
   * contains no surrogate pairs, in which case we'll use {@link #BASIC} as the
   * translator. This
   * method is sensitive to {@link #DEBUG_NON_BMP_METHODS} which if true will
  ...
  private static int[] getSupplementaryCounts(final String string) {
    ...
    /*
     * We scan to the first surrogate code unit, in a simple loop. If we hit the end
     * before we
     * find one, no count array will be necessary and we'll use BASIC. If we find a
     * surrogate it
     * may be half a supplementary character, or a lone surrogate: we'll find out
     * later.
     */
    for (p = 0; p < n; p++) {
    ...
    if (p == n && !DEBUG_NON_BMP_METHODS) {
       ...
       * efficiently translatable to UTF-16 indexes through a table called here
       * count[]. In
       * this array, count[k] contains the total number of supplementary characters up
       * to the
       * end of the k.th block, that is, to the left of code point (k+1)M. We have to
       * fill
      ...
      /*
       * When addressing with a code point index q<=L (the length in code points) we
       * will
       * index the count array with k = q/M. We have q<=L<=n, therefore q/M <= n/M,
       * the
       * maximum valid k is 1 + n/M. A q>=L should raise IndexOutOfBoundsException,
       * but it
      ...
      /*
       * To get the generation of count[] going efficiently, we need to advance the
       * next whole
       * block. The next loop will complete processing of the block containing the
       * first
       * supplementary character. Note that in all these loops, if we exit because p
       * reaches a
       * limit, the count for the last partial block is known from p-q and we take
       * care of
       * that right at the end of this method. The limit of these loops is n-1, so if
       * we spot
      ...
      /*
       * If the string is long enough, we can work in whole blocks of M, and there are
       * fewer
       * things to track. We can't know the number of blocks in advance, but we know
       * there is
      ...
      while (p + 2 * Supplementary.M < n) {
        ...
        // The number of supplementary characters to the left of code point index k*M
        // is:
        count[k++] = p - q;
      }
      ...
      /*
       * There may still be some elements of count[] we haven't set, so we fill to the
       * end
  ...
  /**
   * Called at each code point index, returns 2 if this is a surrogate pair, 1
   * otherwise, and
   * detects lone surrogates as an error. The return is the amount to advance the
   * UTF-16 index. An
   * exception is raised if at <code>p</code> we find a lead surrogate without a
   * trailing one
   * following, or a trailing surrogate directly. It should not be called on the
   * final code unit,
   * when <code>p==string.length()-1</code>, since it may check the next code unit
   * as well.
   *
  ...
  /**
   * Choose an {@link IndexTranslator} implementation for efficient working,
   * according to the
   ...
   * {@inheritDoc}
   * <p>
   * In the <code>PyUnicode</code> version, the arguments are code point indices,
   * such as are
   * received from the Python caller, while the first two elements of the returned
   * array have been
  ...
  protected int[] translateIndices(PyObject start, PyObject end) {
    ...
    // indices[2] and [3] remain Unicode indices (and may be out of bounds) relative
    // to len()
    return indices;
  }
  ...
  /**
   * {@inheritDoc} The indices are code point indices, not UTF-16
   * (<code>char</code>) indices. For
   * example:
   *
   * <pre>
  ...
  /**
   * Creates a PyUnicode from an already interned String. Just means it won't be
   * reinterned if
  ...
  final static PyObject str_new(PyNewWrapper new_, boolean init, PyType subtype,
    ...
    ArgParser ap = new ArgParser("str", args, keywords, new String[] { "string", "encoding",
        "errors" }, 0);
  ...
  // TODO: Parameterize SubsequenceIteratorImpl and friends (and make them
  // Iterable)
  ...
  /**
   * Helper used many times to "coerce" a method argument into a
   * <code>PyUnicode</code> (which it
   ...
   *
   * @param o the object to coerce
   * @return an equivalent <code>PyUnicode</code> (or o itself, or
   *         <code>null</code>)
   */
   ...
   * <code>PyUnicode</code> (which it may already be). A <code>null</code>
   * argument or a
   * <code>PyNone</code> causes <code>null</code> to be returned. A buffer type is
   * not acceptable
  ...
  final PyObject str_strip(PyObject sepObj) {
    ...
    if (isBasicPlane()) {
      ...
      if (sep == null) {
        ...
        // And the strip characters are basic plane too, so use the PyBytes
        // implementation
  ...
  final PyUnicode unicodeJoin(PyObject obj) {
    ...
    // A codec may be invoked to convert str objects to Unicode, and so it's
    // possible
  ...
  /**
   * Encode unicode into a valid decimal String. Throws a UnicodeEncodeError on
   * invalid
   * characters.
   *
   ...
   * <code>unicode.format()</code>. When called with
   * <code>enclosingIterator == null</code>, this
   * method takes this object as its formatting string. The method is also called
   * (calls itself)
   * to deal with nested formatting sepecifications. In that case,
   * <code>enclosingIterator</code>
   * is a {@link MarkupIterator} on this object and <code>value</code> is a
   * substring of this
   * object needing recursive transaltion.
   *
   * @param args              to be interpolated into the string
   * @param keywords          for the trailing args
   * @param enclosingIterator when used nested, null if subject is this
   *                          <code>PyBytes</code>
   * @param value             the format string when
   *                          <code>enclosingIterator</code> is not null
  ...
  /**
   * Return the object referenced by a given field name, interpreted in the
   * context of the given
   * argument list, containing positional and keyword arguments.
   *
   * @param fieldName to interpret.
   * @param bytes     true if the field name is from a PyBytes, false for
   *                  PyUnicode.
  ...
  /**
   * Append to a formatting result, the presentation of one object, according to a
   * given format
