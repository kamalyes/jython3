...
import org.kamalyes.jython3.core.buffer.SimpleBuffer;
...
/**
 * Base class for Jython <code>bytearray</code> (and <code>bytes</code> in due
 * course) that provides
 * most of the Java API, including Java {@link List} behaviour. Attempts to
 * modify the contents
 * through this API will throw a <code>TypeError</code> if the actual type of
 * the object is not
 * mutable. It is possible for a Java client to treat this class as a
 * <code>List&lt;PyInteger></code>, obtaining equivalent functionality to the
 * Python interface in a
 * Java paradigm.
 * <p>
 ...
 * </ul>
 * each returning an appropriate concrete type. Mutable subclasses should
 * override:
 * <ul>
 ...
 * {@link PyByteArray},
 * which offers them as Java API, or exposes them as Python methods. These
 * prototype Python methods
 * mostly accept a {@link PyObject} as argument, where you might have expected a
 * <code>byte[]</code>
 * or <code>BaseBytes</code>, in order to accommodate the full range of types
 * accepted by the Python
 * equivalent: usually, any <code>PyObject</code> that implements
 * {@link BufferProtocol}, providing
 * a one-dimensional array of bytes, is an acceptable argument. In the
 * documentation, the reader
 * will often see the terms "byte array" or "object viewable as bytes" instead
 * of
 * <code>BaseBytes</code> when this broader scope is intended.
 * <p>
 * Where the methods return a <code>BaseBytes</code>, this is will normally be
 * an instance of the
 * class of the object on which the method was actually called. For example
 * {@link #capitalize()},
 * defined in <code>BaseBytes</code> to return a BaseBytes, actually returns a
 * {@link PyByteArray}
 * when applied to a <code>bytearray</code>. Or it may be that the method
 * returns a
 * <code>PyList</code> of instances of the target type, for example
 * {@link #rpartition(PyObject)}.
 * This is achieved by the sub-class defining
 * {@link PySequence#getslice(int, int, int)} and
 * {@link #getBuilder(int)} to return instances of its own type. See the
...
public abstract class BaseBytes extends PySequence implements List<PyInteger> {
  ...
  /**
   * Constructs a zero-length <code>BaseBytes</code> of explicitly-specified
   * sub-type.
   *
  ...
  /**
   * Constructs a byte array of defined type by copying character values from a
   * String. These
  ...
  /**
   * Helper for constructors and methods that manipulate the storage in mutable
   * subclasses. It
   * also permits shared storage between objects, which in general is unsafe if
   * the storage is
   * subject to modification independent of the object now being created.
   * Immutable types may
   ...
   * @throws IllegalArgumentException if the range [offset:offset+size] is not
   *                                  within the array
   ...
   * @param storage byte array allocated by client
   * @param size    number of bytes actually used
   * @throws IllegalArgumentException if the range [0:size] is not within the
   *                                  array bounds of
   *                                  storage.
   */
   ...
   * Helper for constructors and methods that manipulate the storage in mutable
   * subclasses in the
   * case where the storage should consist of exactly the whole of the given
   * array.
   *
   * @param storage byte array allocated by client
   */
   ...
   * ===============
   *
   * Methods here help subclasses set the initial state. They are designed with
   * bytearray in mind,
   * but note that from Python 3, bytes() has the same set of calls and
   * behaviours. In Peterson's
   * "sort of backport" to Python 2.x, bytes is effectively an alias for str and
   * it shows.
   */

  /**
   * Helper for <code>__new__</code> and <code>__init__</code> and the Java API
   * constructor from
   * PyObject in subclasses.
   *
   * @see org.kamalyes.jython3.core.ByteArray#bytearray___init__(PyObject[],
   *      String[])
   * @see org.kamalyes.jython3.core.ByteArray#ByteArray(PyObject)
  ...
  protected void init(PyObject arg) {
    ...
    if (arg == null) {
      ...
      /*
       * bytearray(string) Construct from a text string by default encoding and error
       * policy.
      ...
      /*
       * The remaining alternative is an iterable returning (hopefully) right-sized
       * ints. If
       * it isn't one, we get an exception about not being iterable, or about the
       * values.
       */
      init(arg.asIterable());
  ...
  /**
   * Helper for {@link #setslice(int, int, int, PyObject)}, for
   * <code>__new__</code> and
   * <code>__init__</code> and the Java API constructor from a text string with
   * the specified
   * encoding in subclasses. This method thinly wraps a call to the codecs module
   * and deals with
   ...
   * @throws PyException (TypeError) if the <code>PyBytes</code> is actually a
   *                     {@link PyUnicode}
  ...
  /**
   * Fill a defined section of a byte array by copying character values from a
   * String. These
   * values have to be in the Python byte range 0 to 255.
   *
  ...
  /**
   * Fill a strided slice of a byte array by copying character values from a
   * String. These values
   ...
   * Helper for <code>__new__</code> and <code>__init__</code> and the Java API
   * constructor from
   * objects supporting the Jython implementation of PEP 3118 (Buffer API) in
   * subclasses.
   *
   * @param value an object bearing the Buffer API and consistent with the slice
   *              assignment
   */
   ...
   * Helper for <code>__new__</code> and <code>__init__</code> and the Java API
   * constructor from
   * an arbitrary iterable Python type in subclasses. This will include generators
   * and lists.
   *
  ...
  protected void init(Iterable<? extends PyObject> iter) {
    /*
     * Different strategy is needed from that suited to "random append" operations.
     * We shall
  ...
  /**
   * Intended as a fragment of temporary storage for use we do not know how many
   * bytes of
  ...
  /**
   * A container of temporary storage when we do not know how many bytes to
   * allocate, and we are
  ...
  protected static class FragmentList extends LinkedList<Fragment> {
     ...
     * @throws PyException (ValueError) if any value<0 or value>255 or string
     *                     length!=1
     */
    ...
    /**
     * Move the contents of this container to the given byte array at the specified
     * index. This
    ...
    /**
     * Move the contents of this container to a strided subset of the given byte
     * array at the
     * specified index. Bytes are assigned at start, start+step, start+2*step, and
     * so on until
     * we run out. (You must have checked beforehand that the destination is big
     * enough.) This
     * method leaves this container empty. If the step size is one, it would be much
     * quicker to
   ...
   * ===============
   *
   * The storage is provided by a byte array that may be somewhat larger than the
   * number of bytes
   * actually stored, and these bytes may begin at an offset within the storage.
   * Immutable
   * subclasses of BaseBytes may exploit this to share storage when constructed
   * from a slice of
   * another immutable subclass. Mutable subclasses may exploit it to provide
  ...
  /**
   * Check that an index is within the range of the array, that is
   * <tt>0&lt;=index&lt;size</tt>.
   *
  ...
  /**
   * Allocate fresh, zero-filled storage for the requested number of bytes and
   * make that the size.
   * If the size needed is zero, the "storage" allocated is the shared
   * emptyStorage array. The
   * allocated size may be bigger than needed, and the method chooses a value for
   * offset.
   *
   * @param needed becomes the new value of this.size
   */
  protected void newStorage(int needed) {
    // The implementation for immutable arrays allocates only as many bytes as
    // needed.
    if (needed > 0) {
      try {
  ...
  /**
   * Check that an integer is suitable for storage in a (Python) byte array, and
   * convert it to the
   * Java byte value that can be stored there. (Java bytes run -128..127 whereas
   * Python bytes run
   * 0..255.)
   *
  ...
  /**
   * Check that the value of an PyInteger is suitable for storage in a (Python)
   * byte array, and
   * convert it to the Java byte value that can be stored there. (Java bytes run
   * -128..127 whereas
  ...
  /**
   * Check that the type and value of a PyObject is suitable for storage in a
   * (Python) byte array,
   * and convert it to the Java byte value that can be stored there. (Java bytes
   * run -128..127
  ...
  /**
   * Return a buffer exported by the argument, or return <code>null</code> if it
   * does not bear the
   * buffer API. The caller is responsible for calling {@link PyBuffer#release()}
   * on the buffer,
  ...
  protected static PyBuffer getView(PyObject b) {
    ...
    if (b == null) {
      ...
      /*
       * PyUnicode has the BufferProtocol interface as it extends PyBytes. (It would
       * bring
       * you 0xff&charAt(i) in practice.) However, in CPython the unicode string does
       * not have
       * the buffer API.
       */
      return null;
  ...
  /**
   * Return a buffer exported by the argument or raise an exception if it does not
   * bear the buffer
   * API. The caller is responsible for calling {@link PyBuffer#release()} on the
   * buffer. The
   ...
   * =============================================================================
   * ===============
   * API for org.kamalyes.jython3.core.PySequence
   * =============================================================================
   * ===============
   */
  @Override
  ...
  /*
   * We're not implementing these here, but we can give a stronger guarantee about
   * the return type
   ...
   * The default
   * implementation produces a Python TypeError, for the benefit of immutable
   * types. Mutable types
   * must override it.
   *
  ...
  /**
   * Specialisation of {@link PySequence#getslice(int, int, int)} to contiguous
   * slices (of step size 1) for
   * brevity and efficiency. The default implementation is
   * <code>getslice(start, stop, 1)</code>
   * but it is worth overriding.
   *
   * @param start the position of the first element.
   * @param stop  one more than the position of the last element.
   * @return a subclass instance of BaseBytes corresponding the the given range of
   *         elements.
   */
  @Override
  ...
  /**
   * Class defining the behaviour of <code>bytearray</code> with respect to slice
   * assignment,
  ...
  private class IndexDelegate extends PySequence.DefaultIndexDelegate {
    ...
    /**
     * bytearray treats assignment of a zero-length object to a slice as equivalent
     * to deletion,
  ...
  /**
   * Comparison function between a byte array and a buffer of bytes exported by
   * some other object,
   * such as a String, presented as a <code>PyBuffer</code>, returning 1, 0 or -1
   * as a>b, a==b, or
   * a&lt;b respectively. The comparison is by value, using Python unsigned byte
   * conventions,
   * left-to-right (low to high index). Zero bytes are significant, even at the
   * end of the array:
   * <code>[65,66,67]&lt;"ABC\u0000"</code>, for example and <code>[]</code> is
   * less than every
  ...
  /**
   * Comparison function between byte array types and any other object. The six
   * "rich comparison"
   * operators are based on this.
   *
   * @param b
   * @return 1, 0 or -1 as this>b, this==b, or this&lt;b respectively, or -2 if
   *         the comparison is
   *         not implemented
   */
  ...
  /**
   * Fail-fast comparison function between byte array types and any other object,
   * for when the
   * test is only for equality. The "rich comparison" operators
   * <code>__eq__</code> and
   * <code>__ne__</code> are based on this.
   *
   * @param b
   * @return 0 if this==b, or +1 or -1 if this!=b, or -2 if the comparison is not
   *         implemented
   */
  ...
  /**
   * Implementation of __eq__ (equality) operator, capable of comparison with
   * another byte array.
   * Comparison with an invalid type returns <code>null</code>.
   *
   * @param other Python object to compare with
   * @return Python boolean result or <code>null</code> if not implemented for the
   *         other type.
   */
  ...
  /**
   * Implementation of __ne__ (not equals) operator, capable of comparison with
   * another byte
  ...
  /**
   * Implementation of __lt__ (less than) operator, capable of comparison with
   * another byte array.
   * Comparison with an invalid type returns <code>null</code>.
   *
   * @param other Python object to compare with
  ...
  /**
   * Implementation of __le__ (less than or equal to) operator, capable of
  ...
  /**
   * Implementation of __ge__ (greater than or equal to) operator, capable of
   * comparison with
   * another byte array. Comparison with an invalid type returns
   * <code>null</code>.
   *
   * @param other Python object to compare with
   * @return Python boolean result or <code>null</code> if not implemented for the
  ...
  /**
   * Implementation of __gt__ (greater than) operator, capable of comparison with
   * another byte
   * array. Comparison with an invalid type returns <code>null</code>.
   *
   * @param other Python object to compare with
  ...
  /**
   * Search for the target in this byte array, returning true if found and false
   * if not. The
   * target must either convertible to an integer in the Python byte range, or
   * capable of being
  ...
  protected final synchronized boolean basebytes___contains__(PyObject target) {
    if (target.isIndex()) {
      // Caller is treating this as an array of integers, so the value has to be in
      // range.
   ...
   * <code>endswith( suffix [, start [, end ]] )</code>. An extra boolean argument
   * specifies which
   * to implement on a given call, that is, whether the target is a suffix or
   * prefix. The target
   * may also be a tuple of targets.
   *
   * @param target   prefix or suffix sequence to find (of a type viewable as a
   *                 byte sequence) or a
   *                 tuple of those.
   ...
   * @return true if and only if this byte array ends with (one of)
   *         <code>target</code>.
   */
  protected final synchronized boolean basebytes_starts_or_endswith(PyObject target,
    ...
    /*
     * This cheap 'endswith' trick saves us from maintaining two almost identical
     * methods and
  ...
  /**
   * Test whether the slice <code>[pos:pos+n]</code> of this byte array matches
   * the given target
   * object (accessed as a {@link PyBuffer}) at one end or the orher. That is, if
   * <code>endswith==false</code> test whether the bytes from index
   * <code>pos</code> match all the
   * bytes of the target; if <code>endswith==false</code> test whether the bytes
   * up to index
   * <code>pos+n-1</code> match all the bytes of the target. By implication, the
   * test returns
   ...
   * @return true if and only if the slice [offset:<code>]</code> matches the
   *         given target
   */
  ...
  /**
   * Helper to convert [ostart:oend] to integers with slice semantics relative to
   * this byte array.
  ...
  private int[] indicesEx(PyObject ostart, PyObject oend) {
    // Convert [ostart:oend] to integers with slice semantics relative to this byte
    // array
  ...
  /**
   * Present the bytes of a byte array, with no decoding, as a Java String. The
   * bytes are treated
   * as unsigned character codes, and copied to the to the characters of a String
   * with no change
   * in ordinal value. This could also be described as 'latin-1' or 'ISO-8859-1'
   * decoding of the
   * byte array to a String, since this character encoding is numerically equal to
   * Unicode.
   *
   ...
   * The returned
   * PyObject should be a <code>PyUnicode</code>, since the default codec is
   * well-behaved.
   *
  ...
  /**
   * Decode the byte array to a Unicode string according to the specified encoding
   * and default
   * error policy. The returned PyObject will usually be a <code>PyUnicode</code>,
   * but in practice
   * it is whatever the <code>decode</code> method of the codec decides.
   *
   * @param encoding the name of the codec (uses default codec if
   *                 <code>null</code>)
   * @return object containing the decoded characters
   */
   ...
   * Decode the byte array to a Unicode string according to the specified encoding
   * and error
   * policy. The returned PyObject will usually be a <code>PyUnicode</code>, but
   * in practice it is
   * whatever the <code>decode</code> method of the codec decides.
   *
   * @param encoding the name of the codec (uses default codec if
   *                 <code>null</code>)
   * @param errors   the name of the error policy (uses 'strict' if
   *                 <code>null</code>)
   * @return object containing the decoded characters
   */
  public PyObject decode(String encoding, String errors) {
    /*
     * Provide a Python <code>str</code> input to the decode method of a codec,
     * which in v2.7
     * expects a PyBytes. (In Python 3k the codecs decode from the
     * <code>bytes</code> type, so
  ...
  /**
   * Convenience method to create a <code>TypeError</code> PyException with the
   * message
  ...
  /**
   * Support for pickling byte arrays: reduce a byte array to the actual type,
   * arguments for
   * (re-)construction of the object, and the dictionary of any user-defined
   * sub-class.
   *
  ...
  /**
   * Ready-to-expose implementation of Python __reduce__() method used in pickle
   * (persistence) of
   * Python objects.
   *
   * @return required tuple of type, arguments needed by init, and any user-added
   *         attributes.
   */
  ...
  /**
   * Name the encoding effectively used in __reduce__() suport for pickling: this
   * choice is
   ...
   * ===============
   *
   * A large part of the CPython bytearray.c is devoted to replace( old, new [,
   * count ] ). The
   * special section here reproduces that in Java, but whereas CPython makes heavy
   * use of the
   * buffer API and C memcpy(), we use PyBuffer.copyTo. The logic is much the
   * same, however, even
   * down to variable names.
   */

  /**
   * The very simplest kind of find operation: return the index in the byte array
   * of the first
  ...
  /**
   * This class implements the Boyer-Moore-Horspool Algorithm for find a pattern
   * in text, applied
   * to byte arrays. The BMH algorithm uses a table of bad-character skips derived
   * from the
   * pattern. The bad-character skips table tells us how far from the end of the
   * pattern is a byte
   * that might match the text byte currently aligned with the end of the pattern.
   * For example,
   ...
   * </pre>
   *
   * This puts the 'p' of 'map' against the last byte 'a' of the pattern. Rather
   * than testing the
   * pattern, we will look up 'p' in the skip table. There is an 'p' just 5 steps
   * from the end of
   * the pattern, so we will move the pattern 5 places to the right before trying
   * to match it.
  ...
  protected static class Finder {
    ...
    /**
     * Construct a Finder object that may be used (repeatedly) to find matches with
     * the pattern
    ...
    /**
     * Mask defining how many of the bits of each byte are used when looking up the
     * skip, used
    ...
    /**
     * This method creates a compressed table of bad-character skips from the
     * pattern. The entry
     * for a given byte value tells us how far it is from the end of the pattern,
     * being 0 for
     * the actual last byte, or is equal to the length of the pattern if the byte
     * does not occur
     * in the pattern. The table is compressed in that only the least-significant
     * bits of the
     * byte index are used. In the case where 5 bits are used, the table is only 32
     * elements
     * long, rather than (as it might be) 256 bytes, the number of distinct byte
     * values.
     */
    ...
    /**
     * Set the text to be searched in successive calls to <code>nextIndex()</code>,
     * where the
    ...
    /**
     * Return the index in the text array where the preceding pattern match ends
     * (one beyond the
     * last character matched), which may also be one beyond the effective end ofthe
     * text.
     * Between a call to setText() and the first call to <code>nextIndex()</code>
     * return the
     * start position.
     * <p>
    ...
    /**
     * Find the next index in the text array where the pattern starts. Successive
     * calls to
     * <code>nextIndex()</code> return the successive (non-overlapping) occurrences
     * of the
     * pattern in the text.
     *
    ...
    public int nextIndex() {
      ...
      if (skipTable != null) { // ... which it will not be if m>1 and setText() was called
        /*
         * Boyer-Moore-Horspool Algorithm using a Bloom array. Based on CPython
         * stringlib,
    ...
    /**
     * Count the non-overlapping occurrences of the pattern in the text, where the
     * text is
     * effectively only the bytes <code>text[start]</code> to
     * <code>text[start+size-1]</code>
     * inclusive.
     *
     * @param text  to search
     * @param start first position to consider
  ...
  /**
   * This class is the complement of {@link Finder} and implements the
  ...
  protected static class ReverseFinder extends Finder {
    ...
    /**
     * Construct a ReverseFinder object that may be used (repeatedly) to find
     * matches with the
     ...
     * like: <code>skip = skipTable[MASK & currentByte]</code>.
     * <p>
     * Note that the way this is written at the moment, if <code>MASK</code> is
     * different from
     * <code>super.MASK</code> <code>calculateSkipTable()</code> and
     * <code>nextIndex()</code>
     ...
     * This method creates a compressed table of bad-character skips from the
     * pattern for
     * reverse-searching. The entry for a given byte value tells us how far it is
     * from the start
     * of the pattern, being 0 for the actual first byte, or is equal to the length
     * of the
     * pattern if the byte does not occur in the pattern. The table is compressed in
     * that only
     * the least-significant bits of the byte index are used. In the case where 5
     * bits are used,
     * the table is only 32 elements long, rather than (as it might be) 256 bytes,
     * the number of
    ...
    /**
     * Find the next index in the text array where the pattern starts, but working
     * backwards.
     * Successive calls to <code>nextIndex()</code> return the successive
     * (non-overlapping)
  ...
  /**
   * Class for quickly determining whether a given byte is a member of a defined
   * set. this class
   * provides an efficient mechanism when a lot of bytes must be tested against
   * the same set.
   */
  ...
  /**
   * Convenience routine producing a ValueError for "empty separator" if the
   * PyBuffer is of an
  ...
  /**
   * Return the index [0..size-1] such that all bytes from here to the right match
   * one in
   * <code>byteSet</code>, that is, the index of the matching tail, or
   * <code>size</code> if there
  ...
  protected int rstripIndex(ByteSet byteSet) {
    // Run down the storage checking the next byte against byteSet (or until we hit
    // the start)
  ...
  /**
   * Return the index [0..size-1] such that all bytes from here to the right are
   * whitespace, that
   * is, the index of the whitespace tail, or <code>size</code> if there is no
   * whitespace tail.
   *
   * @return index of strippable tail
   */
   ...
   * <code>count( sub [, start [, end ]] )</code>. Return
   * the number of non-overlapping occurrences of <code>sub</code> in the range
   * [start, end].
   * Optional arguments <code>start</code> and <code>end</code> (which may be
   * <code>null</code> or
  ...
  /**
   * Ready-to-expose implementation of Python
   * <code>find( sub [, start [, end ]] )</code>. Return
   * the lowest index in the byte array where byte sequence <code>sub</code> is
   * found, such that
   * <code>sub</code> is contained in the slice <code>[start:end]</code>.
   * Arguments
   * <code>start</code> and <code>end</code> (which may be <code>null</code> or
   * <code>Py.None</code> ) are interpreted as in slice notation. Return -1 if
   * <code>sub</code> is
   * not found.
   *
   * @param sub    bytes to find
  ...
  /**
   * Almost ready-to-expose implementation of Python class method
   * <code>fromhex(string)</code>.
   * This assigns a value to the passed byte array object from a string of
   * two-digit hexadecimal
   * numbers. Spaces (but not whitespace in general) are acceptable around the
   * numbers, not
   * within. Non-hexadecimal characters or un-paired hex digits raise a
   ...
   * @throws PyException (ValueError) if non-hex characters, or isolated ones, are
   *                     encountered
   */
  static void basebytes_fromhex(BaseBytes result, String hex) throws PyException {
    ...
    /*
     * When charAt(i) is a hex digit, we will always access hex.charAt(i+1), and
     * catch the
  ...
  /**
   * Implementation of Python <code>partition(sep)</code>, returning a 3-tuple of
   * byte arrays (of
   * the same type as <code>this</code>).
   *
   * Split the string at the first occurrence of <code>sep</code>, and return a
   * 3-tuple containing
   * the part before the separator, the separator itself, and the part after the
   * separator. If the
   * separator is not found, return a 3-tuple containing the string itself,
   * followed by two empty
   * byte arrays.
   * <p>
   * The elements of the <code>PyTuple</code> returned by this method are
  ...
  /**
   * Construct return value for implementation of Python
   * <code>partition(sep)</code> or
  ...
  /**
   * Ready-to-expose implementation of Python
   * <code>rfind( sub [, start [, end ]] )</code>. Return
   * the highest index in the byte array where byte sequence <code>sub</code> is
   * found, such that
   * <code>sub</code> is contained in the slice <code>[start:end]</code>.
   * Arguments
   * <code>start</code> and <code>end</code> (which may be <code>null</code> or
   ...
   * <code>rfind( sub [, start [, end ]] )</code>. Return the lowest or highest
   * index in the byte
   * array where byte sequence used to construct <code>finder</code> is found. The
   * particular type
   * (plain <code>Finder</code> or <code>ReverseFinder</code>) determines the
   * direction.
   *
  ...
  private final int find(Finder finder, PyObject ostart, PyObject oend) {
    ...
    // Make this slice the thing we search. Note finder works with Java index in
    // storage.
   ...
   * <code>replace( old, new [, count ] )</code>, returning a
   * <code>PyByteArray</code> with all
   * occurrences of sequence <code>oldB</code> replaced by <code>newB</code>. If
   * the optional
   * argument <code>count</code> is given, only the first <code>count</code>
   * occurrences are
   * replaced.
   *
  ...
  final synchronized PyByteArray basebytes_replace(PyObject oldB, PyObject newB, int maxcount) {
    ...
    try (PyBuffer to = getViewOrError(newB); PyBuffer from = getViewOrError(oldB)) {
      /*
       * The logic of the first section is copied exactly from CPython in order to get
       * the
       * same behaviour. The "headline" description of replace is simple enough but
       * the corner
  ...
  /*
   * Algorithms for different cases of string replacement. CPython also has
   * specialisations for
   * when 'from' or 'to' or both are single bytes. This may also be worth doing in
   * Java when the
   * 'to' is a single byte. (The 'from' is turned into a Finder object which
   * already makes a
   * special case of single bytes.)
   */

  /**
   * Helper for {@link #basebytes_replace(PyObject, PyObject, int)} implementing
   * the general case
   * of byte-string replacement when the new and old strings have different
   * lengths.
   *
  ...
  /**
   * Handle the interleaving case b'hello'.replace(b'', b'..') =
   * b'..h..e..l..l..o..' At the call
   ...
   * Helper for {@link #basebytes_replace(PyObject, PyObject, int)} implementing
   * the special case
   * of byte-string replacement when the new string has zero length, i.e.
   * deletion.
   *
   ...
   * Helper for {@link #basebytes_replace(PyObject, PyObject, int)} implementing
   * the special case
   * of byte-string replacement when the new and old strings have the same length.
   * The key
   * observation here is that the result has the same size as this byte array, and
   * we know this
  ...
  /**
   * Implementation of Python <code>rpartition(sep)</code>, returning a 3-tuple of
   * byte arrays (of
   * the same type as <code>this</code>).
   *
   * Split the string at the rightmost occurrence of <code>sep</code>, and return
   * a 3-tuple
   * containing the part before the separator, the separator itself, and the part
   * after the
   * separator. If the separator is not found, return a 3-tuple containing two
   * empty byte arrays,
  ...
  /**
   * Implementation of Python <code>rsplit()</code>, that returns a list of the
   * words in the byte
   * array, using whitespace as the delimiter. See {@link #rsplit(PyObject, int)}.
   * <p>
   * The elements of the <code>PyList</code> returned by this method are instances
   * of the same
   * actual type as <code>this</code>.
   *
  ...
  /**
   * Implementation of Python <code>rsplit(sep)</code>, that returns a list of the
   * words in the
   * byte array, using <code>sep</code> as the delimiter. See
   * {@link #rsplit(PyObject, int)} for
   ...
   * actual type as <code>this</code>.
   *
   * @param sep <code>bytes</code>, or object viewable as bytes, defining the
   *            separator
   * @return PyList of byte arrays that result from the split
   */
  ...
  /**
   * Implementation of Python <code>rsplit(sep, maxsplit)</code>, that returns a
   * list of the words
   * in the byte array, using <code>sep</code> as the delimiter. If
   * <code>maxsplit</code> is
   * given, at most <code>maxsplit</code> splits are done (thus, the list will
   * have at most
   * <code>maxsplit+1</code> elements). If <code>maxsplit</code> is not specified,
   * then there is
   * no limit on the number of splits (all possible splits are made).
   * <p>
   * The semantics of <code>sep</code> and maxcount are identical to those of
   * <code>split(sep, maxsplit)</code> , except that splits are generated from the
   * right (and
   * pushed onto the front of the result list). The result is only different from
   * that of
   * <code>split</code> if <code>maxcount</code> limits the number of splits. For
   * example,
   * <ul>
  ...
  /**
   * Ready-to-expose implementation of Python <code>rsplit(sep, maxsplit)</code>,
   * that returns a
   * list of the words in the byte array, using <code>sep</code> as the delimiter.
   * Use the defines
   ...
   * Implementation of Python <code>rsplit(sep, maxsplit)</code>, that returns a
   * list of the words
   * in the byte array, using <code>sep</code> (which is not <code>null</code>) as
   * the delimiter.
   * If <code>maxsplit>=0</code>, at most <code>maxsplit</code> splits are done
   * (thus, the list
   * will have at most <code>maxsplit+1</code> elements). If
   * <code>maxsplit&lt;0</code>, then
  ...
  /**
   * Implementation of Python <code>rsplit(None, maxsplit)</code>, that returns a
   * list of the
   * words in the byte array, using whitespace as the delimiter. If
   * <code>maxsplit</code> is
   * given, at most <code>maxsplit</code> splits are done (thus, the list will
   * have at most
   * <code>maxsplit+1</code> elements). If maxsplit is not specified, then there
   * is no limit on
   ...
   * result will
   * contain no empty strings at the start or end if the string has leading or
   * trailing
  ...
  final synchronized PyList basebytes_rsplit_whitespace(int maxsplit) {
    ...
    while (q > offset && maxsplit-- != 0) {
      ...
      // storage[p] is the first byte of the word. (p=offset or storage[p-1] is
      // whitespace.)
  ...
  /**
   * Implementation of Python <code>split()</code>, that returns a list of the
   * words in the byte
  ...
  /**
   * Implementation of Python <code>split(sep)</code>, that returns a list of the
   * words in the
   * byte array, using <code>sep</code> as the delimiter. See
   * {@link #split(PyObject, int)} for
   * the semantics of the separator.
   * <p>
   * The elements of the <code>PyList</code> returned by this method are instances
   * of the same
  ...
  /**
   * Implementation of Python <code>split(sep, maxsplit)</code>, that returns a
   * list of the words
   * in the byte array, using <code>sep</code> as the delimiter. If
   * <code>maxsplit</code> is
   * given, at most <code>maxsplit</code> splits are done. (Thus, the list will
   ...
   * no limit on the number of splits (all possible splits are made).
   * <p>
   * If <code>sep</code> is given, consecutive delimiters are not grouped together
   * and are deemed
   * to delimit empty strings (for example, <code>'1,,2'.split(',')</code> returns
   * <code>['1', '', '2']</code>). The <code>sep</code> argument may consist of
   * multiple
   * characters (for example, <code>'1&lt;>2&lt;>3'.split('&lt;>')</code> returns
   * <code>['1',
   * '2', '3']</code>). Splitting an empty string with a specified separator
   * <code> ['']</code>.
   * <p>
   * If <code>sep</code> is not specified or is <code>None</code>, a different
   * splitting algorithm
   * is applied: runs of consecutive whitespace are regarded as a single
   * separator, and the result
   * will contain no empty strings at the start or end if the string has leading
   * or trailing
   * whitespace. Consequently, splitting an empty string or a string consisting of
   * just whitespace
  ...
  /**
   * Ready-to-expose implementation of Python <code>split(sep, maxsplit)</code>,
   * that returns a
   * list of the words in the byte array, using <code>sep</code> as the delimiter.
   * Use the defines
   * whitespace semantics if <code>sep</code> is <code>null</code>.
  ...
  final synchronized PyList basebytes_split_explicit(PyObject sep, int maxsplit) {
    ...
    try (PyBuffer separator = getViewOrError(sep)) {
      ...
      // While we found a separator, and we have some splits left (if maxsplit
      // started>=0)
  ...
  /**
   * Implementation of Python <code>split(None, maxsplit)</code>, that returns a
   * list of the words
   * in the byte array, using whitespace as the delimiter. If
   * <code>maxsplit</code> is given, at
   * most maxsplit splits are done (thus, the list will have at most
   * <code>maxsplit+1</code>
   * elements). If <code>maxsplit</code> is not specified, then there is no limit
   * on the number of
  ...
  /**
   * Implementation of Python <code>splitlines()</code>, returning a list of the
   * lines in the byte
   * array, breaking at line boundaries. Line breaks are not included in the
   * resulting segments.
   * <p>
   * The elements of the <code>PyList</code> returned by this method are instances
   * of the same
  ...
  /**
   * Implementation of Python <code>splitlines(keepends)</code>, returning a list
   * of the lines in
   * the string, breaking at line boundaries. Line breaks are not included in the
   * resulting list
  ...
  /**
   * Ready-to-expose implementation of Python <code>splitlines(keepends)</code>,
   * returning a list
   * of the lines in the array, breaking at line boundaries. Line breaks are not
   * included in the
  ...
  /**
   * Helper to check the fill byte for {@link #rjust(String)},
   * {@link #ljust(String)} and
   * {@link #center(String)}, which is required to be a single character string,
   * treated as a
   * byte.
   *
   * @param function name
   ...
   * {@link #ljust(String)} and {@link #center(String)}. Clients calculate the
   * left and right fill
   * values according to their nature, and ignoring the possibility that the
   * desired
   * <code>width=left+size+right</code> may be less than <code>this.size</code>.
   * This method does
   * all the work, and deals with that exceptional case by returning
   * <code>self[:]</code>.
   *
  ...
  /**
   * A ready-to-expose implementation of Python
   * <code>center(width [, fillchar])</code>: return
   * the bytes centered in an array of length <code>width</code>. Padding is done
   * using the
   * specified fillchar (default is a space). A copy of the original byte array is
   * returned if
   * <code>width</code> is less than <code>this.size()</code>. (Immutable
   * subclasses may return
   * exactly the original object.)
   *
   * @param width    desired
   * @param fillchar one-byte String to fill with, or <code>null</code> implying
   *                 space
   * @return (possibly new) byte array containing the result
   */
  final BaseBytes basebytes_center(int width, String fillchar) {
    ...
    int fill = width - size;
    // CPython uses this formula, which makes a difference when width is odd and
    // size even
  ...
  /**
   * A ready-to-expose implementation of Python
   * <code>ljust(width [, fillchar])</code>: return the
   * bytes left-justified in an array of length <code>width</code>. Padding is
   * done using the
   * specified fillchar (default is a space). A copy of the original byte array is
   * returned if
   * <code>width</code> is less than <code>this.size()</code>. (Immutable
  ...
  /**
   * A ready-to-expose implementation of Python
   * <code>rjust(width [, fillchar])</code>: return the
   * bytes right-justified in an array of length <code>width</code>. Padding is
   * done using the
   * specified fillchar (default is a space). A copy of the original byte array is
   * returned if
   * <code>width</code> is less than <code>this.size()</code>. (Immutable
  ...
  /**
   * Ready-to-expose implementation of Python <code>expandtabs([tabsize])</code>:
   * return a copy of
   * the byte array where all tab characters are replaced by one or more spaces,
   * depending on the
   * current column and the given tab size. The column number is reset to zero
   * after each newline
   * occurring in the array. This treats other non-printing characters or escape
   * sequences as
   * regular characters.
   * <p>
   * The actual class of the returned object is determined by
   * {@link #getBuilder(int)}.
   *
  ...
  /**
   * Ready-to-expose implementation of Python <code>zfill(width):</code> return
   * the numeric string
   * left filled with zeros in a byte array of length width. A sign prefix is
   * handled correctly if
   * it is in the first byte. A copy of the original is returned if width is less
   * than the current
   * size of the array.
   *
   * @param width desired
  ...
  /**
   * Java API equivalent of Python <code>isalnum()</code>. This method treats the
   * bytes as Unicode
   * pont codes and is consistent with Java's
   * {@link Character#isLetterOrDigit(char)}.
   *
   * @return true if all bytes in the array are point codes for alphanumerics and
   *         there is at
  ...
  /**
   * Java API equivalent of Python <code>isalpha()</code>. This method treats the
   * bytes as Unicode
   * pont codes and is consistent with Java's {@link Character#isLetter(char)}.
   *
   * @return true if all bytes in the array are alphabetic and there is at least
   *         one byte, false
   *         otherwise
   */
  ...
  /**
   * Java API equivalent of Python <code>isdigit()</code>. This method treats the
   * bytes as Unicode
   * pont codes and is consistent with Java's {@link Character#isDigit(char)}.
   *
   * @return true if all bytes in the array are point codes for digits and there
   *         is at least one
  ...
  /**
   * Java API equivalent of Python <code>islower()</code>. This method treats the
   * bytes as Unicode
   * pont codes and is consistent with Java's {@link Character#isLowerCase(char)}.
   *
   * @return true if all cased bytes in the array are point codes for lowercase
   *         characters and
  ...
  /**
   * Java API equivalent of Python <code>isspace()</code>. This method treats the
   * bytes as Unicode
   * pont codes and is consistent with Java's
   * {@link Character#isWhitespace(char)}.
   *
   * @return true if all the bytes in the array are point codes for whitespace
   *         characters and
  ...
  /**
   * Java API equivalent of Python <code>istitle()</code>. This method treats the
   * bytes as Unicode
   ...
   *         cased byte, for
   *         example uppercase characters may only follow uncased bytes and
  ...
  /**
   * Java API equivalent of Python <code>isupper()</code>. This method treats the
   * bytes as Unicode
   * pont codes and is consistent with Java's {@link Character#isUpperCase(char)}.
   *
   * @return true if all cased bytes in the array are point codes for uppercase
   *         characters and
   *         there is at least one cased byte, false otherwise.
   */
  ...
  /**
   * Java API equivalent of Python <code>capitalize()</code>. This method treats
   * the bytes as
   * Unicode pont codes and is consistent with Java's
   * {@link Character#toUpperCase(char)} and
   * {@link Character#toLowerCase(char)}. The <code>BaseBytes</code> returned by
   * this method has
   * the same actual type as <code>this/self</code>.
   *
   * @return a copy of the array with its first character capitalized and the rest
   *         lowercased.
   */
  ...
  /**
   * Java API equivalent of Python <code>lower()</code>. This method treats the
   * bytes as Unicode
   ...
   * <code>this/self</code>.
   *
   * @return a copy of the array with all the cased characters converted to
   *         lowercase.
   */
  ...
  /**
   * Ready-to-expose implementation of Python <code>lower()</code>. The
   * <code>BaseBytes</code>
  ...
  /**
   * Java API equivalent of Python <code>swapcase()</code>. This method treats the
   * bytes as
   * Unicode pont codes and is consistent with Java's
   * {@link Character#toUpperCase(char)} and
   * {@link Character#toLowerCase(char)}. The <code>BaseBytes</code> returned by
   ...
   * the same actual type as <code>this/self</code>.
   *
   * @return a copy of the array with uppercase characters converted to lowercase
   *         and vice versa.
   */
  ...
  /**
   * Ready-to-expose implementation of Python <code>swapcase()</code>. The
   * <code>BaseBytes</code>
   * returned by this method has the same actual type as <code>this/self</code>.
   *
   * @return a copy of the array with uppercase characters converted to lowercase
  ...
  /**
   * Java API equivalent of Python <code>title()</code>. The algorithm uses a
   * simple
   * language-independent definition of a word as groups of consecutive letters.
   * The definition
   * works in many contexts but it means that apostrophes in contractions and
   * possessives form
   * word boundaries, which may not be the desired result. The
   * <code>BaseBytes</code> returned by
   * this method has the same actual type as <code>this/self</code>.
   *
   * @return a titlecased version of the array where words start with an uppercase
   *         character and
  ...
  /**
   * Ready-to-expose implementation of Python <code>title()</code>. The
   * <code>BaseBytes</code>
   * returned by this method has the same actual type as <code>this/self</code>.
   *
   * @return a titlecased version of the array where words start with an uppercase
   ...
   * <code>x.upper().isupper()</code> might be <code>false</code> if the array
   * contains uncased
   * characters. The <code>BaseBytes</code> returned by this method has the same
   * actual type as
   * <code>this/self</code>.
   *
   * @return a copy of the array with all the cased characters converted to
  ...
  /**
   * Ready-to-expose implementation of Python <code>upper()</code>. The
   * <code>BaseBytes</code>
   * returned by this method has the same actual type as <code>this/self</code>.
   *
   * @return a copy of the array with all the cased characters converted to
  ...
  /**
   * Almost ready-to-expose Python <code>__repr__()</code>, based on treating the
   * bytes as point
   * codes. The value added by this method is conversion of non-printing point
   * codes to
   * hexadecimal escapes in printable ASCII, and bracketed by the given before and
   * after strings.
   ...
   *         <code>before + "'" + String(this) + "'" + after</code>
   */
  ...
  /**
   * Access to the byte array as a {@link java.util.List}. The List interface
   * supplied by
  ...
  protected final List<PyInteger> listDelegate = new AbstractList<PyInteger>() {
     ...
     * subsequent
     * elements to the left (subtracts one from their indices). Returns the element
     * that was
  ...
  /**
   * Returns true if this list contains the specified value. More formally,
   * returns true if and
   * only if this list contains at least one integer e such that
   * o.equals(PyInteger(e)).
   */
  @Override
   ...
   * <code>other</code>. In
   * the case where <code>other</code> is a <code>PyObject</code>, the comparison
   * used is the
   ...
   * @return <code>true</code> if and only if this byte array is equal (in value)
   *         to
   *         <code>other</code>
   */
  @Override
  ...
  /**
   * A <code>Builder</code> holds a buffer of bytes to which new bytes may be
   * appended while
   * constructing the value of byte array, even when the type ultimately
   * constructed is immutable.
   * The value it builds may be transferred (normally without copying) to a new
   * instance of the
   * type being built.
   * <p>
   * <code>Builder</code> is an abstract class. The each sub-class of
   * <code>BaseBytes</code> may
   * define its own concrete implementation in which {@link Builder#getResult()}
   * returns an object
   * of its own type, taking its value from the <code>Builder</code> contents
   * using
   * {@link #getStorage()} and {@link #getSize()}. Methods in
   * <code>BaseBytes</code> obtain a
   * <code>Builder</code> by calling the abstract method
   * {@link BaseBytes#getBuilder(int)}, which
   * the sub-class also defines, to return an isnstance of its characteristic
   * <code>Builder</code>
   * sub-class. The subclass that uses a method from <code>BaseBytes</code>
   * returning a
   * <code>BaseBytes</code> has to cast a returned from a BaseBytes method to its
   * proper type.
   * which it can do without error, since it was responsible for its actual type.
   * <p>
   * <b>Implementation note:</b> This can be done in a type-safe way but, in the
   * present design,
  ...
  protected static abstract class Builder /* <B> */ {
    ...
    /**
     * Return an object of type B extends <code>BaseBytes</code> whose content is
     * what we built.
     */
    ...
    /**
     * Get an array of bytes containing the accumulated value, and clear the
     * existing contents
     * of the Builder. {@link #getCount()} returns the number of valid bytes in this
     * array,
     * which may be longer than the valid data.
     * <p>
     * It is intended the client call this method only once to get the result of a
     * series of
     * append operations. A second call to {@link #getCount()}, before any further
     * appending,
     * returns a zero-length array. This is to ensure that the same array is not
     * given out
     * twice. However, {@link #getCount()} continues to return the number bytes
     * accumulated
    ...
    /**
     * Number of bytes accumulated. In conjunctin with {@link #getStorage()}, this
     * provides the
    ...
    /**
     * Append a number of repeats of a single byte to the value, fo example in
     * padding.
     *
  ...
  /**
   * Choose a size appropriate to store the given number of bytes, with some room
   * for growth, when
   * allocating storage for mutable types or <code>Builder</code>. We'll be more
   * generous than
  ...
  protected static final int roundUp(int size) {
    /*
     * The CPython formula is: size + (size >> 3) + (size < 9 ? 3 : 6). But when the
     * array
     * grows, CPython can use a realloc(), which will often be able to extend the
     * allocation
     * into memory already secretly allocated by the initial malloc(). Extension in
     * Java means
  ...
  /**
   * Every sub-class of BaseBytes overrides this method to return a
   * <code>Builder&lt;B></code>
   * where <code>B</code> is (normally) that class's particular type, and it
   * extends
   * <code>Builder&lt;B></code> so that {@link Builder#getResult()} produces an
   * instance of
