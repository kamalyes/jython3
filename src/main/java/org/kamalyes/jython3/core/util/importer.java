...
/**
 * A base class for PEP-302 path hooks. Handles looking through source,
 * compiled, package and module
...
public abstract class importer extends PyObject {
  ...
  /**
   * Returns the value to fill in __path__ on a module with the given full module
   * name created by
   * this importer.
   */
  ...
  /**
   * Given a full module name, return the potential file path in the archive
   * (without extension).
   */
  protected abstract String makeFilename(String fullname);

  /**
   * Given a full module name, return the potential file path including the
   * archive (without
   * extension).
   */
  ...
  /**
   * Returns an entry for a filename from makeFilename with a potential suffix
   * such that this
   * importer can make a bundle with it, or null if fullFilename doesn't exist in
   * this importer.
   */
  ...
  protected final PyObject importer_load_module(String fullname) {
    ...
    // the module *must* be in sys.modules before the loader executes the module
    // code; the
  ...
  /**
   * Bundle is an InputStream, bundled together with a method that can close the
   * input stream and
  ...
  protected abstract static class Bundle implements AutoCloseable {
    ...
    /**
     * Close the underlying resource if necessary. Raises an IOError if a problem
     * occurs.
     */
    @Override
  ...
  /**
   * Given a path to a compiled file in the archive, return the modification time
   * of the
   * matching .py file.
   *
  ...
  /**
   * Return the code object and its associated data for the module with the fully
   * qualified name.
   *
   * @param fullname
   *                 the fully qualified name of the module
  ...
  /**
   * ModuleInfo is the blueprint for the importlib.machinery.ModuleSpec from
   * PEP-451
   */
