...
/**
 * A wrapper for all python exception. Note that the well-known python
 * exceptions are <b>not</b>
 * subclasses of PyException. Instead the python exception class is stored in
 * the <code>type</code>
...
public class PyException extends RuntimeException implements Traverseproc {
  ...
  /**
   * The python exception class (for class exception) or identifier (for string
   * exception).
   */
  public PyObject type;

  /**
   * The exception instance (for class exception) or exception value (for string
   * exception).
   */
  ...
  public void tracebackHere(PyFrame here, boolean isFinally) {
    ...
    // finally blocks immediately tracebackHere: so they toggle isReRaise to skip
    // the
