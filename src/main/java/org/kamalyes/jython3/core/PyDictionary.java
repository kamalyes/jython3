...
public class PyDictionary extends PyObject implements ConcurrentMap, Traverseproc {
  ...
  /**
   * Create a dictionary with keys and values list, used when create keyvalue-only
   * parameter defaults
   */
   ...
   * factories, with different backing maps, at the cost that it prevents us from
   * making table be final.
   */
  // TODO we may want to revisit this API, but our chain calling of super makes
  // this tough
   ...
   *
   * @param elements The initial elements that is inserted in the dictionary. Even
   *                 numbered elements
   ...
   * @param defaultObj the value to return if the key does not exists in the
   *                   mapping.
   */
  ...
  // TODO: needs __doc__ but CPython does not define setifabsent
  ...
  /*
   * The following methods implement the java.util.Map interface which allows
   * PyDictionary to be
   * passed to java methods that take java.util.Map as a parameter. Basically, the
   * Map methods are
   * a wrapper around the PyDictionary's Map container stored in member variable
   * 'table'. These
   * methods convert java Object to PyObjects on insertion, and PyObject to
   * Objects on retrieval.
   */
...
/**
 * Basic implementation of Entry that just holds onto a key and value and
 * returns them.
 */
...
/**
 * Wrapper for a Entry object returned from the java.util.Set object which in
 * turn is returned by
 * the entrySet method of java.util.Map. This is needed to correctly convert
 * from PyObjects to java
 * Objects. Note that we take care in the equals and hashCode methods to make
 * sure these methods are
 * consistent with Entry objects that contain java Objects for a value so that
 * on the java side they
 ...
 * by the entrySet method of PyDictionary. Essentially like PyTojavaMapEntry,
 * but going the other
...
/**
 * Set wrapper for the entrySet method. Entry objects are wrapped further in
 * JavaToPyMapEntry and
 * PyToJavaMapEntry. Note - The set interface is reliable for standard objects
 * like strings and
 * integers, but may be inconsistent for other types of objects since the equals
 * method may return
 * false for Entry object that hold more elaborate PyObject types. However, we
 * insure that this
 * interface works when the Entry object originates from a Set object retrieved
 * from a PyDictionary.
 */
class PyMapEntrySet extends PyMapSet {
  ...
  // We know that PyMapEntrySet will only contains entries, so if the object being
  // passed in is
  // null or not an Entry, then return null which will match nothing for remove
  // and contains
  // methods.
  @Override
  Object toPython(Object o) {
...
/**
 * PyMapSet serves as a wrapper around Set Objects returned by the java.util.Map
 * interface of
 * PyDictionary. entrySet, values and keySet methods return this type for the
 * java.util.Map
 * implementation. This class is necessary as a wrapper to convert PyObjects to
 * java Objects for
 * methods that return values, and convert Objects to PyObjects for methods that
 * take values. The
 * translation is necessary to provide java access to jython dictionary objects.
 * This wrapper also
 * provides the expected backing functionality such that changes to the wrapper
 * set or reflected in
 * PyDictionary.
 */
