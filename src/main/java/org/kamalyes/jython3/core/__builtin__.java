...
import org.kamalyes.jython3.core.util.ExtraMath;
import org.kamalyes.jython3.core.util.RelativeFile;
import org.kamalyes.jython3.modules._io._io;
...
public class __builtin__ {
  ...
  public static final int ord(PyObject c) throws PyException {
    ...
    /*
     * It was a qualifying string-like object, but if we didn't return or throw by
     * now, the
  ...
  /**
   * Return number of items in range (lo, hi, step), when arguments are PyInteger
   * or
   * PyLong objects. step > 0 required. Return a value < 0 if & only if the true
   * value
  ...
  /**
   * Helper function for handleRangeLongs. If arg is int or long object, returns
   * it. If
   * arg is float, raises type error. As a last resort, creates a new int by
   * calling arg
  ...
  /**
   * Companion to <code>raw_input</code> built-in function used when the
   * interactive interpreter
   * is directed to a file.
   *
   * @param prompt to issue at console before read
   * @param file   a file-like object to read from
   * @return line of text from the file (encoded as bytes values compatible with
   *         PyBytes)
   */
  ...
  /**
   * Implementation of <code>raw_input(prompt)</code> built-in function using the
   * console
   * indirectly via <code>sys.stdin</code> and <code>sys.stdin</code>.
   *
   * @param prompt to issue at console before read
   * @return line of text from console (encoded as bytes values compatible with
   *         PyBytes)
   */
  ...
  /**
   * Implementation of <code>raw_input()</code> built-in function using the
   * console directly.
   *
   * @return line of text from console (encoded as bytes values compatible with
   *         PyBytes)
  ...
  /**
   * Return an interned String from name, raising a TypeError when conversion
   * fails.
   *
...
class CompileFunction extends PyBuiltinFunction {
   ...
   *          null
   *
   *          XXX: Reaches into implementation details -- needs to be reviewed if
   *          our
