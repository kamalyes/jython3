...
public class PyList extends PySequenceList implements List {
   ...
   *
   * @param o
   *          the argument to test for. Testing is done with the <code>==</code>
   *          operator.
   */
  ...
  /**
   * Remove the first occurence of the argument from the list. The elements
   * arecompared with the
  ...
  /**
   * Reverses the items of s in place. The reverse() methods modify the list in
   * place for economy
   * of space when reversing a large list. It doesn't return the reversed list to
   * remind you of
   * this side effect.
   */
  public void reverse() {
  ...
  /**
   * Sort the items of the list in place. The compare argument is a function of
   * two arguments
   * (list items) which should return -1, 0 or 1 depending on whether the first
   * argument is
   * considered smaller than, equal to, or larger than the second argument. Note
   * that this slows
   * the sorting process down considerably; e.g. to sort a list in reverse order
   * it is much faster
   * to use calls to the methods sort() and reverse() than to use the built-in
   * function sort()
   ...
   */
  /**
   * Sort the items of the list in place. Items is compared with the normal
   * relative comparison
   * operators.
   */
  ...
  // a bunch of optimized paths for sort to avoid unnecessary work, such as DSU or
  // checking compare functions for null
