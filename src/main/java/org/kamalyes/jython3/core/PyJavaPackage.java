...
public class PyJavaPackage extends PyObject implements Traverseproc {
  private static final PyObject JAVAPKG_PATH = new PyList(
      new PyObject[] { new PyUnicode(JavaImporter.JAVA_IMPORT_PATH_ENTRY) });
  ...
  /**
   * Used for 'from xyz import *', dynamically dir pkg filling up __dict__. It
   * uses
   * {@link PackageManager#doDir} implementation furnished by the control package
   * manager with
   * instantiate true. The package manager should load classes with
   * {@link #addClass} in the
   * package.
   *
