...
public abstract class BaseSet extends PyObject implements Set, Traverseproc {
   ...
   *
   * @return <code>true</code> if the set is not empty, <code>false</code>
   *         otherwise
   */
  ...
  /**
   * Return a PyFrozenSet whose contents are shared with value when value is a
   * BaseSet and pye is
   * a TypeError.
   *
   * WARNING: The PyFrozenSet returned is only intended to be used temporarily
   * (and internally);
   * since its contents are shared with value, it could be mutated!
   *
   * This is better than special-casing behavior based on isinstance, because a
   * Python subclass
   * can override, say, __hash__ and all of a sudden you can't assume that a
   * non-PyFrozenSet is
   * unhashable anymore.
   *
