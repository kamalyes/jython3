...
import static org.kamalyes.jython3.core.CompareOp.*;
 ...
 * such objects.
 *
 * Method names are designed to make it possible for subclasses of PySequence to
 * implement
 * java.util.List.
 *
 * Subclasses must also implement get, getslice, and repeat methods.
 *
 * Subclasses that are mutable should also implement: set, setslice, del, and
 * delRange.
 */
public abstract class PySequence extends PyObject {

  /**
   * A delegate that handles index checking and manipulation for get, set and del
   * operations on
   * this sequence in the form of a "pluggable behaviour". Because different types
   * of sequence
   * exhibit subtly different behaviour, there is scope for subclasses to
  ...
  /**
   * Construct a PySequence for the given sub-type with the default index
   * behaviour.
   *
  ...
  /**
   * Construct a PySequence for the given sub-type with custom index behaviour. In
   * practice,
   * restrictions on the construction of inner classes will mean null has to be
   * passed and the
   ...
   * point called by
   * PySequence in its implementation of {@link #__getitem__} It is guaranteed by
   * PySequence that
   * when it calls <code>pyget(int)</code> the index is within the bounds of the
   * array. Any other
  ...
  /**
   * Returns a (concrete subclass of) PySequence that repeats the given sequence,
   * as in the
  ...
  /**
   * Sets the indexed element of the sequence to the given value. This is an
   * extension point
   * called by PySequence in its implementation of {@link #__setitem__} It is
   * guaranteed by
   * PySequence that when it calls pyset(int) the index is within the bounds of
   * the array. Any
   ...
   * <code>start==stop</code>, it is equivalent to insertion at that point. If the
   * step size is
   * not one, and <code>start!=stop</code>, the slice defines a certain number of
   * elements to be
   * replaced, and the value must be a sequence of exactly that many elements (or
   * convertible to
   * such a sequence).
   *
   * @param start the position of the first element.
  ...
  /**
   * Deletes a simple or extended slice and closes up the gap(s). The slice
   * parameters
   * <code>[start:stop:step]</code> mean what they would in Python, <i>after</i>
   * application of
   * the "end-relative" rules for negative numbers and <code>None</code>. The
   * count <code>n</code>
   * is as supplied by {@link PySlice#indicesEx(int)}. This method is unsafe in
   * that slice
   * parameters are assumed correct.
   *
   * @param start the position of the first element.
   * @param stop  beyond the position of the last element (not necessarily just
   *              beyond).
  ...
  /**
   * Make step a long in case adding the start, stop and step together overflows
   * an int.
   */
   ...
   * <code>index</code> starts
   * off negative, it's treated as an index from the end of the sequence going
   * back to the start.
   */
   ...
   *
   * {0} is the op name. {1} is the left operand type. {2} is the right operand
   * type.
   */
  @Override
  ...
  /**
   * Class defining the default behaviour of sequences with respect to slice
   * assignment, etc.,
