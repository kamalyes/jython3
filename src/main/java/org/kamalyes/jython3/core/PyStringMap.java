...
/**
 * Special fast dict implementation for __dict__ instances. Allows interned
 * String keys in addition
...
public class PyStringMap extends PyObject implements Traverseproc {
   ...
   * @param defaultObj
   *                   the value to return if the key does not exists in the
   *                   mapping.
   */
  ...
  /**
   * Return this[key] if the key exists in the mapping, None is returned
   * otherwise.
   *
   * @param key
   *            the key to lookup in the mapping.
  ...
  /**
   * Return this[key] if the key exist, otherwise insert key with a None value and
   * return None.
   *
   * @param key
   *            the key to lookup in the mapping.
  ...
  /**
   * Return this[key] if the key exist, otherwise insert key with the value of
   * failobj and return
   * failobj
   *
   * @param key
   *                the key to lookup in the mapping.
   * @param failobj
   *                the default value to insert in the mapping if key does not
   *                already exist.
   */
  ...
  /**
   * Return a copy of the mappings list of keys. We have to take in account that
   * we could be
