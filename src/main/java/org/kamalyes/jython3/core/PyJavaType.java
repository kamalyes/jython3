...
public class PyJavaType extends PyType {
  ...
  /**
   * Deprecated methods in java.awt.* that have bean property equivalents we
   * prefer.
   */
  ...
  /**
   * Other Java classes this type has MRO conflicts with. This doesn't matter for
   * Java method
   * resolution, but if Python methods are added to the type, the added methods
   * can't overlap with
   * methods added to any of the types in this set. If this type doesn't have any
   * known conflicts,
   * this is null.
   */
  ...
  // Java types are ok with things being added and removed from their dicts as
  // long as there isn't
  @Override
  ...
  void handleMroError(MROMergeState[] toMerge, List<PyObject> mro) {
    ...
    for (PyJavaType type : conflictedAttributes) {
      ...
      for (String method : type.modified) {
        if (!allModified.add(method)) { // Another type in conflict has this method, possibly fail
          ...
          // the conflict
          // in having duplicate __iter__ added (see getCollectionProxies), while still
          // allowing each
          // path on the inheritance hierarchy to get an __iter__. Annoying but necessary
          // logic.
    ...
    // We can keep trucking, there aren't any existing method name conflicts. Mark
    // the
    // conflicts in all the classes so further method additions can check for
    // trouble
    for (PyJavaType type : conflictedAttributes) {
  ...
  protected void init(Class<?> forClass, Set<PyJavaType> needsInners) {
    ...
    if (name.startsWith("org.kamalyes.jython3.core.Py")) {
      name = name.substring("org.kamalyes.jython3.core.Py".length()).toLowerCase();
    }
    dict = new PyStringMap();
    Class<?> baseClass = forClass.getSuperclass();
    if (PyObject.class.isAssignableFrom(forClass)) {
      // Non-exposed subclasses of PyObject use a simple linear mro to PyObject that
      // ignores
      // their interfaces
      ...
      // Wrapped Java types fill in their mro first using all of their interfaces then
      // their
      // super class.
    ...
    // PyReflected* can't call or access anything from non-public classes that
    // aren't in
    // org.kamalyes.jython3.core
    if (!Modifier.isPublic(forClass.getModifiers()) &&
        !name.startsWith("org.kamalyes.jython3.core") && Options.respectJavaAccessibility) {
    ...
    if (Options.respectJavaAccessibility) {
      ...
      // Grab all methods on this class and all of its superclasses and make them
      // accessible
    ...
    for (Method meth : methods) {
      ...
      // deprecated and interfere too badly with bean properties to be tolerated. This
      // is
      ...
      if (beanPropertyName != null) {
        ...
        if (get) {
          ...
          // TODO: should we issue a warning if setX and getX have different
          // types?
          if (prop.myType == null) {
    ...
    for (Method meth : methods) {
      ...
      if (reflfunc != null) {
        ...
        // flipside of what handleSuperMethodArgCollisions does for inherited public
        // methods
    ...
    for (PyBeanProperty prop : props.values()) {
      ...
      // If one of our superclasses has something defined for this name, check if its
      // a bean
      ...
      if (superForName instanceof PyBeanProperty) {
        ...
        // If it has a set method and we don't, take it regardless. If the types don't
        // line
      ...
      // If the return types on the set and get methods for a property don't agree,
      // the get
    ...
    // No matter the security manager, trying to set the constructor on class to
    // accessible
    // blows up
    ...
    // allow for some methods to override the Java type's methods as a late
    // injection
    ...
    if (forClass == Cloneable.class) {
      addMethod(new PyBuiltinMethodNarrow("__copy__") {
        ...
        public PyObject __call__() {
          ...
          //
          // Lastly we can potentially cache the method handle in the proxy instead of
  ...
  /**
   * Private, protected or package protected classes that implement public
   * interfaces or extend
   * public classes can't have their implementations of the methods of their
   * supertypes called
   * through reflection due to Sun VM bug 4071957(http://tinyurl.com/le9vo). They
   * can be called
   * through the supertype version of the method though. Unfortunately we can't
   * just let normal
   * mro lookup of those methods handle routing the call to the correct version as
   * a class can
   * implement interfaces or classes that each have methods with the same name
   * that takes
   * different number or types of arguments. Instead this method goes through all
   * interfaces
   * implemented by this class, and combines same-named methods into a single
   * PyReflectedFunction.
   *
   * Prior to Jython 2.5, this was handled in PyJavaClass.setMethods by setting
   * methods in package
   * protected classes accessible which made them callable through reflection.
   * That had the
   * drawback of failing when running in a security environment that didn't allow
   * setting
  ...
  private void handleSuperMethodArgCollisions(Class<?> forClass) {
    ...
    if (forClass.getSuperclass() != null) {
      ...
      if (!Modifier.isPublic(forClass.getSuperclass().getModifiers())) {
        // If the superclass is also not public, it needs to get the same treatment as
        // we
  ...
  private void mergeMethods(Class<?> parent) {
    for (Method meth : parent.getMethods()) {
      ...
      if (obj == null) {
        ...
        // There's something in a superclass with the same name. Add an item to this
        // type's
        // dict to hide it. If it's this method, nothing's changed. If it's a field, we
        // want to make the method visible. If it's a different method, it'll be added
        // to
  ...
  /**
   * Build a map of common Java collection base types (Map, Iterable, etc) that
   * need to be
   * injected with Python's equivalent types' builtin methods (__len__, __iter__,
   * iteritems, etc).
   *
   * @return A map whose key is the base Java collection types and whose entry is
   *         a list of
   *         injected methods.
   */
