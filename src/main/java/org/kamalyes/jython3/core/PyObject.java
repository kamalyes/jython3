...
import org.kamalyes.jython3.core.stringlib.IntegerFormatter;
import org.kamalyes.jython3.core.stringlib.InternalFormat;
...
public class PyObject implements Serializable {
  ...
  /**
   * This should have been suited at {@link org.kamalyes.jython3.modules.gc},
   ...
   * {@link org.kamalyes.jython3.modules.gc#MONITOR_GLOBAL}-flag in Jython's
   * gc module.<br>
   * <br>
   ...
   * {@link org.kamalyes.jython3.core.JyAttribute}.
   ...
   *
   * @see org.kamalyes.jython3.core.JyAttribute
   * @see org.kamalyes.jython3.core.JyAttribute#JAVA_PROXY_ATTR
   * @see #getJavaProxy()
   */
  ...
  /**
   * The standard constructor for a <code>PyObject</code>. It will set the
   * <code>objtype</code>
   * field to correspond to the specific subclass of <code>PyObject</code> being
   * instantiated.
   **/
  public PyObject() {
  ...
  /**
   * Creates the PyObject for the base type. The argument only exists to make the
   * constructor
   * distinct.
   */
   ...
   * If a finalizer, a.k.a. {@code __del__} is needed, follow the instructions in
   * the
   * documentation of
   * {@link org.kamalyes.jython3.core.finalization.FinalizablePyObject}.
   * </p>
   * <p>
   ...
   * So {@code PyObject}s are not expensively treated as finalizable objects by
   * the
  ...
  /**
   * Attempts to automatically initialize our Java proxy if we have one and it
   * wasn't initialized
   * by our __init__.
   */
  void proxyInit() {
   ...
   * <code>org.kamalyes.jython3.core.finalization.HasFinalizeTrigger</code>
  ...
  public Object __tojava__(Class<?> c) {
    ...
    // convert faux floats
    // TODO: should also convert faux ints, but that breaks test_java_visibility
  ...
  public PyObject _callextra(PyObject[] args,
    ...
    for (PyObject kwargs : kwargsArray) {
      ...
      for (PyObject key; keysIter.hasNext();) {
        ...
        newkeywords[keywords.length + i++] = ((PyUnicode) key).internedString();
  ...
  /**
   * Return an iterator that is used to iterate the element of this sequence. From
   * version 2.2,
   * this method is the primary protocol for looping over sequences.
   * <p>
   * If a PyObject subclass should support iteration based in the __finditem__()
   * method, it must
   ...
   * </pre>
   *
   * When iterating over a python sequence from java code, it should be done with
   * code like this:
   *
   * <pre>
  ...
  /**
   * Returns an Iterable over the Python iterator returned by __iter__ on this
   * object. If this
   * object doesn't support __iter__, a TypeException will be raised when iterator
   * is called on
   ...
   *                                     is not mandatory, null can be returned if
   *                                     it fits the implementation
  ...
  public void readonlyAttributeError(String name) {
    // TODO::: Should be an AttributeError but CPython throws TypeError for read only
    // member descriptors (in structmember.c::PyMember_SetOne), which is expected by
    // a
   ...
   * @see org.kamalyes.jython3.core.PyObject#__coerce_ex__(org.kamalyes.jython3.core.PyObject)
   **/
  ...
  public final int _cmp(PyObject o) {
    ...
    // }
    //
    // throw Py.TypeError(String.format(UNORDERABLE_ERROR_MSG, getType().getName(),
    // o.getType().getName()));
    // } finally {
  ...
  public PyObject _is(PyObject o) {
    // Access javaProxy directly here as is is for object identity, and at best
    // getJavaProxy
    // will initialize a new object with a different identity
    return this == o || (JyAttribute.hasAttr(this, JyAttribute.JAVA_PROXY_ATTR) &&
        JyAttribute.getAttr(this, JyAttribute.JAVA_PROXY_ATTR) == JyAttribute.getAttr(o, JyAttribute.JAVA_PROXY_ATTR))
            ? Py.True
            : Py.False;
  }
  ...
  private boolean isStrUnicodeSpecialCase(PyType t1, PyType t2, String op) {
    // TODO::: We may need to generalize this rule to apply to other
    // situations
    // TODO::: This method isn't expensive but could (and maybe
  ...
  /**
   * Helper to check for object.__setattr__ or __delattr__ applied to a type (The
   * Carlo
   * Verre hack).
   *
  ...
  /**
   * Convert this object into an index-sized integer. Throws a PyException on
   * failure.
   *
