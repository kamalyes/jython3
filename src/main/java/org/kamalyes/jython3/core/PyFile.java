...
import org.kamalyes.jython3.core.io.BinaryIOWrapper;
import org.kamalyes.jython3.core.io.BufferedIOBase;
import org.kamalyes.jython3.core.io.BufferedRandom;
import org.kamalyes.jython3.core.io.BufferedReader;
import org.kamalyes.jython3.core.io.BufferedWriter;
import org.kamalyes.jython3.core.io.FileIO;
import org.kamalyes.jython3.core.io.IOBase;
import org.kamalyes.jython3.core.io.LineBufferedRandom;
import org.kamalyes.jython3.core.io.LineBufferedWriter;
import org.kamalyes.jython3.core.io.RawIOBase;
import org.kamalyes.jython3.core.io.StreamIO;
import org.kamalyes.jython3.core.io.TextIOBase;
import org.kamalyes.jython3.core.io.TextIOWrapper;
import org.kamalyes.jython3.core.io.UniversalIOWrapper;
...
public class PyFile extends PyObject implements FinalizableBuiltin, Traverseproc {
  ...
  /**
   * Creates a file object wrapping the given <code>InputStream</code>. The
   * builtin
   * method <code>file</code> doesn't expose this functionality (<code>open</code>
   * does
   * albeit deprecated) as it isn't available to regular Python code. To wrap an
   * InputStream in a file from Python, use
   * {@link org.kamalyes.jython3.core.util.FileUtil#wrap(InputStream, String, int)}
   * {@link org.kamalyes.jython3.core.util.FileUtil#wrap(InputStream, String)}
   * {@link org.kamalyes.jython3.core.util.FileUtil#wrap(InputStream, int)}
   * {@link org.kamalyes.jython3.core.util.FileUtil#wrap(InputStream)}
   */
  ...
  /**
   * Creates a file object wrapping the given <code>OutputStream</code>. The
   * builtin
   * method <code>file</code> doesn't expose this functionality (<code>open</code>
   * does
   * albeit deprecated) as it isn't available to regular Python code. To wrap an
   * OutputStream in a file from Python, use
   * {@link org.kamalyes.jython3.core.util.FileUtil#wrap(OutputStream, String, int)}
   * {@link org.kamalyes.jython3.core.util.FileUtil#wrap(OutputStream, String)}
   * {@link org.kamalyes.jython3.core.util.FileUtil#wrap(OutputStream, int)}
   * {@link org.kamalyes.jython3.core.util.FileUtil#wrap(OutputStream)}
   */
  ...
  /**
   * Set the strings defining the encoding and error handling policy. Setting
   * these strings
   * affects behaviour of the {@link #writelines(PyObject)} when passed a
   * {@link PyUnicode} value.
   *
   * @param encoding the <code>encoding</code> property of <code>file</code>.
   * @param errors   the <code>errors</code> property of <code>file</code> (or
   *                 <code>null</code>).
   */
  ...
  /**
   * Parse and validate the python file mode, returning a cleaned file mode
   * suitable for FileIO.
   *
  ...
  @ExposedMethod(names = { "__enter__", "__iter__", "xreadlines" }, doc = BuiltinDocs.TextIOBase___iter___doc)
  ...
  /**
   * Return a String for writing to the underlying file from obj. This is a helper
   * for {@link file_write}
  ...
  private String asWritable(PyObject obj, String message) {
    ...
    if (obj instanceof PyUnicode) {
      // Unicode must be encoded into bytes (null arguments here invoke the default
      // values)
    ...
    if (message == null) {
      // Messages differ for text or binary streams (CPython) but we always add the
      // type
  ...
  /**
   * XXX update docs - A mechanism to make sure PyFiles are closed on exit. On
   * creation Closer adds itself
   * to a list of Closers that will be run by PyFileCloser on JVM shutdown. When a
   * PyFile's close or finalize methods are called, PyFile calls its Closer.close
   * which
   * clears Closer out of the shutdown queue.
   *
   * We use a regular object here rather than WeakReferences and their ilk as they
   * may
   * be collected before the shutdown hook runs. There's no guarantee that
   * finalize will
   * be called during shutdown, so we can't use it. It's vital that this Closer
   * has no
   * reference to the PyFile it's closing so the PyFile remains garbage
   * collectable.
   */
