...
public class PyReflectedConstructor extends PyReflectedFunction {
  ...
  public PyObject __call__(PyObject self, PyObject[] args, String[] keywords) {
    ...
    // If the declaring class is a pure Java type but we're instantiating a Python
    // proxy,
    ...
    if (nkeywords > 0) {
      ...
      // Look for a constructor; the ReflectedArgs#matches() method exits early in the
      // case
    ...
    // Do setattr's for keyword args. This convenience allows Java bean properties
    // to be set in
    // by a Python constructor call.
    // However, this is not done if the Java constructor accepts (PyObject[],
    // String[]) as its arguments,
    // in which case the intention is that the Java constructor will handle the
    // keyword arguments itself.
