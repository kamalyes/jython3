...
import org.kamalyes.jython3.modules._weakref.WeakrefModule;
...
public class PyType extends PyObject implements Serializable, Traverseproc {
  ...
  /**
   * Whether finalization is required for this type's instances (implements
   * __del__).
   */
  ...
  /**
   * Creates the PyType instance for type itself. The argument just exists to make
   * the constructor
   * distinct.
   */
  ...
  static final PyObject type___new__(PyNewWrapper new_, boolean init, PyType subtype,
    ...
    // If that didn't trigger, we need 3 arguments. but ArgParser below may give a
    // msg
  ...
  public static PyObject newType(PyNewWrapper new_, PyType metatype, String name, PyTuple bases,
    ...
    // PyJavaType. Using PyJavaType as metaclass exposes the java.lang.Object
    // methods
    ...
    if (type.qualname != null) {
      if (!(type.qualname instanceof PyUnicode)) {
        throw Py
            .TypeError(String.format("type __qualname__ must be a str, not %s", type.qualname.getType().fastGetName()));
      }
  ...
  private void createWeakrefSlot() {
    ...
    dict.__setitem__("__weakref__", new PyDataDescr(this, "__weakref__", PyObject.class, doc) {
      ...
      @Override
      public void invokeSet(PyObject obj, Object value) {
        // TODO::: Maybe have PyDataDescr do notWritable() for us
        notWritable(obj);
      }
  ...
  /**
   * Called on builtin types for a particular class. Should fill in dict, name,
   * mro, base and
  ...
  protected void init(Class<?> forClass, Set<PyJavaType> needsInners) {
    ...
    if (BootstrapTypesSingleton.getInstance().contains(underlying_class)) {
      // init will be called again from addBuilder which also removes underlying_class
      // from
      // BOOTSTRAP_TYPES
      return;
    }
    ...
    // TODO::: Can't create a __doc__ str until the PyUnicode types are
    // created
    if (dict.__finditem__("__doc__") == null) {
      PyObject docObj;
      if (doc != null) {
        ...
        if (underlying_class == PyUnicode.class)
          docObj.objtype = this;
      } else {
        // TODO::: Hack: Py.None may be null during bootstrapping. Most types
  ...
  /**
   * Fills the base and bases of this type with the type of baseClass as sets its
   * mro to this type
  ...
  /**
   * Ensures that the physical layout between this type and <code>other</code> are
   * compatible.
  ...
  /**
   * Gets the most parent PyType that determines the layout of this type, ie it
   * has slots or an
  ...
  /**
   * Get the most parent Java proxy Class from bases, tallying any encountered
   * Java
   * interfaces.
   *
  ...
  /**
   * Collects the subclasses and current mro of this type in mroCollector. If this
   * type has
   * subclasses C and D, and D has a subclass E current mroCollector will equal
   * [C, C.__mro__, D,
  ...
  /**
   * Returns the Java Class that this type inherits from, or null if this type is
   * Python-only.
   */
  ...
  PyObject[] computeMro(MROMergeState[] toMerge, List<PyObject> mro) {
    ...
    PyType proxyAsType = !JyAttribute.hasAttr(this, JyAttribute.JAVA_PROXY_ATTR) ? null
        : PyType.fromClass(((Class<?>) JyAttribute.getAttr(this, JyAttribute.JAVA_PROXY_ATTR)), false);
    scan: for (int i = 0; i < toMerge.length; i++) {
      ...
      if (!addedProxy && !(this instanceof PyJavaType) && candidate instanceof PyJavaType
          ...
          && JyAttribute.getAttr(candidate, JyAttribute.JAVA_PROXY_ATTR) != JyAttribute.getAttr(this,
        ...
        // to
        // those generated for the superclass while allowing methods from the superclass
        // to
  ...
  /**
   * Must either throw an exception, or bring the merges in <code>toMerge</code>
   * to completion by
  ...
  /**
   * Finds the parent of type with an underlying_class or with slots sans a
   * __dict__
   * slot.
   */
  ...
  /**
   * Finds the base in bases with the most derived solid_base, ie the most base
   * type
   *
  ...
  /**
   * Attribute lookup for name directly through mro objects' dicts. This isn't
   * cached,
  ...
  /**
   * Attribute lookup for name through mro objects' dicts. This isn't cached, and
   * should
  ...
  public synchronized static void addBuilder(Class<?> forClass, TypeBuilder builder) {
    ...
    if (class_to_type.containsKey(forClass)) {
      ...
      // The types in BootstrapTypesSingleton.getInstance() are initialized before
      // their builders are assigned,
  ...
  private static TypeBuilder getBuilder(Class<?> c) {
    if (classToBuilder == null) {
      // PyType itself has yet to be initialized. This should be a bootstrap type, so
      // it'll
      // go through the builder process in a second
      return null;
    }
    if (c.isPrimitive() || !PyObject.class.isAssignableFrom(c)) {
      // If this isn't a PyObject, don't bother forcing it to be initialized to load
      // its
      // builder
      return null;
    }

    // This is a PyObject, call forName to force static initialization on the class
    // so if it has
  ...
  // so we just need to prevent data races. all public methods that access
  // class_to_type
  ...
  public static synchronized PyType fromClass(Class<?> c, boolean hardRef) {
    ...
    // We haven't seen this class before, so its type needs to be created. If it's
    // being
    // exposed as a Java class, defer processing its inner types until it's
    // completely
    // created in case the inner class references a class that references this
    // class.
  ...
  /**
   * Returns true if the given attribute retrieved from an object's metatype
  ...
  /**
   * Invalidate this type's MethodCache entries. *Must* be called after any
   * modification
  ...
  final PyObject type___call__(PyObject[] args, String[] keywords) {
    ...
    // When the call was type(something) or the returned object is not an instance
    // of
  ...
  /**
   * Returns the actual dict underlying this type instance. Changes to Java types
   * should go
   * through {@link #addMethod} and {@link #removeMethod}, or unexpected mro
   * errors can occur.
   */
  ...
  /**
   * Equivalent of CPython's typeobject.c::type_get_doc; handles __doc__
   * descriptors.
   */
  ...
  // TODO::: consider pulling this out into a generally accessible place
  ...
  private static String confirmIdentifier(PyObject obj) {
    ...
    // throw Py.TypeError(String.format("__slots__ items must be strings, not
    // '%.200s'",
    // obj.getType().fastGetName()));
    // } else
  ...
  // TODO::: copied from CodeCompiler.java and changed variable names.
  ...
  static class TypeResolver implements Serializable {
    ...
    TypeResolver(Class<?> underlying_class, String module, String name) {
      // Don't store the underlying_class for PyProxies as the proxy type needs to
      // fill in
  ...
  /**
   * Tracks the status of merging a single base into a subclass' mro in
   * computeMro.
   */
  static class MROMergeState {
    ...
    /**
     * The index of the next item to be merged from mro, or mro.length if this base
     * has been
     * completely merged.
     */
    public int next;
    ...
    /**
     * Returns true if candidate is in the items past this state's next item to be
     * merged.
     */
  ...
  static class MethodCache {
     ...
     *
     * Since the cache can keep references to names alive longer than usual, it
     * avoids
