...
final class StringFormatter {
  ...
  /**
   * Index into args of argument currently being worked, or special values
   * indicating -1: a single
   * item that has not yet been used, -2: a single item that has already been
   * used, -3: a mapping.
   */
  int argIndex;
  ...
  /**
   * Initialise the interpreter with the given format string, ready for
   * {@link #format(PyObject)}.
   *
   ...
   *
   * @param format          string to interpret
   * @param unicodeCoercion to indicate a <code>PyUnicode</code> result is
   *                        expected
   */
  ...
  /**
   * Read the next object from the argument list, taking special values of
   * <code>argIndex</code>
   * into account.
   */
  PyObject getarg() {
  ...
  /**
   * Parse a number from the format, except if the next thing is "*", read it from
   * the argument
   * list.
   */
  int getNumber() {
  ...
  /**
   * Return the argument as either a {@link PyInteger} or a {@link PyLong}
   * according to its
   * <code>__int__</code> method, or its <code>__int__</code> method. If the
   * argument has neither
   * method, or both raise an exception, we return the argument itself. The caller
   * must check the
   * return type.
   *
  ...
  /**
   * Return the argument as a {@link PyFloat} according to its
   * <code>__float__</code> method. If
   * the argument has no such method, or it raises an exception, we return the
   * argument itself.
  ...
  private PyObject asFloat(PyObject arg) {
    ...
    if (arg instanceof PyFloat) {
      ...
      if (arg.getClass() == PyFloat.class) {
        ...
        /*
         * In general, we can't simply call arg.__float__() because PyBytes implements
         * it
  ...
  /**
   * Main service of this class: format one or more arguments with the format
   * string supplied at
   * construction.
   *
  ...
  public PyObject format(PyObject args) {
    ...
    if (args instanceof PyTuple) {
      ...
      // Not a tuple, but possibly still some kind of container: use special argIndex
      // values.
      argIndex = -1;
    ...
    while (index < format.length()) {
      ...
      /*
       * Minimum field width (optional). If specified as an '*' (asterisk), the actual
       * width
       * is read from the next element of the tuple in values, and the object to
       * convert comes
       * after the minimum field width and optional precision. A custom getNumber()
       * takes care
       * of the '*' case.
       */
      width = getNumber();
       ...
       * specified as
       * '*' (an asterisk), the actual precision is read from the next element of the
       * tuple in
       * values, and the value to convert comes after the precision. A custom
       * getNumber()
      ...
      /*
       * As a function of the conversion type (currently in c) override some of the
       * formatting
      ...
      /*
       * Encode as an InternalFormat.Spec. The values in the constructor always have
       * specified
      ...
      /*
       * Process argument according to format specification decoded from the string.
       * It is
       * important we don't read the argument from the list until this point because
       * of the
    ...
    /*
     * All fields in the format string have been used to convert arguments (or used
     * the argument
     * as a width, etc.). This had better not leave any arguments unused. Note
     * argIndex is an
     * index into args or has a special value. If args is a 'proper' index, It
     * should now be out
     * of range; if a special value, it would be wrong if it were -1, indicating a
     * single item
