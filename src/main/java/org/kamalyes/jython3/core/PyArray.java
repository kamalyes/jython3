...
import org.kamalyes.jython3.core.util.ByteSwapper;
...
import org.kamalyes.jython3.modules.gc;
...
public class PyArray extends PySequence implements Cloneable, BufferProtocol, Traverseproc {
  ...
  static final PyObject array_new(PyNewWrapper new_, boolean init, PyType subtype,
    ...
    /*
     * The initialiser may be omitted, or may validly be one of several types in the
     * broad
     * categories of a byte string (which is treated as a machine representation of
     * the data) or
     * an iterable yielding values assignable to the elements. There is special
     * treatment for
     * type 'u' Unicode.
     */
   ...
   * <code>init</code>.
   *
   * @param init  an initialiser for the array - can be PyBytes or PySequence
   *              (including PyArray)
   *              or iterable type.
  ...
  /**
   * Common helper method used internally to append a new value x to the end of
   * the array:
   ...
   * are 1, 2, 4, or 8
   * bytes in size; for other types of values, RuntimeError is raised. It is
   * useful when reading
  ...
  /**
   * Delete the slice defined by <code>start</code> to <code>stop</code> from the
   * array.
   *
  ...
  /**
   * Append items from <code>iterable</code> to the end of the array. If iterable
   * is another
   * array, it must have exactly the same type code; if not, TypeError will be
   * raised. If iterable
   * is not an array, it must be iterable and its elements must be the right type
   * to be appended
   * to the array.
   *
   ...
   * Handles specific
   * cases of <code>iterable</code> being PyStrings or PyArrays. Default behaviour
   * is to defer to
   * {@link #extendInternalIter(PyObject) extendInternalIter }
   *
   * @param iterable object of type PyBytes, PyArray or any object that can be
   *                 iterated over.
   */
  ...
  /**
   * Helper used only when the array elements are Unicode characters
   * (<code>typecode=='u'</code>).
   * (Characters are stored as integer point codes.) The parameter must be an
   * iterable yielding
   * <code>PyUnicode</code>s. Often this will be an instance of {@link PyUnicode},
   * which is an
   * iterable yielding single-character <code>PyUnicode</code>s. But it is also
   * acceptable to this
   * method for the argument to yield arbitrary <code>PyUnicode</code>s, which
   * will be
  ...
  /**
   * Read <code>count</code> items (as machine values) from the file object
   * <code>f</code> and
   * append them to the end of the array. If less than <code>count</code> items
   * are available,
   * EOFError is raised, but the items that were available are still inserted into
   * the array.
   * <code>f</code> must be a real built-in file object; something else with a
   * read() method won't
   * do.
   *
  ...
  public void fromfile(PyObject f, int count) {
    /*
     * Prohibit when exporting a buffer. Different from CPython, BufferError takes
     * precedence in
  ...
  /**
   * Generic stream reader to read <code>count</code> primitive types from a
   * stream into the
   * array.
   *
   * @param is    InputStream to source the data from
  ...
  private int fromStream(InputStream is, int count) throws IOException, EOFException {
    ...
    // Read into the array, after the current contents, up to new size (or EOF
    // thrown)
  ...
  /**
   * Read primitive values from a stream into the array without resizing. Data is
   * read until the
   * array is filled or the stream runs out. If the stream does not contain a
   * whole number of
   * items (possible if the item size is not one byte), the behaviour in respect
   * of the final
  ...
  /**
   * Helper for reading primitive values from a stream into a slice of the array.
   * Data is read
   * until the array slice is filled or the stream runs out. The purpose of the
   * method is to
   * concentrate in one place the manipulation of bytes into the several primitive
   * element types
   * on behalf of {@link #fillFromStream(InputStream)} etc.. The storage is
   * resized if the slice
   * being written ends beyond the current end of the array, i.e. it is increased
   * to the value of
   * <code>limit</code>.
   * <p>
   * Since different read methods respond differently to it, the caller must
   * specify whether the
   * exhaustion of the stream (EOF) should be treated as an error or not. If the
   * stream does not
   * contain a whole number of items (possible if the item size is not one byte),
   * the behaviour in
   ...
   * @return index of first element not read (<code>=limit</code>, if not ended by
   *         EOF)
   * @throws IOException  reflecting I/O errors during reading
   * @throws EOFException if stream ends before read is satisfied and eofIsError
   *                      is true
   */
  private int fromStream(InputStream is, int index, int limit, boolean eofIsError)
    ...
    // We need a wrapper capable of decoding the data from the representation
    // defined by Java.
  ...
  /**
   * Appends items from the object, which is a byte string of some kind (PyBytes
   * or object with
   * the buffer interface providing bytes) The string of bytes is interpreted as
   * an array of
   * machine values (as if it had been read from a file using the
   * {@link #fromfile(PyObject, int)
   * fromfile()} method).
   *
   ...
   * values (as if
   * it had been read from a file using the {@link #fromfile(PyObject, int)
   * fromfile()} method).
   *
   * @param input string of bytes containing array data
   */
  ...
  /**
   * Copy into this array, the remaining bytes of a ByteBuffer (from the current
   * position to the
   * limit). This is common code supporting Java and Python versions of
   ...
   * <p>
   * The sizes returned by this method represent the number of bytes used to store
   * the type. In
   * the case of streams, this is the number of bytes written to, or read from a
   * stream. For
   * memory this value is the <em>minimum</em> number of bytes required to store
   * the type.
   * <p>
   * This method is used by other methods to define read/write quanta from strings
   * and streams.
   * <p>
  ...
  /**
   * Retrieve a slice from the array specified by the <code>start</code>,
   * <code>stop</code> and
   * <code>step</code>.
   *
  ...
  /**
   * Getter for the type code of the array. {@link #char2class(char) char2class}
   * describes the
   ...
   * <code>index</code>. Negative values are treated as being relative to the end
   * of the array.
   *
  ...
  /**
   * Removes the item at the index <code>i</code> from the array and returns it.
   * The optional
  ...
  /**
   * Removes the item with the index <code>index</code> from the array and returns
   * it.
   *
  ...
  /**
   * Set an element in the array - the index needs to exist, this method does not
   * automatically
   * extend the array. See {@link AbstractArray#setSize(int)
   * AbstractArray.setSize()} or
   * {@link AbstractArray#ensureCapacity(int) AbstractArray.ensureCapacity()} for
   * ways to extend
   * capacity.
   * <p>
  ...
  // xxx - add more efficient comparable typecode lookup via an enumset, and
  // expand
  ...
  /**
   * Sets a slice of the array. <code>value</code> can be a string (for
   * <code>byte</code> and
   * <code>char</code> types) or PyArray. If a PyArray, its type must be
   * convertible into the type
  ...
  /**
   * Generic stream writer to write the entire contents of the array to the stream
   * as primitive
   * types.
   *
   ...
   *
   * This is a close duplicate of the same mechanism in PyByteArray. There is
   * perhaps scope for a
   * shared helper class to implement this logic. For type code 'b', the workings
   * are almost
   * identical. The fully-fledged buffer interface for PyArray is richer, more
   * like the Python 3
   * memoryview, as it must cope with items of size other than one byte. This goes
   * beyond the
  ...
  /**
   * Test to see if the array may be resized and raise a BufferError if not. This
   * must be called
   * by the implementation of any operation that changes the number of elements in
   * the array.
   *
