...
public class FinalizeTrigger {
   ...
   * {@link org.kamalyes.jython3.modules.gc} that the underlying object was
   * finalized.
  ...
  /**
   * Recreates the {@code FinalizeTrigger} of the given object. This makes sure
   * that
   * once the resurrected object is gc'ed again, its {@code __del__}-method will
   * be
   * called again.
   */
  ...
  protected boolean isCyclic() {
    gc.CycleMarkAttr cm = (gc.CycleMarkAttr) JyAttribute.getAttr(toFinalize, JyAttribute.GC_CYCLE_MARK_ATTR);
    if (cm != null && cm.isCyclic()) {
      return true;
    } else {
      gc.markCyclicObjects(toFinalize, (flags & NOT_FINALIZABLE_FLAG) == 0);
      cm = (gc.CycleMarkAttr) JyAttribute.getAttr(toFinalize, JyAttribute.GC_CYCLE_MARK_ATTR);
