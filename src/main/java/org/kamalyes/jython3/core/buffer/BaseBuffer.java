package org.kamalyes.jython3.core.buffer;
...
/**
 * Base implementation of the Buffer API providing variables and accessors for
 * the navigational
 * arrays, methods for expressing and checking the buffer request flags, methods
 * and mechanism for
 * get-release counting, boilerplate error checks and their associated
 * Exceptions, and default
 * implementations of some methods for access to the buffer content. The design
 * aim is to ensure
 * unglamorous common code need only be implemented once.
 * <p>
 * Where provided, the buffer access methods are appropriate to 1-dimensional
 * arrays where the units
 * are single bytes, stored contiguously. Sub-classes that deal with
 * N-dimensional arrays,
 * non-contiguous storage and items that are not single bytes must override the
 * default
 * implementations.
 * <p>
 * This base implementation is writable only if {@link PyBUF#WRITABLE} is in the
 * feature flags
 * passed to the constructor. Otherwise, all methods for write access raise a
 * <code>BufferError</code> read-only exception and {@link #isReadonly()}
 * returns <code>true</code>.
 * Sub-classes can follow the same pattern, setting {@link PyBUF#WRITABLE} in
 * the constructor and,
 * if they have to, overriding the operations that write (<code>storeAt</code>
 * and
 ...
 * </pre>
 * 
 * Another approach, used in the standard library, is to have distinct classes
 * for the writable and
 * read-only variants. The implementors of simple buffers will find it efficient
 * to override the
 * generic access methods to which performance might be sensitive, with a
 * calculation specific to
 * their actual type.
 * <p>
 * At the time of writing, only one-dimensional buffers of item size one are
 * used in the Jython
 * core.
 */
public abstract class BaseBuffer implements PyBuffer {
  ...
  /**
   * The dimensions of the array represented by the buffer. The length of the
   * <code>shape</code>
   * array is the number of dimensions. The <code>shape</code> array should always
   * be created and
   * filled (difference from CPython). This value is returned by
   * {@link #getShape()}.
   */
  protected int[] shape;

  /**
   * Step sizes in the underlying buffer essential to correct translation of an
   * index (or indices)
   * into an index into the storage. The <code>strides</code> array should always
   * be created and
   * correctly filled to at least the length of the <code>shape</code> array
   * (difference from
  ...
  /**
   * Reference to the underlying <code>byte[]</code> storage that the exporter is
   * sharing with the
   * consumer. The data need not occupy the whole array: in the constructor of a
   * particular type
   * of buffer, the exporter usually indicates an offset to the first significant
   * byte and length
   * (contiguous cases) or the index in <code>storage</code> that should be
   * treated as the item
  ...
  /**
   * Absolute index in <code>storage</code> of <code>item[0]</code>. In one
   * dimension, for a
   * positive <code>stride</code> this is equal to the offset of the first byte
   * used in
   * {@link #storage}, and for a negative <code>stride</code> it is the last. In
   * an N-dimensional
  ...
  /**
   * Count the number of times {@link #release()} must be called before actual
   * release actions
   * need to take place. Equivalently, this is the number of calls to
   * {@link BufferProtocol#getBuffer(int)} that have returned this object: one for
   * the call on the
   * original exporting object that constructed <code>this</code>, and one for
   * each subsequent
  ...
  /**
   * Bit pattern using the constants defined in {@link PyBUF} that records the
   * actual features
   * this buffer offers. When checking consumer flags against the features of the
   * buffer, it is an
   * error if the consumer requests a capability the buffer does not offer, and it
   * is an error if
   * the consumer does not specify that it will use a navigation array the buffer
   * requires.
   * <p>
   * In order to support efficient checking with {@link #checkRequestFlags(int)}
   * we store a
   * mutilated version of the apparent <code>featureFlags</code> in which the
   * non-navigational
   * flags are inverted. The syndrome <code>S</code> of the error is computed as
   * follows. Let
   * <code>N=1</code> where we are dealing with a navigation flag, let
   * <code>F</code> be a buffer
   ...
   * </pre>
   *
   * In the above, <code>A=0</code> only if all the navigation flags set in
   * <code>F</code> are
   * also set in <code>X</code>, and <code>B=0</code> only if all the
   * non-navigation flags clear
   * in <code>F</code> are also clear in <code>X</code>. <code>S=0</code> only if
   * both these
   * conditions are true and furthermore the positions of the <code>1</code>s in
   * the syndrome
   * <code>S</code> tell us which bits in <code>X</code> are at fault. Now if we
   * define:
   ...
   * </pre>
   *
   * Which permits the check in one XOR and one AND operation instead of four ANDs
   * and an OR. The
   * down-side is that we have to provide methods for setting and getting the
   * actual flags in
   * terms a client might expect them to be expressed. We can recover the original
   * <code>F</code>
   * since:
   *
   * <pre>
   ...
   * 'feature
   * flags', or at least a starting set to be adjusted later. These are the
   * features of the buffer
   * exported, not the flags that form the consumer's request. The buffer will be
   * read-only unless
   * {@link PyBUF#WRITABLE} is set in the feature flags. {@link PyBUF#FORMAT} and
   * {@link PyBUF#AS_ARRAY} are implicitly added to the feature flags. The
   * navigation arrays are
   * all null, awaiting action by the sub-class constructor. To complete
   * initialisation, the
   * sub-class normally must assign: the buffer ( {@link #storage},
   * {@link #index0}), and the
   * navigation arrays ({@link #shape}, {@link #strides}), and call
   * {@link #checkRequestFlags(int)} passing the consumer's request flags.
   *
   * @param featureFlags bit pattern that specifies the actual features
   *                     allowed/required
   */
   ...
   * {@link PyBUF},
   * replacing any previous set. Set individual flags or add to those already set
   * by using
  ...
  /**
   * General purpose method to check the consumer request flags (typically the
   * argument to
   * {@link BufferProtocol#getBuffer(int)}) against the feature flags (see
   * {@link #getFeatureFlags()}) that characterise the features of the buffer, and
   * to raise an
   * exception (Python <code>BufferError</code>) with an appropriate message in
   * the case of a
   * mismatch. The flags are defined in the interface {@link PyBUF} and are used
   * in two ways.
   * <p>
   * In a subset of the flags, the consumer specifies assumptions it makes about
   * the index order
   * (contiguity) of the buffer, and whether it is writable. When the buffer
   * implementation calls
   * this check method, it has already specified in {@link #setFeatureFlags(int)}
   * what
   * capabilities this type (or instance) buffer actually has. It is an error, for
   * the consumer to
   * specify in its request a feature that the buffer does not offer.
   * <p>
   * In a subset of the flags, the consumer specifies the set of navigational
   * arrays (
   * <code>shape</code>, <code>strides</code>, and <code>suboffsets</code>) it
   * intends to use in
   * navigating the buffer. When the buffer implementation calls this check
   * method, it has already
   * specified in {@link #setFeatureFlags(int)} what navigation is necessary for
   * the consumer to
   * make sense of the buffer. It is an error for the consumer <i>not to
   * specify</i> the flag
   ...
   * @throws PyException (BufferError) when expectations do not correspond with
   *                     the buffer
   */
  protected void checkRequestFlags(int flags) throws PyException {
    /*
     * It is an error if any of the navigation flags is 0 when it should be 1, or if
     * any of the
  ...
  @Override
  public int[] getShape() {
    // Difference from CPython: never null, even when the consumer doesn't request
    // it.
    return shape;
  }
   ...
   * {@inheritDoc}
   * <p>
   * The default implementation in <code>BaseBuffer</code> deals with the general
   * one-dimensional
  ...
  /**
   * Convert an item index (for a one-dimensional buffer) to an absolute byte
   * index in the actual
   * storage being shared by the exporter. See {@link #calcIndex(int...)} for
   * discussion.
   *
  ...
  /**
   * Convert a multi-dimensional item index (if we are not using indirection) to
   * an absolute byte
   * index in the actual storage array being shared by the exporter. The purpose
   * of this method is
   * to allow a sub-class to define, in one place, an indexing calculation that
   * maps the index as
   * provided by the consumer into an index in the storage known to the buffer.
   * <p>
   * In the usual case where the storage is referenced via the {@link #storage}
   * and
   * {@link #index0} members, the buffer implementation may use
   * <code>storage[calcIndex(i)]</code>
   * to reference the (first byte of) the item x[i]. This is what the default
   * implementation of
   * accessors in <code>BaseBuffer</code> will do. In the simplest cases, calling
   * <code>calcIndex</code> may be an overhead to avoid, and an implementation
   * will specialise the
  ...
  /**
   * Calculate the absolute byte index in the storage array of the last item of
   * the exported data
   * (if we are not using indirection). This is the greatest value attained by
   * {@link #calcIndex(int...)}. The first byte not used will be one
   * <code>itemsize</code> more
  ...
  /**
   * Calculate the absolute byte index in the storage array of the first item of
   * the exported data
  ...
  public void copyTo(int srcIndex, byte[] dest, int destPos, int length)
    ...
    // Strategy depends on whether items are laid end-to-end contiguously or there
    // are gaps
    if (skip == 0) {
  ...
  /**
   * Allow an exporter to re-use a BaseBytes even if it has been "finally"
   * released. Many
   * sub-classes of <code>BaseBytes</code> can be re-used even after a final
   * release by consumers,
   * simply by incrementing the <code>exports</code> count again: the navigation
   * arrays and the
   * buffer view of the exporter's state all remain valid. We do not let consumers
   * do this through
   * the {@link PyBuffer} interface: from their perspective, calling
   * {@link PyBuffer#release()}
   * should mean the end of their access, although we can't stop them holding a
   * reference to the
   * PyBuffer. Only the exporting object, which handles the implementation type is
   * trusted to know
   * when re-use is safe.
   * <p>
   * An exporter will use this method as part of its implementation of
   * {@link BufferProtocol#getBuffer(int)}. On return from that, the buffer <i>and
   * the exporting
   * object</i> must then be in effectively the same state as if the buffer had
   * just been
   * constructed by that method. Exporters that destroy related resources on final
   * release of
   * their buffer (by overriding {@link #releaseAction()}), or permit themselves
   * structural change
   * invalidating the buffer, must either reconstruct the missing resources or
   * avoid
   * <code>getBufferAgain</code>.
   */
  public synchronized BaseBuffer getBufferAgain(int flags) {
    // If only the request flags are correct for this type, we can re-use this
    // buffer
   ...
   * {@inheritDoc}
   * <p>
   * When the final matching release occurs (that is the number of
   * <code>release</code> calls
   * equals the number of <code>getBuffer</code> calls), the implementation here
   * calls
   * {@link #releaseAction()}, which the implementer of a specific buffer type
   * should override if
  ...
  // @Override public PyBuffer getBufferSlice(int flags, int start, int length,
  // int stride) {}
  ...
  /**
   * This method will be called when the number of calls to {@link #release()} on
   * this buffer is
   * equal to the number of calls to {@link PyBuffer#getBuffer(int)} and to
   * {@link BufferProtocol#getBuffer(int)} that returned this buffer. The default
   * implementation
   * does nothing. Override this method to add release behaviour specific to an
   * exporter. A common
   * convention is to do this within the definition of
   * {@link BufferProtocol#getBuffer(int)}
  ...
  /**
   * Some <code>PyBuffer</code>s, those created by slicing a <code>PyBuffer</code>
   * are related to
   * a root <code>PyBuffer</code>. During creation of such a slice, we need to
   * supply a value for
   * this root. If the present object is not itself a slice, this is root is the
   * object itself; if
   * the buffer is already a slice, it is the root it was given at creation time.
   * Often this is
   * the only difference between a slice-view and a directly-exported buffer.
  ...
  /**
   * Check the number of indices (but not their values), raising a Python
   * BufferError if this does
   * not match the number of dimensions. This is a helper for N-dimensional
   * arrays.
   *
  ...
  /**
   * Check that the number offered is in fact the number of dimensions in this
   * buffer, raising a
   * Python BufferError if this does not match the number of dimensions. This is a
   * helper for
   * N-dimensional arrays.
   *
  ...
  /**
   * The toString() method of a buffer reproduces the values in the buffer (as
   * unsigned integers)
  ...
  /**
   * General purpose method to construct an exception to throw according to the
   * syndrome.
   *
