...
public class SimpleBuffer extends BaseBuffer {
  ...
  /**
   * The strides array for this type is always a single element array with a 1 in
   * it.
   */
  protected static final int[] SIMPLE_STRIDES = { 1 };

  /**
   * Provide an instance of <code>SimpleBuffer</code> with navigation variables
   * partly
   * initialised, for sub-class use. One-dimensional arrays without strides are C-
   * and
   * F-contiguous. To complete initialisation, the sub-class must normally assign
   * the buffer (
   * {@link #storage}, {@link #index0}), and the navigation ({@link #shape}
   * array), and then call
   ...
   * Provide an instance of <code>SimpleBuffer</code> with navigation variables
   * initialised, for
   * sub-class use. The buffer ({@link #storage}, {@link #index0}), and the
   * {@link #shape} array
   * will be initialised from the arguments (which are checked for range). The
   * {@link #strides} is
   * set for (one-byte) unit stride. Only the call to
   * {@link #checkRequestFlags(int)}, passing the
   ...
   * </pre>
   *
   * @param storage the array of bytes storing the implementation of the exporting
   *                object
   ...
   * @throws ArrayIndexOutOfBoundsException if <code>index0</code> and
   *                                        <code>size</code> are
   *                                        inconsistent with
  ...
  public SimpleBuffer(byte[] storage, int index0, int size) throws PyException,
    ...
    this();
    // FIXME(isaiah) somewhere the messed up the size calculation, the last
    // parameter is always confusing in similar
  ...
  /**
   * Provide an instance of <code>SimpleBuffer</code>, on a slice of a byte array,
   * meeting the
   * consumer's expectations as expressed in the <code>flags</code> argument,
   * which is checked
  ...
  /**
   * Provide an instance of <code>SimpleBuffer</code>, on the entirety of a byte
   * array, with
   * navigation variables initialised, for sub-class use. The buffer (
   * {@link #storage},
   * {@link #index0}), and the navigation ({@link #shape} array) will be
   ...
   * Provide an instance of <code>SimpleBuffer</code>, on the entirety of a byte
   * array, meeting
   * the consumer's expectations as expressed in the <code>flags</code> argument,
   * which is checked
   * against the capabilities of the buffer type.
   *
   * @param flags   consumer requirements
   ...
   * {@inheritDoc}
   * <p>
   * <code>SimpleBuffer</code> provides an implementation optimised for contiguous
   * bytes in
   * one-dimension.
   */
  @Override
   ...
   * {@inheritDoc}
   * <p>
   * <code>SimpleBuffer</code> provides an implementation for slicing contiguous
   * bytes in one
   * dimension. In that case, <i>x(i) = u(r+i)</i> for <i>i = 0..L-1</i> where u
   * is the underlying
   * buffer, and <i>r</i> and <i>L</i> are the start and length with which
   * <i>x</i> was created
   * from <i>u</i>. Thus <i>y(k) = u(r+s+km)</i>, that is, the composite offset is
   * <i>r+s</i> and
   * the stride is <i>m</i>.
   */
  @Override
  ...
  /**
   * A <code>SimpleBuffer.SimpleView</code> represents a contiguous subsequence of
   * another
