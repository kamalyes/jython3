...
/**
 * Read-write buffer API over a one-dimensional array of one-byte items, that
 * are evenly-spaced in a
 * storage array. The storage conventions are described in
 * {@link Strided1DBuffer} of which this is
 * an extension providing write operations and a writable slice.
 */
public class Strided1DWritableBuffer extends Strided1DBuffer {

  /**
   * Provide an instance of <code>Strided1DWritableBuffer</code> on a particular
   * array of bytes
   * specifying a starting index, the number of items in the result, and a
   * byte-indexing stride.
   * The result of <code>byteAt(i)</code> will be equal to
   * <code>storage[index0+stride*i]</code>
   * (whatever the sign of <code>stride>0</code>), valid for
   ...
   * <code>stride</code>. The caller is responsible for checking these fall within
   * the array, or
  ...
  /**
   * {@inheritDoc} <code>Strided1DWritableBuffer</code> provides a version
   * optimised for strided
   ...
   * {@inheritDoc}
   * <p>
   * <code>Strided1DWritableBuffer</code> provides an implementation that returns
   * a writable
   * slice.
   */
  @Override
  ...
  /**
   * A <code>Strided1DWritableBuffer.SlicedView</code> represents a non-contiguous
   * subsequence of
   * a simple buffer.
   */
