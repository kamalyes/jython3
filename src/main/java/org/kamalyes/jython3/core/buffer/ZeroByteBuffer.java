...
/**
 * Buffer API over a zero length, one-dimensional array of one-byte items. The
 * buffer is nominally
 * writable, but since there is nowhere to write to, any attempt to write or
 * read throws an
 * <code>IndexOutOfBoundsException</code>. This class exists mostly to represent
 * zero-length arrays,
 * and particularly, zero-length slices for which implementations of
 * {@link PyBuffer#getBufferSlice(int, int, int, int)} in any case need special
 * logic. Bulk
 * operations like {@link #copyTo(byte[], int)}) and {@link #toString()}
...
public class ZeroByteBuffer extends BaseBuffer {
  ...
  /**
   * Construct an instance of a zero-length buffer, choosing whether it should
   * report itself to be
   * read-only through {@link #isReadonly()}. This is moot, as any attempt to
   * write to it produces
   * an {@link IndexOutOfBoundsException}, but it is less surprising for client
   * code that may ask,
   * if the readability follows that of the object from which the buffer is
   * derived.
   *
  ...
  /**
   * In a ZeroByteBuffer, if the dimensions are right, the index is out of bounds
   * anyway.
   */
  @Override
  protected int calcIndex(int... indices) throws IndexOutOfBoundsException {
    // Bootless dimension check takes precedence (for consistency with other
    // buffers)
  ...
  /**
   * In a ZeroByteBuffer, there is no room for anything, so this throws unless the
   * source length
   * is zero.
   */
  @Override
  ...
  /**
   * Only a zero-length slice at zero is valid (in which case, the present buffer
   * will do nicely
   ...
   * {@inheritDoc}
   * <p>
   * The implementation in <code>ZeroByteBuffer</code> efficiently returns an
   * empty buffer.
   */
  @Override
  ...
  /**
   * A <code>ZeroByteBuffer.View</code> represents a contiguous subsequence of
   * another
   * <code>PyBuffer</code>. We don't need it to make slices of the ZeroByteBuffer
   * itself, but it
   * is useful for making zero-length slices of anything else. Lock-release
   * semantics must still
   * be observed. In Python, a zero-length slice obtained from the memoryview of a
   * bytearray still
  ...
  static class View extends ZeroByteBuffer {
    ...
    /**
     * Construct a slice of a ZeroByteBuffer, which it goes without saying is of
     * zero length at
     * position zero.
     *
