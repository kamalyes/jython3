...
/**
 * Buffer API that appears to be a one-dimensional array of one-byte items
 * providing read-only API,
 * but which is actually backed by a Java String. Some of the buffer API
 * absolutely needs access to
 * the data as a byte array (those parts that involve a
 * <code>PyBuffer.Pointer</code> result), and
 * therefore this class must create a byte array from the String for them.
 * However, it defers
 * creation of a byte array until that part of the API is actually used. Where
 * possible, this class
 * overrides those methods in SimpleBuffer that would otherwise access the byte
 * array attribute to
...
public class SimpleStringBuffer extends SimpleBuffer {
  ...
  /**
   * The string backing this PyBuffer. A substitute for {@link #buf} until we can
   * no longer avoid
   * creating it.
   */
  ...
  /**
   * Provide an instance of SimpleStringBuffer meeting the consumer's expectations
   * as expressed in
   * the flags argument.
   *
   ...
   * {@inheritDoc}
   * <p>
   * This method uses {@link String#length()} rather than create an actual byte
   * buffer.
   */
  @Override
  public int getLen() {
   ...
   * {@inheritDoc}
   * <p>
   * This method uses {@link String#charAt(int)} rather than create an actual byte
   * buffer.
   */
  @Override
   ...
   * {@inheritDoc}
   * <p>
   * The <code>SimpleStringBuffer</code> implementation avoids creation of a byte
   * buffer.
   */
  @Override
  public PyBuffer getBufferSlice(int flags, int start, int length) {
    if (length > 0) {
      ...
      // Special case for length==0 where start out of bounds sometimes raises
      // exception.
   ...
   * {@inheritDoc}
   * <p>
   * The <code>SimpleStringBuffer</code> implementation creates an actual byte
   * buffer.
   */
  @Override
  ...
  /**
   * This method creates an actual byte array from the underlying String if none
   * yet exists.
   */
  ...
  /**
   * The <code>toString()</code> method of a <code>SimpleStringBuffer</code>
   * simply produces the
  ...
  /**
   * A <code>SimpleStringBuffer.SimpleStringView</code> represents a contiguous
   * subsequence of
