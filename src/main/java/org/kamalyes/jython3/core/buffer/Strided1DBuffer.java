...
/**
 * Read-only buffer API over a one-dimensional array of one-byte items, that are
 * evenly-spaced in a
 * storage array. The buffer has <code>storage</code>, <code>index0</code> and
 * <code>length</code>
 * properties in the usual way, designating a slice (or all) of a byte array,
 * but also a
 * <code>stride</code> property (equal to <code>getStrides()[0]</code>).
 * <p>
 * Let the underlying buffer be the byte array <i>u(i)</i> for <i>i=0..N-1</i>,
 * let <i>x</i> be the
 * <code>Strided1DBuffer</code>, and let the stride be <i>p</i>. The storage
 * works as follows.
 * Designate by <i>x(j)</i>, for <i>j=0..L-1</i>, the byte at index <i>j</i>,
 * that is, the byte
 * retrieved by <code>x.byteAt(j)</code>. Thus, we store <i>x(j)</i> at
 * <i>u(a+pj)</i>, that is,
 * <i>x(0) = u(a)</i>. When we construct such a buffer, we have to supply
 * <i>a</i> =
 * <code>index0</code>, <i>L</i> = <code>length</code>, and <i>p</i> =
 * <code>stride</code> as the
 * constructor arguments. The last item in the slice <i>x(L-1)</i> is stored at
 * <i>u(a+p(L-1))</i>.
 * For the simple case of positive stride, constructor argument
 * <code>index0</code> is the low index
 * of the range occupied by the data. When the stride is negative, that is to
 * say <i>p&lt;0</i>, and
 * <i>L&gt;1</i>, this will be to the left of <i>u(a)</i>, and the constructor
 * argument
 * <code>index0</code> is not then the low index of the range occupied by the
 * data. Clearly both
 * these indexes must be in the range 0 to <i>N-1</i> inclusive, a rule enforced
 * by the constructors
 * (unless <i>L=0</i>, when it is assumed no array access will take place).
 * <p>
 * The class may be used by exporters to create a strided slice (e.g. to export
 * the diagonal of a
 * matrix) and in particular by other buffers to create strided slices of
 * themselves, such as to
...
public class Strided1DBuffer extends BaseBuffer {
  ...
  /**
   * Step size in the underlying buffer essential to correct translation of an
   * index (or indices)
   * into an index into the storage. The value is returned by
   * {@link #getStrides()} is an array
   * with this as the only element.
   */
  protected int stride;

  /**
   * Provide an instance of <code>Strided1DBuffer</code> with navigation variables
   * partly
   * initialised, for sub-class use. To complete initialisation, the sub-class
   * normally must
   * assign the navigational properties and call {@link #checkRequestFlags(int)}
   * passing the
   ...
   * </pre>
   *
   * The pre-defined {@link #strides} field remains <code>null</code> until
   * {@link #getStrides} is
   * called.
   */
   ...
   * Provide an instance of <code>Strided1DBuffer</code> with navigation variables
   * initialised,
   * for sub-class use. The buffer ({@link #storage}, {@link #index0}), and the
   * navigation (
   * {@link #shape} array and {@link #stride}) will be initialised from the
   * arguments (which are
   * checked for range).
   * <p>
   * The sub-class constructor should check that the intended access is compatible
   * with this
   ...
   * @throws ArrayIndexOutOfBoundsException if <code>index0</code>,
   *                                        <code>length</code> and
   *                                        <code>stride</code> are inconsistent
   *                                        with <code>storage.length</code>
   */
  // TODO: "for sub-class use" = should be protected?
  ...
  /**
   * Provide an instance of <code>Strided1DBuffer</code> on a particular array of
   * bytes specifying
   * a starting index, the number of items in the result, and a byte-indexing
   * stride. The result
   * of <code>byteAt(i)</code> will be equal to
   * <code>storage[index0+stride*i]</code> (whatever
   * the sign of <code>stride</code>), valid for <code>0&lt;=i&lt;length</code>.
   * The constructor
   * checks that all these indices lie within the <code>storage</code> array
   * (unless
   * <code>length=0</code>).
   * <p>
   * The constructed <code>PyBuffer</code> meets the consumer's expectations as
   * expressed in the
   * <code>flags</code> argument, or an exception will be thrown if these are
   * incompatible with
   * the type (e.g. the consumer does not specify that it understands the strides
   * array). Note
   * that the actual range in the <code>storage</code> array, the lowest and
   * highest index, is not
   * explicitly passed, but is implicit in <code>index0</code>,
   * <code>length</code> and
   ...
   *                                        <code>stride</code> are inconsistent
   *                                        with <code>storage.length</code>
   * @throws PyException                    (BufferError) when expectations do not
   *                                        correspond with the type
   */
  ...
  protected int calcIndex(int... indices) throws IndexOutOfBoundsException {
    // BaseBuffer implementation can be simplified since if indices.length!=1 we
    // error.
  ...
  /**
   * {@inheritDoc} <code>Strided1DBuffer</code> provides a version optimised for
   * strided bytes in
   * one dimension.
   */
  @Override
   ...
   * {@inheritDoc}
   * <p>
   * <code>Strided1DBuffer</code> provides an implementation for slicing
   * already-strided bytes in
   * one dimension. In that case, <i>x(i) = u(r+ip)</i> for <i>i = 0..L-1</i>
   * where u is the
   * underlying buffer, and <i>r</i>, <i>p</i> and <i>L</i> are the start, stride
   * and length with
   * which <i>x</i> was created from <i>u</i>. Thus <i>y(k) = u(r+sp+kmp)</i>,
   * that is, the
   * composite <code>index0</code> is <i>r+sp</i> and the composite
   * <code>stride</code> is
   * <i>mp</i>.
   */
  @Override
  ...
  /**
   * A <code>Strided1DBuffer.SlicedView</code> represents a non-contiguous
   * subsequence of a simple
   * buffer.
   */
