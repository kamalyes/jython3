...
/**
 * Class implementing the Python <code>memoryview</code> type. It provides a
 * wrapper around the
 * Jython buffer API.
 */
...
public class PyMemoryView extends PySequence implements BufferProtocol, Traverseproc {
  ...
  /**
   * A memoryview in the released state forbids most Python API actions. If the
   * underlying
   * PyBuffer is shared, the memoryview may be released while the underlying
   * PyBuffer is not
   * "finally" released.
   */
  ...
  /**
   * Construct a <code>PyMemoryView</code> from an object bearing the
   * {@link BufferProtocol}
   * interface. If this object is already an exported buffer, the
   * <code>memoryview</code> takes a
   * new lease on it. The buffer so obtained will be writable if the underlying
   * object permits it.
   *
  ...
  public PyMemoryView(BufferProtocol pybuf) {
    super(TYPE);
    /*
     * Ask for the full set of facilities (strides, indirect, etc.) from the object
     * in case they
     * are necessary for navigation, but only ask for read access. If the object is
     * writable,
  ...
  /**
   * Implementation of Python <code>tobytes()</code>. Return the data in the
   * buffer as a byte
   ...
   */
  /*
   * From Python 3, this is equivalent to calling the <code>bytes</code>
   * constructor on the
  ...
  /**
   * Implementation of Python <code>tolist()</code>. Return the data in the buffer
   * as a
   * <code>list</code> where the elements are an appropriate type
   * (<code>int</code> in the case of
  ...
  /**
   * Make an integer array into a PyTuple of PyLong values or None if the argument
   * is null.
   *
  ...
  /**
   * Comparison function between two buffers of bytes, returning 1, 0 or -1 as
   * a>b, a==b, or
  ...
  /**
   * Comparison function between this memoryview and any other object. The
   * inequality comparison
   * operators are based on this.
   * <p>
   * In Python 2.7, <code>memoryview</code> objects are ordered by their
   * equivalent byte sequence
   * values, and there is no concept of a released <code>memoryview</code>. In
   * Python 3,
   * <code>memoryview</code> objects are not ordered but may be tested for
   * equality: a
   * <code>memoryview</code> is always equal to itself, and distinct
   * <code>memoryview</code>
   * objects are equal if they are not released, and view equal bytes. This method
   * supports the
  ...
  /**
   * Implementation of __eq__ (equality) operator. Comparison with an invalid type
   * returns null.
   *
  ...
  /**
   * Implementation of __ne__ (not equals) operator. Comparison with an invalid
   * type returns null.
   *
   * @param other Python object to compare with
   * @return Python boolean result or null if not implemented for the other type.
  ...
  /**
   * Implementation of __lt__ (less than) operator. Comparison with an invalid
   * type returns null.
   *
   * @param other Python object to compare with
   * @return Python boolean result or null if not implemented for the other type.
  ...
  /**
   * Implementation of __le__ (less than or equal to) operator. Comparison with an
   * invalid type
   * returns null.
   *
   * @param other Python object to compare with
  ...
  /**
   * Implementation of __ge__ (greater than or equal to) operator. Comparison with
   * an invalid type
   * returns null.
   *
   * @param other Python object to compare with
  ...
  /**
   * Implementation of __gt__ (greater than) operator. Comparison with an invalid
   * type returns
   * null.
   *
   * @param other Python object to compare with
  ...
  /*
   * These strings are adapted from the patch in CPython issue 15855 and the
   * on-line documentation
   * most attributes do not come with any docstrings in CPython 2.7, so the
   * make_pydocs trick
   * won't work. This is a complete set, although not all are needed in Python
   * 2.7.
   */
   ...
   * <code>memoryview</code> was first constructed. The Jython buffer API is such
   * that sharing
  ...
  public synchronized PyBuffer getBuffer(int flags) {
    ...
    /*
     * The PyBuffer itself does all the export counting, and since the behaviour of
     * memoryview
   ...
   * <code>release()</code> is handy to remove these restrictions (and free any
   * dangling
   * resources) as soon as possible.
   * <p>
   * After this method has been called, any further operation on the view raises a
   * <code>ValueError</code> (except <code>release()</code> itself which can be
   * called multiple
   * times with the same effect as just one call).
   * <p>
   * This becomes an exposed method in CPython from 3.2. The Jython implementation
   * of
   * <code>memoryview</code> follows the Python 3.3 design internally and
   * therefore safely
   * anticipates Python 3 in exposing <code>memoryview.release</code> along with
   * the related
  ...
  public synchronized final void memoryview_release() {
    /*
     * It is not an error to call this release method while this
     * <code>memoryview</code> has
     * buffer exports (e.g. another <code>memoryview</code> was created on it), but
     * it will not
   ...
   */
  /**
   * Gets the indexed element of the memoryview as a one byte string. This is an
   * extension point
   * called by PySequence in its implementation of {@link #__getitem__}. It is
   * guaranteed by
  ...
  /**
   * Sets the indexed element of the memoryview to the given value, treating the
   * operation as
   * assignment to a slice of length one. This is different from the same
   * operation on a byte
   * array, where the assigned value must be an int: here it must have the buffer
   * API and length
   * one. This is an extension point called by PySequence in its implementation of
   * {@link #__setitem__} It is guaranteed by PySequence that the index is within
   * the bounds of
   * the memoryview. Any other clients calling <tt>pyset(int, PyObject)</tt> must
   * make the same
   * guarantee.
   *
   ...
   * size is one, it is a simple slice and the operation is equivalent to
   * replacing that slice,
   ...
   * <p>
   * If the step size is one, but stop-start does not match the length of the
   * right-hand-side a
   * ValueError is thrown.
   * <p>
   * If the step size is not one, and start!=stop, the slice defines a certain
   * number of elements
   * to be replaced. This function is not available in Python 2.7 (but it is in
   * Python 3.3).
   * <p>
   *
   * <pre>
  ...
  protected synchronized void setslice(int start, int stop, int step, PyObject value) {
    ...
    try {
      ...
      /*
       * In the next section, we get a sliced view of the backing and write the value
       * to it.
       * The approach to errors is unusual for compatibility with CPython. We pretend
       * we will
       * not need a WRITABLE buffer in order to avoid throwing a BufferError. This
       * does not
       * stop the returned object being writable, simply avoids the check. If in fact
       * it is
