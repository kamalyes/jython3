...
/**
 * Handles all the index checking and manipulation for get, set and del
...
public abstract class SequenceIndexDelegate implements Serializable {
  ...
  public void checkIdxAndSetItem(PyObject idx, PyObject value) {
    if (idx.isIndex()) {
      ...
      throw Py.TypeError(
          getTypeName() + String.format(" indices must be integers or slices, not %s", idx.getType().fastGetName()));
    }
  }
   ...
   * {@link #checkIdxAndDelItem(PyObject)} when the argument is a {@link PySlice}.
   * The argument is
  ...
  protected void delSlice(int[] indices) {
    ...
    if (step > 1) {
      /*
       * Key observation: each deletion causes material to the right (not yet visited)
       * to move
       * one place left, so the deletion pointer moves by step-1 not by step as might
       * be
       * expected.
       */
      step = step - 1;
