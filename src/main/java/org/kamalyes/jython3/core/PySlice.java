...
public class PySlice extends PyObject implements Traverseproc {
  ...
  /**
   * Calculates the actual indices of a slice with this slice's start, stop, step
   * and
   * slicelength values for a sequence of length <code>len</code>.
   *
   * @return an array with the start at index 0, stop at index 1, step at index 2
   *         and
  ...
  public static PyObject[] indices2(PyObject obj, PyObject start, PyObject stop) {
    ...
    int istop = (stop == null || stop == Py.None)
        ? PySystemState.maxint
        : calculateSliceIndex(stop);
    if (istart < 0 || istop < 0) {
      try {
        int len = obj.__len__();
        if (istart < 0) {
