...
class JavaProxyMap {
  ...
  // Map doesn't extend Collection, so it needs its own version of len, iter and
  // contains
  ...
  // "get" needs to override java.util.Map#get() in its subclasses, too, so this
  // needs to be injected last
  // (i.e. when HashMap is loaded not when it is recursively loading its
  // super-type Map)
  ...
  private static final PyBuiltinMethodNarrow mapUpdateProxy = new MapMethod("update", 0, 1) {
    ...
    @Override
    public PyObject __call__(PyObject other) {
      // `other` is either another dict-like object, or an iterable of key/value pairs
      // (as tuples
    ...
    public PyObject __call__(PyObject[] args, String[] keywords) {
      ...
      // update with entries from `other` (adapted from their equivalent in
      // PyDictionary#update)
