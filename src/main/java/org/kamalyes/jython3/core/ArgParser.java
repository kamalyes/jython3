...
public class ArgParser {
  ...
  /**
   * Return a required argument as a PyObject, ensuring the object is of the
   * specified type.
   * 
   * @param pos  the position of the argument. First argument is numbered 0
  ...
  /**
   * Return an optional argument as a PyObject, or return the default value
   * provided, which may
   * be <code>null</code>. If the returned value is not <code>null</code>, it must
   * be of the
   * specified type.
   * 
   * @param pos  the position of the argument. First argument is numbered 0
  ...
  private void check() {
    ...
    // throw Py.TypeError(String.format("%.200s() missing %d keyword-only %s: %s",
    // funcname, missingKwonlyArgs.size(),
    // missingKwonlyArgs.size() > 1 ? "arguments" : "argument",
    // Joiner.on(',').join(missingKwonlyArgs)));
    // }
  }
