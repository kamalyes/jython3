...
public class PyInteger extends PyObject {
  ...
  /**
   * Convert all sorts of object types to either <code>PyInteger</code> or
   * <code>PyLong</code>,
   * using their {@link PyObject#__int__()} method, whether exposed or not, or if
   * that raises an
   * exception (as the base <code>PyObject</code> one does), using any
   * <code>__trunc__()</code>
   * the type may have exposed. If all this fails, this method raises an
  ...
  private static PyObject asPyInteger(PyObject x) throws PyException {
    // TODO: Not sure that this perfectly matches CPython semantics.
    try {
  ...
  /**
   * Helper called on whatever exposed method <code>__trunc__</code> returned: it
   * may be
   * <code>int</code>, <code>long</code> or something with an exposed
   * <code>__int__</code>.
   *
  ...
  // TODO::: need separate __doc__ for __repr__
  ...
  /**
   * Coercion logic for int. Implemented as a final method to avoid invocation of
   * virtual methods
  ...
  private static long divide(long x, long y) {
    ...
    // If the signs of x and y differ, and the remainder is non-0, C89 doesn't
    // define
  ...
  /**
   * Common code for PyInteger and PyLong to prepare an IntegerFormatter. This
   * object has an
