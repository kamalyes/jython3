...
public class PyReflectedFunction extends PyObject implements Traverseproc {
  ...
  public void addMethod(Method m) {
    ...
    if (isPackagedProtected(m.getDeclaringClass())) {
       ...
       * calls to
       * the method in subclasses of the package protected class will succeed. Yes,
       * it's
       * convoluted.
       *
       * This fails when done through reflection due to Sun JVM bug
       * 4071957(http://tinyurl.com/le9vo). 4533479 actually describes the problem
       * we're
      ...
      try {
        ...
        // This case is pretty far in the corner, so don't scream if we can't set the
        // method
  ...
  public PyObject __call__(PyObject self, PyObject[] args, String[] keywords) {
    ...
    // If this is a direct call to a Java class instance method with a PyProxy
    // instance as the
    // arg, use the super__ version to actually route this through the method on the
    // class.
  ...
  protected void throwBadArgError(int errArg, int nArgs, boolean self) {
    ...
    for (int i = 0; i < nargs; i++) {
      ...
      // I'm still a little worried about non-static methods called with an explicit
      // self...
