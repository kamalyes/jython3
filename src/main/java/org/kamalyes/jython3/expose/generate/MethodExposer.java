...
public abstract class MethodExposer extends Exposer {
  ...
  private void pushDefault(String def, Type arg) {
    if (def.equals("Py.None")) {
      ...
      // For primitive types, parse using the Java wrapper for that type and push onto
      // the
      // stack as a constant. If the default is malformed, a NumberFormatException
      // will be
      // raised.
      ...
      // byte, char, boolean and short go as int constants onto the stack, so convert
      // them
