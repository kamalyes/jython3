...
import org.kamalyes.jython3.core.PyBuiltinFunctionSet;
...
/**
 * This class provides the read, execute, print loop needed by a Python console;
 * it is not actually
 * a console itself. The primary capability is the {@link #interact()} method,
 * which repeatedly
 * calls {@link #raw_input(PyObject)}, and hence
 * {@link __builtin__#raw_input(PyObject)}, in order
 * to get lines, and {@link #push(String)} them into the interpreter. The
 * built-in
 * <code>raw_input()</code> method prompts on <code>sys.stdout</code> and reads
 * from
 * <code>sys.stdin</code>, the standard console. These may be redirected using
 * {@link #setOut(java.io.OutputStream)} and
 * {@link #setIn(java.io.InputStream)}, as may also
...
public class InteractiveConsole extends InteractiveInterpreter {
  ...
  /**
   * Construct an interactive console, which will "run" when {@link #interact()}
   * is called. The
   * name of the console (e.g. in error messages) will be
   * {@value #CONSOLE_FILENAME}.
   */
   ...
   * {@value #CONSOLE_FILENAME}.
   *
   * @param locals dictionary to use, or if <code>null</code>, a new empty one
   *               will be created
   */
   ...
   * @param locals   dictionary to use, or if <code>null</code>, a new empty one
   *                 will be created
   * @param filename name with which to label this console input (e.g. in error
   *                 messages).
   */
   ...
   * {@link #interact()} is called. This version allows the caller to replace the
   * built-in
   * raw_input() methods with {@link #raw_input(PyObject)} and
   * {@link #raw_input(PyObject, PyObject)}, which may be overridden in a
   * sub-class.
   *
   * @param locals          dictionary to use, or if <code>null</code>, a new
   *                        empty one will be created
   * @param filename        name with which to label this console input
   * @param replaceRawInput if true, hook this class's <code>raw_input</code> into
   *                        the built-ins.
   */
  ...
  /**
   * Operate a Python console, as in {@link #interact(String, PyObject)}, on the
   * standard input.
   * The standard input may have been redirected by
   * {@link #setIn(java.io.InputStream)} or its
  ...
  /**
   * Returns the banner to print before the first interaction: "Jython <version>
   * on <platform>".
   *
   * @return the banner.
   */
   ...
   * {@link #raw_input(PyObject, PyObject)} and
   * interpreting the lines read. An end of file causes the method to return.
   *
   * @param banner to print before accepting input, or if <code>null</code>, no
   *               banner.
   * @param file   from which to read commands, or if <code>null</code>, read the
   *               console.
   */
  ...
  public void _interact(String banner, PyObject file) {
    ...
    while (true) {
      ...
      try {
        ...
        // One would expect that it would be possible to then catch the
        // KeyboardInterrupt at the
        // bottom of this loop, however, for some reason the control-C restores the
        // input text,
        // so simply doing
   ...
   * The line is
   * appended to a buffer and the interpreter's runsource() method is called with
   * the concatenated
   * contents of the buffer as source. If this indicates that the command was
   * executed or invalid,
   * the buffer is reset; otherwise, the command is incomplete, and the buffer is
   * left as it was
   * after the line was appended. The return value is 1 if more input is required,
   * 0 if the line
  ...
  /**
   * Write a prompt and read a line from standard input. The returned line does
   * not include the
   * trailing newline. When the user enters the EOF key sequence, EOFError is
   * raised. The base
   * implementation uses the built-in function raw_input(); a subclass may replace
   * this with a
