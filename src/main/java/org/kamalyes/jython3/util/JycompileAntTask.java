...
/**
 * Compiles all python files in a directory to bytecode, and writes them to
 * another directory,
...
public class JycompileAntTask extends GlobMatchingTask {
  ...
  private void filter(File[] files) {
    for (File src : files) {
      ...
      Path classPath = src.toPath().resolveSibling(
          Paths.get(imp.CACHEDIR,
              src.getName().substring(0, src.getName().length() - 3) + Version.PY_CACHE_TAG + ".class"));
  ...
  /**
   * Compiles the python file <code>src</code> to bytecode filling in
   * <code>moduleName</code> a
   * its name, and stores it in <code>compiled</code>. This is called by process
   * for every file
   * that's compiled, so subclasses can override this method to affect or track
   * the compilation.
   */
  ...
  protected static final String getModuleName(File f) {
    ...
    // Make the compiled classfile's name fully qualified with a package by walking
    // up the
    // directory tree looking for __init__.py files. Don't check for
    // __init__.${cache_tag}.class since
    // we're compiling source here and the existence of a class file without
    // corresponding
