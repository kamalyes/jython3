...
import org.kamalyes.jython3.core.ParserFacade;
...
import org.kamalyes.jython3.core.PyFileReader;
import org.kamalyes.jython3.core.PyFileWriter;
...
public class PythonInterpreter implements AutoCloseable, Closeable {
  ...
  /**
   * Initializes the Jython runtime. This should only be called once, before any
   * other Python
   * objects (including PythonInterpreter) are created.
   *
   * @param preProperties  A set of properties. Typically System.getProperties()
   *                       is used.
   *                       preProperties override properties from the registry
   *                       file.
   * @param postProperties Another set of properties. Values like python.home,
   *                       python.path and all
   *                       other values from the registry files can be added to
   *                       this property set.
   *                       postProperties override system properties and registry
   *                       properties.
  ...
  /**
   * Creates a new interpreter with the ability to maintain a separate local
   * namespace for each
   * thread (set by invoking setLocals()).
   *
   * @param dict a Python mapping object (e.g., a dictionary) for use as the
   *             default namespace
   */
  ...
  /**
   * Sets a {@link Reader} to use for the standard input stream,
   * <code>sys.stdin</code>. This
   * stream is wrapped such that characters will be narrowed to bytes. A character
   * greater than
   * <code>U+00FF</code> will raise a Java <code>IllegalArgumentException</code>
   * from within
   * {@link PyBytes}.
   *
   ...
   * <code>sys.stdout</code>. This
   * stream is used in a byte-oriented way (mostly) that depends on the type of
   * file-like object.
  ...
  /**
   * Sets a {@link Writer} to use for the standard output stream,
   * <code>sys.stdout</code>. The
  ...
  /**
   * Compiles a string of Python source as either an expression (if possible) or a
   * module.
   *
   * Designed for use by a JSR 223 implementation: "the Scripting API does not
   * distinguish between
   * scripts which return values and those which do not, nor do they make the
   * corresponding
   ...
   * @param value the object to set the variable to (as converted to an
   ...
   * @return the value of the variable as the given class, or null if that name
   *         isn't assigned
   */
