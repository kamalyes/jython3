...
import org.kamalyes.jython3.core.*;

/**
 * This class provides the interface for compiling and running code that
...
public class InteractiveInterpreter extends PythonInterpreter {
  ...
  /**
   * Construct an InteractiveInterpreter with all default characteristics: default
   * state (from
   ...
   * {@link Py#getSystemState()}), and the
  ...
  /**
   * Construct an InteractiveInterpreter with, and system state the specified
   * dictionary of local
   * variables.
   *
   * @param locals      dictionary to use, or if <code>null</code>, a new empty
   *                    one will be created
   * @param systemState interpreter state, or if <code>null</code> use
   *                    {@link Py#getSystemState()}
   */
   ...
   * {@link CompileMode} given.
   * This method supports incremental compilation and interpretation through the
   * return value,
   * where <code>true</code> signifies that more input is expected in order to
   * complete the Python
   * statement. An interpreter can use this to decide whether to use
   * <code>sys.ps1</code> ("
   * <code>>>> </code> ") or <code>sys.ps2</code> ("<code>... </code> ") to prompt
   * the next line.
   * The arguments are the same as the mandatory ones in the Python
   * <code>compile()</code>
   * command.
   * <p>
   * One the following can happen:
   * <ol>
   * <li>The input is incorrect; compilation raised an exception (SyntaxError or
   * OverflowError). A
   * syntax traceback will be printed by calling
   * {@link #showexception(PyException)}. Return is
   * <code>false</code>.</li>
   *
   * <li>The input is incomplete, and more input is required; compilation returned
   * no code.
   * Nothing happens. Return is <code>true</code>.</li>
   *
   * <li>The input is complete; compilation returned a code object. The code is
   * executed by
   * calling {@link #runcode(PyObject)} (which also handles run-time Exceptions,
   * except for
   ...
   * @param filename name with which to label this console input (e.g. in error
   *                 messages).
   * @param kind     of compilation required: {@link CompileMode#eval},
   *                 {@link CompileMode#exec} or
   ...
   * {@link #showexception(PyException)} is
   * called to display a stack trace, except in the case of SystemExit, which is
   * re-raised.
   * <p>
  ...
  /**
   * Pause the current code, sneak an exception raiser into sys.trace_func, and
   * then continue the
