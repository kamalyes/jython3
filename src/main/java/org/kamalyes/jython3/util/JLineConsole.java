...
import org.kamalyes.jython3.core.PlainConsole;
...
public class JLineConsole extends PlainConsole {
  ...
  /**
   * <b>Not</b> currently set by <code>readline.set_pre_input_hook</code>. Why
   * not?
   */
  ...
  /**
   * Errno strerrors possibly caused by a SIGSTP (ctrl-z). They may propagate up
   * to IOException
   * messages.
   */
   ...
   * <p>
   * Most of the initialisation is deferred to the {@link #install()} method so
   * that any prior
  ...
  public JLineConsole(String encoding) {
    ...
    super(encoding);
    /*
     * Communicate the specified encoding to JLine. jline.ConsoleReader.readLine()
     * edits a line
    ...
    // ... not "jline.UnixTerminal.input.encoding" as you might think, not even in
    // JLine2
  }
   ...
   * <code>FilterInputStream</code> object that wraps JLine, and
  ...
  public void install() {
    ...
    try {
      ...
      /*
       * Everybody else, using sys.stdout or java.lang.System.out, gets to write on a
       * special
       * PrintStream that keeps the last incomplete line in case it turns out to be a
       * console
       * prompt.
       */
  ...
  /**
   * Class to wrap the line-oriented interface to JLine with an InputStream that
   * can replace
   * System.in.
   */
  private class Stream extends ConsoleInputStream {
    ...
    /**
     * Create a System.in replacement with JLine that adds system-specific line
     * endings
     */
    Stream() {
    ...
    protected CharSequence getLine() throws IOException, EOFException {
      ...
      // Get a line and hope to be done. The prompt is the current partial output
      // line.
  ...
  /**
   * Wrapper on reader.readLine(prompt) that deals with retries (on Unix) when the
   * user enters
   * ctrl-Z to background Jython, then brings it back to the foreground. The
   * inherited
  ...
  /**
   * Determine if the IOException was likely caused by a SIGSTP (ctrl-z). Seems
   * only applicable to
   * BSD platforms.
   */
