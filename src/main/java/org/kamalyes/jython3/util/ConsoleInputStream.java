...
/**
 * This class is intended to replace <code>System.in</code> for use with console
 * libraries that
 * provide a line-oriented input mechanism. The console libraries provide a
 * method to get the next
 * line from the console as a String. Particular sub-classes should wrap this
 * character-oriented
 * method in a definition of {@link #getLine()}.
 * <p>
 * The libraries JLine and Java Readline have both been used to give Jython
 * line-recall, editing and
 * a line history preserved between sessions. Both deal with the console
 * encoding internally, and
 * interact with the user in terms of a buffer of characters. Our need in Jython
 * is to access a
 * byte-stream encoding the characters, with line-endings, since it is the text
 * layer of the Python
 * io stack, whether we are using the <code>io</code> module or
 * <code>file</code> built-in, that
 * should deal with encoding.
 */
public abstract class ConsoleInputStream extends FilterInputStream {

  /**
   * Enumeration used to specify whether an end-of-line should be added or
   * replaced at the end of
   * each line read. LEAVE means process the line exactly as the library returns
   * it; ADD means
   * always add an end-of-line; and REPLACE means strip any final '\n', '\r', or
   * '\r\n' and add an
   * end-of-line. The end-of-line to add is specified as a String in the
   * constructor.
   */
  ...
  /**
   * Create a wrapper configured with end-of-line handling that matches the
   * specific console
   * library being wrapped, and a character encoding matching the expectations of
   * the client.
   * Since this is an abstract class, this constructor will be called as the first
   * action of the
   * library-specific concrete class. The end-of-line policy can be chosen from
   * <code>LEAVE</code>
   * (do not modify the line), <code>ADD</code> (always append <code>eol</code>,
   * and
   ...
   * @param eol       the end-of-line to use when <code>eolPolicy</code> is not
   *                  <code>LEAVE</code>
   */
  ...
  /**
   * Get one line of input from the console. Override this method with the actions
   * specific to the
   * library in use.
   *
  ...
  /**
   * Get a line of text from the console and re-encode it using the console
   * encoding to bytes that
   ...
   * The byte is returned as an int in the range 0 to 255. If no byte is available
   * because the end
   * of the stream has been recognised, the value -1 is returned. This method
   * blocks until input
   * data are available, the end of the stream is detected, or an exception is
   * thrown. Normally, an
   ...
   * If len is not
   * zero, the method blocks until some input is available; otherwise, no bytes
   * are read and 0 is
   * returned. This implementation calls {@link #getLine()} at most once to get a
   * line of
  ...
  /**
   * Skip forward n bytes within the current encoded line. A call to
   * <code>skip</code> will not
